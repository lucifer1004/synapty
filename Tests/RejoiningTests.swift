...
/// as a value so they can ([[PaneStrip]]).
final class PaneStripGeometryTests: XCTestCase {
    ...
    func testThePaneKeepsItsWholeRectWhenNothingIsSaid() {
        XCTAssertEqual(PaneStrip.paneRect(pane, strips: 0), pane)
    }

    func testThePaneStartsBELOWTheStripAndLosesExactlyItsHeight() {
        let inset = PaneStrip.paneRect(pane, strips: 1)
        XCTAssertEqual(inset.minY, pane.minY + PaneStrip.height,
                       "the terminal still starts where the strip does — it is covered")
        XCTAssertEqual(inset.height, pane.height - PaneStrip.height)
        XCTAssertEqual(inset.minX, pane.minX)
        XCTAssertEqual(inset.width, pane.width, "the strip takes height, not width")
    }

    /// TWO NOTICES TAKE TWICE THE ROOM. A pane that came back restarted
    /// AND has lost its link has something to say about each, and two
    /// strips that each inset by their own height would both draw at the
    /// top with the terminal under whichever won
    /// ([[WI-2026-09-07-004]]).
    func testTwoStripsEachGetTheirOwnRoomAndTheirOwnPlace() {
        let inset = PaneStrip.paneRect(pane, strips: 2)
        XCTAssertEqual(inset.minY, pane.minY + PaneStrip.height * 2)
        XCTAssertEqual(inset.height, pane.height - PaneStrip.height * 2)

        XCTAssertEqual(PaneStrip.offsetY(pane, index: 0), pane.minY)
        XCTAssertEqual(PaneStrip.offsetY(pane, index: 1), pane.minY + PaneStrip.height)
    }

    /// AND THE CALLER MUST ASK FOR THE RIGHT NUMBER. The arithmetic above
    /// was tested and the count was not: nothing said the pane is inset by
    /// TWO when both notices show, or that the rejoin notice takes the
    /// lower row. Both lived private to a `View`, so nothing could
    /// ([[WI-2026-09-08-004]]).
    ///
    /// The defect that hides is a terminal drawn UNDER a notice — the
    /// failure this file's own geometry tests were written for.
    func testTheNumberOfStripsFollowsWhatIsActuallyShowing() {
        XCTAssertEqual(PaneStrip.count(link: true, hole: false, rejoining: false), 0)
        XCTAssertEqual(PaneStrip.count(link: false, hole: false, rejoining: false), 1)
        XCTAssertEqual(PaneStrip.count(link: true, hole: false, rejoining: true), 1)
        XCTAssertEqual(PaneStrip.count(link: true, hole: true, rejoining: false), 1)
        XCTAssertEqual(PaneStrip.count(link: false, hole: false, rejoining: true), 2,
                       "a pane that came back restarted AND has lost its link "
                       + "gives up room for both")
        XCTAssertEqual(PaneStrip.count(link: false, hole: true, rejoining: true), 3,
                       "and all three can be true of one pane at once")
    }

    /// THE LINK NOTICE LEADS, because it is the condition that is true
    /// now; the hole reports something that happened during this session;
    /// the rejoin notice reports how the pane came back, which is the
    /// oldest of the three. Two notices that both took row zero would draw
    /// on each other with the terminal under whichever won.
    func testEachNoticeTakesItsOwnRowNewestFirst() {
        XCTAssertEqual(PaneStrip.holeIndex(link: false), 1)
        XCTAssertEqual(PaneStrip.holeIndex(link: true), 0)
        XCTAssertEqual(PaneStrip.rejoinIndex(link: false, hole: true), 2)
        XCTAssertEqual(PaneStrip.rejoinIndex(link: false, hole: false), 1)
        XCTAssertEqual(PaneStrip.rejoinIndex(link: true, hole: true), 1)
        XCTAssertEqual(PaneStrip.rejoinIndex(link: true, hole: false), 0)

        // And they agree with the count: with everything showing, the
        // last strip's row is one less than the room the pane gave up, so
        // nothing is drawn past it.
        let all = PaneStrip.count(link: false, hole: true, rejoining: true)
        XCTAssertEqual(PaneStrip.rejoinIndex(link: false, hole: true), all - 1)
    }

    /// A pane shorter than the strip must not be handed a negative height.
    func testATinyPaneDoesNotGetANegativeHeight() {
        let sliver = CGRect(x: 0, y: 0, width: 300, height: PaneStrip.height / 2)
        XCTAssertEqual(PaneStrip.paneRect(sliver, strips: 1).height, 0)
        XCTAssertEqual(PaneStrip.paneRect(sliver, strips: 3).height, 0,
                       "three notices on a sliver is still a rect, not a negative one")
    }
