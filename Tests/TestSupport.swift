...
/// PLACEMENT WITHOUT CREDENTIALS, for tests that never authenticate.
///
/// The pool's opening calls take the ssh arguments a connection must
/// present, resolved on the main actor by whoever owns the host records
/// ([[WI-2026-09-12-001]]). Every test here fakes `openMaster`, so there
/// is no ssh and nothing to present — and threading an empty array
/// through eighty call sites would say nothing except that it is empty.
///
/// DELIBERATELY NOT A DEFAULT ARGUMENT ON THE POOL ITSELF. A default
/// would let a production caller omit the credentials and open a master
/// with the wrong identity, which is the failure this signature exists to
/// make impossible.
extension MasterPool {
    func place(_ key: HostKey, tenant: String) -> String? {
        place(key, tenant: tenant, credentials: [])
    }

    @discardableResult
    func placeExclusive(_ key: HostKey, tenant: String) -> String? {
        placeExclusive(key, tenant: tenant, credentials: [])
    }

    @discardableResult
    func growIfLoaded(_ key: HostKey) -> String? {
        growIfLoaded(key, credentials: [])
    }

    @discardableResult
    func placeAfterRefusal(on socket: String, _ key: HostKey, tenant: String) -> String? {
        placeAfterRefusal(on: socket, key, tenant: tenant, credentials: [])
    }
}

extension TransferPlan.PoolBinding {
    init(pool: MasterPool, key: MasterPool.HostKey, tenant: String? = nil) {
        self.init(pool: pool, key: key, credentials: [], tenant: tenant)
    }
}
