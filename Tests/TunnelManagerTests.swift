...
final class TunnelManagerTests: XCTestCase {
    ...
    private func makeManager() -> TunnelManager {
        ...
        // A HUB THIS WORKBENCH HAS. `hubPort` no longer defaults to 9000 —
        // that literal was a second answer to a question
        // `HubManager.boundPort` owns, and nil now means "adoption was
        // refused", which is a state a pane command must not paper over
        // ([[WI-2026-09-10-008]]). A test that wants a hub says so.
        m.hubPort = 9000
        return m
    }
    ...
    /// What still rides the environment, and that it gets there through
    /// `env`.
    ///
    /// LESS THAN IT USED TO. The list was positional and ended in a
    /// variadic run of forwarding quads, so anything else had to be a
    /// variable; with named options there is room for the name to start
    /// under and the directory to open in, and those two moved. What is
    /// left is what the workbench tells the whole connection rather than
    /// this one call: which binary, whether the host keeps sessions, which
    /// connection to ride, and where to write the account ([[ADR-0020]]).
    func testEnvironmentIsCarriedByEnv() {
        ...
        let verb = tokens.firstIndex(of: "connect") ?? tokens.count
        let assignments = tokens[(envIndex + 1)..<(verb - 1)]
        XCTAssertTrue(assignments.allSatisfy { $0.contains("=") },
                      "a non-assignment sits between env and the command: \(assignments)")
        XCTAssertTrue(assignments.contains { $0.hasPrefix("SYNAPTY_DURABLE=") })
        XCTAssertFalse(assignments.contains { $0.hasPrefix("SYNAPTY_FRESH_ID=") },
                       "the name to start under is an option now, not a variable")
    }

    /// The command, with any leading environment assignments dropped:
    /// counting from the front of the string makes every index a hostage
    /// to whether one happens to be set.
    private func commandTokens(_ command: String) -> [String] {
        let all = command.split(separator: " ").map(String.init)
        let start = all.firstIndex(of: "connect").map { $0 - 1 } ?? 0
        return Array(all[start...])
    }

    /// The value following a flag, or nil.
    private func value(_ flag: String, in tokens: [String]) -> String? {
        guard let i = tokens.firstIndex(of: flag), i + 1 < tokens.count else { return nil }
        return tokens[i + 1]
    }

    /// THE PANE RUNS THE BINARY, NOT A SCRIPT ([[ADR-0020]]).
    ///
    /// NAMED OPTIONS RATHER THAN NINE POSITIONALS. The old layout was the
    /// script's to own and this test restated its order; two of those
    /// positionals — the tunnel port and the hub port — were never read by
    /// it at all and were carried for years because a fixed layout cannot
    /// drop a slot in the middle.
    func testConnectCommandNamesItsOptions() {
        let m = makeManager()
        ...
        XCTAssertEqual(tokens[1], "connect")
        XCTAssertTrue(tokens[0].hasSuffix("synapty'") || tokens[0] == "synapty",
                      "the pane's command is not the binary: \(tokens[0])")
        XCTAssertEqual(value("--id", in: tokens), "'\(result.agentID)'")
        XCTAssertEqual(value("--host", in: tokens), "'10.0.1.5'")
        XCTAssertEqual(value("--port", in: tokens), "2222")
        XCTAssertEqual(value("--user", in: tokens), "'ml'")
        XCTAssertEqual(value("--key", in: tokens), "'/keys/gpu'")
        XCTAssertNil(value("--jump", in: tokens))
        XCTAssertNil(value("--forward", in: tokens))
    }

    /// AN ABSENT OPTION IS ABSENT, rather than an empty positional holding
    /// a slot open. The script needed `''` for a key it did not have,
    /// because the slot after it had to stay in the right place.
    func testConnectCommandOmitsWhatTheHostDoesNotHave() {
        let m = makeManager()
        let host = HostEntry(label: "Min", address: "localhost", username: "u")
        let tokens = commandTokens(m.connectCommand(for: host).command)
        XCTAssertNil(value("--key", in: tokens))
        XCTAssertNil(value("--jump", in: tokens))
        XCTAssertFalse(tokens.contains("''"), "an empty placeholder survived: \(tokens)")
    }

    /// ONE ARGUMENT EACH, which is what made room for everything else.
    func testConnectCommandForwardingsAreOneArgumentEach() {
        let m = makeManager()
        var host = HostEntry(label: "Fwd", address: "10.0.1.6", username: "u")
        host.forwardings = [
            PortForward(kind: .local, listenPort: 8080, targetHost: "localhost", targetPort: 80),
            PortForward(kind: .remote, listenPort: 9000, targetHost: "127.0.0.1", targetPort: 9001),
        ]
        let tokens = commandTokens(m.connectCommand(for: host).command)
        XCTAssertEqual(tokens.filter { $0 == "--forward" }.count, 2)
        XCTAssertTrue(tokens.contains("'local:8080:localhost:80'"), "\(tokens)")
        XCTAssertTrue(tokens.contains("'remote:9000:127.0.0.1:9001'"), "\(tokens)")
    }

    /// WHAT THE PANE SENDS IS WHAT THE CLI CAN READ. The empty target and
    /// the IPv6 literal are both shapes the forwardings editor produces,
    /// and both used to leave here as a spec `synapty connect` dropped in
    /// silence while dialling the pane anyway ([[WI-2026-09-10-002]]).
    func testConnectCommandNormalisesTargetsTheCLIWouldRefuse() {
        let m = makeManager()
        var host = HostEntry(label: "Fwd", address: "10.0.1.6", username: "u")
        host.forwardings = [
            PortForward(kind: .local, listenPort: 8080, targetHost: "", targetPort: 80),
            PortForward(kind: .local, listenPort: 8081, targetHost: "::1", targetPort: 80),
        ]
        let tokens = commandTokens(m.connectCommand(for: host).command)
        XCTAssertTrue(tokens.contains("'local:8080:localhost:80'"), "\(tokens)")
        XCTAssertTrue(tokens.contains("'local:8081:[::1]:80'"), "\(tokens)")
    }
    ...
    func testALocalPaneStartsAHolderAndThenAttachesToIt() throws {
        ...
        XCTAssertTrue(result.command.contains("--hub 127.0.0.1:9000"),
                      "the pane was not told where its hub is: \(result.command)")
        XCTAssertFalse(result.command.contains("--parent-pid"),
                       "a durable pane still hangs up when the workbench exits")
    }

    /// WITH NO HUB, NOTHING IS ADDRESSED TO 9000.
    ///
    /// `HubManager.boundPort` answers nil in exactly the cases where there
    /// is no hub to talk to — the adoption was refused as a `conflict`, or
    /// the spawn failed — and every consumer used to substitute the
    /// literal 9000 for that nil. 9000 is the one number that must not
    /// stand in for it: it names the hub the refusal was ABOUT. The
    /// workbench told the human A2A was unavailable and went on pointing
    /// panes, wake reports, exec receipts, tool receipts and passive
    /// detection into it ([[WI-2026-09-10-008]]).
    func testWithNoHubThePaneIsNotToldThereIsOneAt9000() {
        let m = TunnelManager()
        heldStore = HostStore()
        m.hostStore = heldStore
        XCTAssertNil(m.hubPort, "a fresh workbench must not invent a hub")

        let result = m.localCommand()
        // A WORD, NOT AN OMISSION. This asserted the flag was ABSENT, and
        // absence is not the fact: an absent `--hub` resolves
        // SYNAPTY_HUB_PORT, then the discovery file, then 9000, so the
        // pane landed on the refused hub exactly as before and only the
        // command string had changed. The far side needed a way to be
        // told there is no hub, and now has one ([[WI-2026-09-12-002]]).
        XCTAssertTrue(result.command.contains("--hub none"),
                      "a pane was left to resolve a hub on a machine that has none: \(result.command)")
        // THE ADDRESS, NOT THE DIGITS. This asserted the command held no
        // "9000" anywhere, which a `--parent-pid` of 90002 satisfies by
        // accident — it passed for a run and failed for the next one on a
        // different pid ([[WI-2026-09-11-007]]).
        XCTAssertFalse(result.command.contains("127.0.0.1:9000"),
                       "the refused hub address reached the pane anyway: \(result.command)")
        // And the pane still opens: a terminal without A2A is degraded,
        // not broken, which is why this omits a flag rather than refusing.
        // Asserted across BOTH branches of the durability setting, since
        // which one this run takes is the human's preference and not this
        // test's subject.
        XCTAssertTrue(result.command.contains("run --id") ||
                      result.command.contains("run --hold --detach --id"),
                      "omitting the hub cost the pane its shell: \(result.command)")
    }

    /// THE SERVICES HOLD NO PORT OF THEIR OWN EITHER. Each kept a
    /// `private var port: Int = 9000` placeholder and was pointed once at
    /// launch, so four of them had no path back when the hub moved.
    @MainActor
    func testNoServiceInventsAHubPort() {
        XCTAssertNil(AgentDetector().hubPort)
        XCTAssertNil(WakeCoordinator().hubPort)
        XCTAssertNil(ExecController().hubPort)
        XCTAssertNil(ToolBridge().hubPort)
    }
...
extension TunnelManagerTests {
    ...
    /// WI-2026-08-12-008: `synapty host-setup` reports the loopback port that
    ...
    /// A SETUP THAT RAN AND FAILED IS A FAILURE.
    ///
    /// The success test was `output.error == nil && !output.timedOut`, and
    /// `Output.error` is documented as a LAUNCH failure — the process
    /// could not start. Neither field is set by a non-zero exit, so a host
    /// that refuses key authentication, runs an unsupported platform, or
    /// has no cross-compiled binary for it exited 1 or 2 and was recorded
    /// as CONNECTED: forwards that were never made recorded, every pending
    /// pane callback fired against a host with no binary deployed, and the
    /// host stamped to the top of the recently-used list
    /// ([[WI-2026-09-10-001]]).
    ///
    /// AND `SETUP_OK` IS READ, WHICH MAKES A DOCUMENTED CONTRACT TRUE.
    /// `synapty host-setup --help` says "its stdout is read by the
    /// workbench … `SETUP_OK` is the last line of a run that got that
    /// far", and nothing in the app read it. A status alone would have
    /// been enough today; the marker is what says the run reached its end
    /// rather than exiting zero somewhere on the way.
    func testASetupThatRanAndFailedIsNotASuccess() {
        let ok = SubprocessRunner.Output(
            stdout: "Remote hub ready on h:9000 (loopback there)\nPEER_PORT=9007\nSETUP_OK\n",
            stderr: "", timedOut: false, error: nil, exitCode: 0)
        XCTAssertTrue(TunnelManager.setupSucceeded(ok))

        // The measured failures: a host that refuses auth exits 1, a
        // forwarding rule the binary will not accept exits 2.
        for code: Int32 in [1, 2] {
            let failed = SubprocessRunner.Output(
                stdout: "Detecting remote platform...\n",
                stderr: "someone@h: Permission denied (publickey).\n",
                timedOut: false, error: nil, exitCode: code)
            XCTAssertFalse(TunnelManager.setupSucceeded(failed),
                           "a setup that exited \(code) was recorded as a connected host")
        }

        // A run killed at the timeout, and one that could not start.
        XCTAssertFalse(TunnelManager.setupSucceeded(SubprocessRunner.Output(
            stdout: "SETUP_OK\n", stderr: "", timedOut: true, error: nil, exitCode: 0)))
        XCTAssertFalse(TunnelManager.setupSucceeded(SubprocessRunner.Output(
            stdout: "", stderr: "", timedOut: false, error: "no such file", exitCode: nil)))

        // AND AN EXIT ZERO THAT NEVER REACHED THE END. Nothing produces
        // this today — every error path exits non-zero before the marker —
        // which is the point of asserting on the marker rather than on the
        // status alone.
        XCTAssertFalse(TunnelManager.setupSucceeded(SubprocessRunner.Output(
            stdout: "Detecting remote platform...\n", stderr: "",
...
extension TunnelManagerTests {
    ...
    /// session. The far side asks whether a holder of that name is running
    /// and joins it when one is, so the id has to travel into the command
    /// — a generated one would start a second agent beside the one the
    /// human was trying to reach.
    ///
    /// (The comment here named tmux and a `synapty-<agent-id>` session
    /// long after [[ADR-0012]] replaced tmux with the holder. A comment
    /// describing a mechanism is a claim, and this one had been wrong for
    /// months without the assertion below noticing.)
    @MainActor
...
extension TunnelManagerTests {
    ...
    /// told us".
    ///
    /// AND THE LIST IS THE HUB'S, NOT OURS. This asserted only that a
    /// hardcoded array was non-empty, which is true of any array anyone
    /// ever writes there — including the one that went stale the hour
    /// `pending-wake` was added on the Zig side and not here
    /// ([[WI-2026-09-10-010]]).
    func testTheCapabilityListIsTheHubsAndAnUnansweredHubNamesNoAbsence() {
        let store = HostStore()
        store.hosts.append(HostEntry(label: "builder", address: "b.example", username: "u"))
        let manager = TunnelManager()
        manager.hostStore = store

        // Nothing asked yet: no absence may be named about anyone.
        manager.adoptExistingPeers([(peer: "builder", port: 9200)],
                                   capabilities: ["builder": ["presence_relay"]])
        XCTAssertTrue(manager.knownCapabilities.isEmpty)
        for summary in manager.peerSummaries {
            XCTAssertNil(summary.missing,
                         "an unanswered hub let us claim a peer was missing nothing")
        }

        // The hub answers, and the peer's gap becomes nameable — with the
        // word the hub used, not one this side invented.
        manager.adoptExistingPeers([(peer: "builder", port: 9200)],
                                   capabilities: ["builder": ["presence_relay"]],
                                   ours: ["presence_relay", "pending-wake"])
        let named = manager.peerSummaries.first { $0.peerID == "builder" }
        XCTAssertEqual(named?.missing, ["pending-wake"])
    }
    ...
    func testAPaneNamesItsConnectionAndGivesItBackWhenItCloses() throws {
        ...
        tm.pool.openMaster = { _, path, _ in
    ...
    func testAPaneCommandForAHostHoldingNothingLeavesTheScriptToOpenItsOwn() throws {
        ...
        tm.hostStore = HostStore()
        var opens = 0
        tm.pool.openMaster = { _, _, _ in opens += 1; return true }
        let host = HostEntry(label: "builder", address: "builder.example", username: "someone")
    ...
    /// THE CREDENTIALS A MASTER IS OPENED WITH COME FROM THE ACTOR THAT
    /// OWNS THE RECORDS, AND TRAVEL WITH THE KEY.
    ///
    /// `MasterPool` used to hold a `sshArgs` closure, installed by this
    /// class and reading its `@MainActor` `trackedHosts`. `grow` called
    /// it after dropping the pool's lock, on one of the background
    /// threads `MasterPool`'s own header REQUIRES — while the heartbeat's
    /// main hop mutates that dictionary in place. A background
    /// `values.first(where:)` over a Dictionary being rehashed is not a
    /// stale read; the crash is inside Dictionary's internals, and the
    /// milder outcome is a master opened with another host's `-i` or `-J`
    /// which then fails to authenticate and reads as an SSH problem
    /// ([[WI-2026-09-12-001]]).
    ///
    /// Nothing in the type system was left to notice: the property's type
    /// is a bare `(HostKey) -> [String]`, which erases the isolation the
    /// closure was written in, and `MasterPool` is `@unchecked Sendable`.
    ///
    /// This asserts the replacement end to end: what the main actor
    /// resolved for THIS host is what the pool would spawn ssh with.
    func testAMasterIsOpenedWithTheCredentialsTheRecordAsksFor() throws {
        ...
        let key = m.poolKey(for: host)
        let credentials = m.poolCredentials(for: host)
        XCTAssertEqual(credentials, ["-J", "admin@bastion:2222"],
                       "the host is only reachable through a bastion, and this is how")

        var spawnedWith: [String]?
        m.pool.openMaster = { _, path, args in
            spawnedWith = args
            FileManager.default.createFile(atPath: path, contents: nil)
            return true
        }
        XCTAssertNotNil(m.pool.place(key, tenant: "pane.a", credentials: credentials),
                        "nothing was opened, so nothing was presented")
        XCTAssertEqual(spawnedWith, credentials,
                       "the master was opened without what the record says it needs")
    }
