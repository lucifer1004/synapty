...
final class TerminalPaneManagerTests: XCTestCase {
    ...
    func testAddRemoteSession() {
        ...
        let manager = makeManager()
        manager.addRemoteWorkspace(label: "GPU Box", hostEntry: host, command: "synapty connect --id agent-1 --host 10.0.1.5 --port 22 --user user")
    ...
    func testCompletingADialDoesNotSwitchTheActiveWorkspace() {
        ...
        manager.paneDidConnect(paneID, command: "synapty connect --id a --host 10.0.1.5 --port 22 --user user", agentID: "a")
    ...
    func testSplittingARemoteTerminalCarriesTheDirectoryToTheFarSide() {
        ...
        // An option rather than a variable, for the same reason
        // `--fresh-id` is ([[ADR-0020]]).
        XCTAssertTrue(command.contains("--cwd "), "got: \(command)")
    ...
    func testFocusLeafDoesNotSwitchActiveSession() {
        ...
        manager.addRemoteWorkspace(label: "GPU Box", hostEntry: host,
                                command: "synapty connect --id agent-1 --host 10.0.1.5 --port 22 --user user", agentID: "agent-1")
...
final class ReorderingTests: XCTestCase {
    ...
    func testAPaneDockedAmongAnotherMachinesPanesStillReportsItsOwnHost() {
        ...
        manager.paneDidConnect(remotePane, command: "synapty connect --id a --host 10.0.1.5 --port 22 --user user",
                               agentID: "a")
    ...
    func testATravellingPaneKeepsItsOwnConnection() {
        ...
        manager.paneDidConnect(manager.workspaces.first { $0.id == remoteID }!.panes[0].id,
                               command: "synapty connect --id a --host 10.0.1.5 --port 22 --user user", agentID: "a")
    ...
    func testALinkBroughtUpWithoutAPaneStillLetsItsLeavesThrough() {
        ...
        manager.addRemoteWorkspace(label: "GPU Box", hostEntry: host,
                                   command: "synapty connect --id a --host 10.0.1.5 --port 22 --user user")
    ...
    func testAPaneOnAnAlreadyReachedHostWaitsForItsOwnCommand() {
        ...
        manager.paneDidConnect(first, command: "synapty connect --id a --host 10.0.1.5 --port 22 --user user",
                               agentID: "a")
        ...
        manager.paneDidConnect(second, command: "synapty connect --id b --host 10.0.1.5 --port 22 --user user",
                               agentID: "b")

        XCTAssertEqual(manager.surface(of: second), .terminal)
        XCTAssertEqual(manager.leafContent(second)?.terminalCommand,
                       "synapty connect --id b --host 10.0.1.5 --port 22 --user user",
    ...
    func testTheLeafTakesItsCommandWhenTheDialReturns() {
        ...
        XCTAssertEqual(pane.content.terminalCommand, "synapty connect --id a --host 10.0.1.5 --port 22 --user user")
...
final class AttentionTests: XCTestCase {
    ...
    /// REVEALING IT CLEARS IT TOO, and the sidebar's agent glyph follows
    /// the same record the workspace row does.
    ///
    /// There were TWO records — `LeafFacts.needsAttention` keyed by leaf
    /// and `AgentMonitor.needsAttention` keyed by agent — written in
    /// lockstep and cleared by different gestures. `revealLeaf`, which is
    /// what the go-to-pane palette calls, cleared the leaf's and left the
    /// agent's set, and unlike `focusLeaf` it fires no gaze, so nothing
    /// ever reconciled them: the row kept its mark on a pane the human was
    /// reading ([[WI-2026-09-11-008]]).
    func testRevealingALeafClearsTheOneAttentionRecord() {
        let manager = makeManager()
        manager.splitFocusedLeaf(direction: .horizontal)
        let leaves = manager.workspaces[0].panes
        manager.focusSlot(index: 2)
        ...
        manager.revealLeaf(leaves[0].id)

        // BOTH READERS, because the defect was that one of them lagged.
        XCTAssertFalse(manager.isAwaitingAttention(leaves[0].id),
                       "the glyph the sidebar colours still says this pane wants me")
        XCTAssertFalse(manager.workspaceNeedsAttention(manager.workspaces[0]))
        XCTAssertEqual(manager.attentionCount, 0)
    }
...
final class RemoteAgentEndingTests: XCTestCase {
    ...
    private func makeRemote(
        ...
        let paneID = manager.workspaces.first { $0.id == id }!.panes[0].id
        manager.paneDidConnect(paneID, command: "synapty connect --id gpu-first", agentID: "gpu-first")
        return (manager, id)
    }
    ...
    /// ONLY its own agent.
    func testEndingOneArchivedPaneTouchesOnlyItsOwnAgent() async {
        var ended: [String] = []
        let (manager, id) = makeRemote { ended.append($0) }
        manager.addPaneToActiveWorkspace()
        ...
        await manager.endArchivedPane(second.id)
    ...
    func testClosingASplitSetsItAsideAndLeavesItsSibling() async {
        var ended: [String] = []
        let (manager, id) = makeRemote { ended.append($0) }
        manager.splitFocusedLeaf(direction: .horizontal)
        ...
        await manager.endArchivedPane(focused)
    ...
    func testEndingAArchivedPaneTwiceEndsItOnce() async {
        var ended: [String] = []
        let (manager, id) = makeRemote { ended.append($0) }
