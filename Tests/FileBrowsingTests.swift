...
final class FileBrowsingTests: XCTestCase {
    ...
    func testTheAddressBarNamesTheDestinationBeforeTheAnswerArrives() {
        ...
        let ticket = browsing.request()
        XCTAssertEqual(browsing.address, "/var/log")
        XCTAssertTrue(browsing.here.isLoading)

        browsing.arrive(at: "/var/log", files: rows, answering: ticket)
        XCTAssertEqual(browsing.address, "/var/log")
    ...
    func testAFailedListingLeavesThePaneWhereItWasWithNothingClickable() {
        ...
        browsing.fail(answering: ticket)
    ...
    // MARK: - Which reply is the answer ([[WI-2026-09-23-005]])

    /// THE DEFECT. A refresh after a delete asks for the same directory
    /// again, and the listing from BEFORE the delete can land after the one
    /// that followed it. Compared by path the two were the same answer, so
    /// the deleted file came back.
    func testAnOlderListingOfTheSameDirectoryCannotOverwriteANewerOne() {
        var browsing = FileBrowsing()
        browsing.here = .at(Listing(path: "/srv", files: []))
        let before = [BrowsedFile(name: "doomed.txt", size: 1, modified: nil,
                                  isDirectory: false, isSymlink: false, isBrokenLink: false)]

        let older = browsing.request()    // in flight when the file is deleted
        let newer = browsing.request()    // the refresh the delete triggered

        XCTAssertTrue(browsing.arrive(at: "/srv", files: [], answering: newer))
        XCTAssertFalse(browsing.arrive(at: "/srv", files: before, answering: older),
                       "a listing from before the delete settled the pane after the one "
                       + "that followed it")
        XCTAssertEqual(browsing.here.showing.files, [], "the deleted file came back")
    }

    /// AND THE CASE THE PATH CHECK DID COVER STILL IS: a reply for the
    /// directory being left must not move the human back to it — including
    /// in the moment after leaving and before the new request goes out.
    func testAReplyForTheDirectoryBeingLeftCannotMoveTheHumanBack() {
        var browsing = FileBrowsing()
        browsing.here = .at(Listing(path: "/home/me", files: rows))
        let stale = browsing.request()

        browsing.navigate(to: "/var/log", cached: nil)

        XCTAssertFalse(browsing.arrive(at: "/home/me", files: rows, answering: stale))
        XCTAssertEqual(browsing.here.destination, "/var/log",
                       "a reply for where the human was moved them back there")
        XCTAssertFalse(browsing.fail(answering: stale),
                       "a failure for where the human was stopped their journey")
    }
    ...
    func testArrivalTakesTheHostsAnswerForWhereItLanded() {
        ...
        let ticket = browsing.request()
        browsing.arrive(at: "/home/me/src", files: rows, answering: ticket)
