...
final class PortForwardServiceTests: XCTestCase {
    ...
    // MARK: - A list that changes during a round trip ([[WI-2026-09-23-003]])

    /// THE SECOND AWAIT. `expose` re-checks the list after its first round
    /// trip, because a second expose of the same port can land during it
    /// ([[WI-2026-09-02-033]]); the loser then closes its own forward — a
    /// second round trip, added later — and went on using the index it
    /// found BEFORE that one. A withdrawal in between moved every later
    /// exposure down a place.
    ///
    /// The interleaving is forced, not hoped for: each ssh control action
    /// for the port under test can be parked until the test lets it go.
    func testTheLoserOfARaceSettlesOnTheWinnerEvenIfTheListMovedWhileItClosed() async throws {
        let hosts = try seededStore()
        let gate = ControlGate()
        gate.hold("forward", port: 8888, nth: 1)
        gate.hold("cancel", port: 8888, nth: 1)
        service.control = { _, action, spec, _ in gate.call(action, spec: spec) }

        guard case .ok(let first) = await service.expose(
            hostID: hosts.a, remotePort: 7000, agent: "e0", title: nil, path: "/e0")
        else { return XCTFail("setup: the first exposure was refused") }

        // Y asks for 8888 and parks in its forward.
        let y = Task { await service.expose(
            hostID: hosts.a, remotePort: 8888, agent: "y", title: nil, path: "/y") }
        await until { gate.reached("forward", port: 8888, nth: 1) }

        // X asks for the same port and gets there first.
        guard case .ok = await service.expose(
            hostID: hosts.a, remotePort: 8888, agent: "x", title: nil, path: "/x")
        else { return XCTFail("setup: the winning exposure was refused") }

        // Y finds X, loses, and parks while closing its own forward.
        gate.release("forward", port: 8888, nth: 1)
        await until { gate.reached("cancel", port: 8888, nth: 1) }

        // THE LIST MOVES UNDER IT: the exposure ahead of X goes, and a new
        // one lands behind it — so X's old index now names somebody else.
        await service.withdraw(first.id)
        guard case .ok(let later) = await service.expose(
            hostID: hosts.a, remotePort: 7001, agent: "e2", title: nil, path: "/e2")
        else { return XCTFail("setup: the later exposure was refused") }

        gate.release("cancel", port: 8888, nth: 1)
        guard case .ok(let settled) = await y.value
        else { return XCTFail("the loser was refused although the winner is still there") }

        XCTAssertEqual(settled.remotePort, 8888,
                       "the loser was handed back a different service")
        XCTAssertEqual(service.exposures.first { $0.id == later.id }?.url.path, "/e2",
                       "the loser rewrote the path of an exposure it had nothing to do with")
    }
    ...
    private func seededStore() throws -> (a: UUID, b: UUID) {
        ...
        tunnels.pool.openMaster = { _, path, _ in
...
/// PARKS AN SSH CONTROL ACTION until the test lets it go, by action, by
/// the remote port in its spec, and by how many times that pair has been
/// asked for — so two calls for the same port can be told apart without
/// knowing which local port each was given.
private final class ControlGate: @unchecked Sendable {
    ...
    private static func key(_ action: String, _ port: Int, _ nth: Int) -> String {
        "\(action):\(port)#\(nth)"
    }

    func hold(_ action: String, port: Int, nth: Int) {
        lock.withLock { held[Self.key(action, port, nth)] = DispatchSemaphore(value: 0) }
    }

    func call(_ action: String, spec: String) -> Bool {
        let port = Int(spec.split(separator: ":").last ?? "") ?? -1
        let gate: DispatchSemaphore? = lock.withLock {
            let base = "\(action):\(port)"
            counts[base, default: 0] += 1
            let key = Self.key(action, port, counts[base]!)
            arrived.insert(key)
            return held[key]
        }
        gate?.wait()
    ...
    func reached(_ action: String, port: Int, nth: Int) -> Bool {
        lock.withLock { arrived.contains(Self.key(action, port, nth)) }
    }

    func release(_ action: String, port: Int, nth: Int) {
        lock.withLock { held[Self.key(action, port, nth)] }?.signal()
    }
}
