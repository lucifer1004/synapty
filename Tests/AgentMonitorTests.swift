...
extension AgentMonitorParserTests {
    ...
    /// [[RFC-0011]] C-DEFAULT: "A consumer that renders a missing field as
    /// 'no pending wake' has not declined the capability, it has MISREAD
    /// it — and the concrete failure is that an agent with unread mail on
    /// an older peer looks identical to one with none."
    @MainActor
    func testAnAbsentPendingWakeIsNotTheSameAsFalse() {
        let m = AgentMonitor()
        m.applySnapshot([
            ["id": "claude-waiting001", "tool": "claude", "project": "/p", "session": "s",
             "status": "idle", "pending_wake": true],
            ["id": "claude-drained001", "tool": "claude", "project": "/p", "session": "s",
             "status": "idle", "pending_wake": false],
            // A peer whose build cannot report it. The hub omits the
            // field and says why in the one beside it.
            ["id": "claude-older0001", "tool": "claude", "project": "/p", "session": "s",
             "status": "idle", "pending_wake_unknown_cause": "peer_lacks_capability"],
        ], machine: "remotehost-7f3a")

        let waiting = m.agents.first { $0.id == "claude-waiting001" }
        let drained = m.agents.first { $0.id == "claude-drained001" }
        let older = m.agents.first { $0.id == "claude-older0001" }

        XCTAssertEqual(waiting?.pendingWake, true)
        XCTAssertEqual(drained?.pendingWake, false)
        // THE ONE THAT MATTERS: nil, not false. `XCTAssertEqual(_, false)`
        // would pass for nil under an implicit unwrap, so this is written
        // as the nil check it is.
        XCTAssertNil(older?.pendingWake ?? nil)
        XCTAssertEqual(older?.pendingWakeUnknownCause, .peerLacksCapability)
        XCTAssertNil(waiting?.pendingWakeUnknownCause ?? nil)

        // AND IT REACHES A HUMAN, which is the whole obligation: a fact
        // with no reader is what C-DEFAULT wrote this carrier to prevent.
        XCTAssertEqual(waiting?.pendingWakePhrase, "has mail nobody has announced")
        XCTAssertNil(drained?.pendingWakePhrase)
        XCTAssertTrue(older?.pendingWakePhrase?.contains("build") == true,
                      "a peer that cannot report unread mail must read as a property of its SOFTWARE")
    }
    ...
    func testAToolRequestFromAPeerCarriesAQualifiedRequester() {
        let m = AgentMonitor()
        // THE PEER IS KNOWN, as it is whenever its events are arriving.
        // These tests used to skip this and passed only because an unknown
        // peer's reply fell back to the local hub ([[WI-2026-09-23-007]]).
        m.attachPeer(machine: "deskmac-2630", port: 9310)
        var seen: String?
    ...
    /// THE ANSWER GOES TO THE HUB HOLDING THE REQUEST ([[WI-2026-09-23-007]]).
    /// Nothing checked the port at all: the tests above ignored it, and
    /// they passed while an unknown peer's reply went to the local hub.
    @MainActor
    func testAToolRequestFromAPeerIsAnsweredOnThatPeersPort() {
        let m = AgentMonitor()
        m.attachPeer(machine: "deskmac-2630", port: 9310)
        var replyPort: Int?
        m.onToolRequest = { _, port in replyPort = port }
        m.applyEvent([
            "type": "tool_request", "tool": "view.ask", "request_id": "r1",
            "requester": "local-1a2b", "args": [:],
        ], machine: "deskmac-2630")

        XCTAssertEqual(replyPort, 9310)
    }

    /// AND A REQUEST FROM A HUB THIS SIDE CAN NO LONGER REACH IS NOT
    /// ANSWERED AT ALL. It used to go to the local hub, which drops the
    /// receipt as "requester gone" — leaving the agent that asked blocked
    /// forever, with nothing anywhere saying why. Reachable: an event
    /// already queued on the main thread arrives after `forgetPeer`.
    @MainActor
    func testAToolRequestFromAForgottenPeerIsNotSentToTheLocalHub() {
        let m = AgentMonitor()
        m.attachPeer(machine: "deskmac-2630", port: 9310)
        m.forgetPeer("deskmac-2630")
        var delivered = false
        m.onToolRequest = { _, _ in delivered = true }
        m.applyEvent([
            "type": "tool_request", "tool": "view.ask", "request_id": "r1",
            "requester": "local-1a2b", "args": [:],
        ], machine: "deskmac-2630")
        ...
        XCTAssertFalse(delivered,
                       "a request from a hub this side cannot reach was answered to another one")
    }
