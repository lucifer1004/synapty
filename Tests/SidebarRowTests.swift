...
final class SidebarRowTests: XCTestCase {
    ...
    func testAWorkspaceWithABrokenConnectionSaysWhatBroke() {
        ...
        // THE ONE THE ROW READS. The dot used to spell out
        // a disjunction of its own while the
        // sentence beside it asked `failureReason`, which answers for both
        // halves — two expressions for one fact, and they had already
        // diverged once ([[WI-2026-09-11-008]]).
        XCTAssertEqual(manager.failureReason(workspace, now: Date()), "host key changed")
    }
    ...
    func testAHealthyWorkspaceReportsNothing() {
        ...
        manager.addLocalWorkspace()
        XCTAssertNil(manager.firstFailure(manager.workspaces[0]))
        XCTAssertNil(manager.failureReason(manager.workspaces[0], now: Date()),
                     "a healthy workspace must wear no dot and give no reason")
    }
}
