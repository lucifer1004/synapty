...
/// What the approval sheet and the badge count as "waiting on the human".
///
/// [[WI-2026-09-15-002]]
@MainActor
final class QuestionWaitingTests: XCTestCase {

    /// A QUESTION NOBODY IS WAITING ON IS NOT A DECISION.
    ///
    /// `synapty ask` polls once a second until its `--timeout` and then
    /// gives up with exit 3. The question stayed in the service forever,
    /// so the approval sheet went on saying "<who> is waiting on this"
    /// about an agent that had left, a human answering it unblocked
    /// nobody, and the badge stayed lit for the life of the process.
    ///
    /// [[FileToolServer]] records that exact consequence for a DIFFERENT
    /// cause — a question with no options, which it refuses at the door —
    /// and calls the badge "a queue of DECISIONS". This is the same
    /// sentence with the other cause.
    func testAQuestionNobodyIsWaitingOnLeavesTheQueue() {
        let questions = QuestionService()
        let id = questions.ask(agent: "claude-abc1", text: "Deploy to production?",
                               options: ["yes", "no"])
        XCTAssertEqual(questions.unanswered.count, 1,
                       "a question just asked is one somebody is waiting on")

        // THE POLL IS THE LIVENESS, and it is the only thing an asker does
        // while it waits — so no deadline rides the wire, where a second
        // copy of the fact could only disagree with the polling.
        XCTAssertNil(questions.answer(to: id))
        XCTAssertEqual(questions.unanswered.count, 1)

        // Silence past the bound: the agent gave up, or is gone.
        questions.forgetPollsForTesting(olderThan: QuestionService.waiterSilence + 1)
        XCTAssertEqual(questions.unanswered.count, 0,
                       "the sheet would have said an agent that had left was waiting")

        // AND IT COMES BACK WITH ITS ASKER. An agent that re-asks gets the
        // same id, and its next poll says it is waiting again — so a
        // retrying agent is not made invisible by having once given up.
        let again = questions.ask(agent: "claude-abc1", text: "Deploy to production?",
                                  options: ["yes", "no"])
        XCTAssertEqual(again, id, "re-asking must not make a second card")
        XCTAssertNil(questions.answer(to: again))
        XCTAssertEqual(questions.unanswered.count, 1)
    }

    /// An answered question is still collectable after it stops being
    /// waited on — dropping it in the gap before the next poll is the
    /// failure `collect` exists to prevent, and this must not reintroduce
    /// it by a different route.
    func testAnAnsweredQuestionSurvivesTheWaiterGoingQuiet() {
        let questions = QuestionService()
        let id = questions.ask(agent: "claude-abc1", text: "Which one?", options: ["a", "b"])
        questions.answer(id, with: "a")
        questions.forgetPollsForTesting(olderThan: QuestionService.waiterSilence + 1)
        XCTAssertEqual(questions.answer(to: id), "a",
                       "a decision that was made must reach the agent that asked for it")
    }
}
