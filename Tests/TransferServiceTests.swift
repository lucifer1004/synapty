...
final class TransferServiceTests: XCTestCase {
    ...
    // MARK: - What the approval sheet calls a machine

    /// THE SENTENCE THE HUMAN READS WHEN THEY ARE ASKED TO ALLOW A
    /// TRANSFER, and the one they read when they withdraw it, must name
    /// the machine the same way.
    ///
    /// `ApprovalSheet` renders both in one flow: its title takes
    /// `HostStore.displayName` and its explanation takes this, and this
    /// was a private re-implementation of it — so the sheet said "This
    /// Mac → deskmac" above "this Mac already has a file with this name",
    /// lowercase at the start of its own sentence. `displayName`'s own
    /// comment was written about exactly this pair: "A grant a human
    /// withdraws must read exactly as it did when they made it, so the two
    /// cannot be allowed to drift" ([[WI-2026-09-11-010]]).
    @MainActor
    func testTheLocalMachineIsNamedMidSentenceWhereTheSheetPutsIt() {
        let store = HostStore()
        let service = TransferService()
        service.hostStore = store

        let here = FileEndpoint(hostID: nil, path: "/tmp/x")
        XCTAssertEqual(service.machineName(of: here), "this Mac",
                       "the sheet reads '… already has a file with this name' after it")
        // And the sheet's OTHER rendering of the same machine, which
        // begins a line of its own.
        XCTAssertEqual(store.displayName(of: nil), "This Mac")

        // A named host reads identically in both, which is the point of
        // there being one rule.
        let host = HostEntry(label: "deskmac", address: "10.0.1.5", username: "u")
        store.hosts.append(host)
        XCTAssertEqual(service.machineName(of: FileEndpoint(hostID: host.id, path: "/tmp/y")),
                       store.displayName(of: host.id))

        // AND THIS MAC IS STILL THIS MAC WITH NO STORE TO ASK. `hostStore`
        // is `weak`, and `hostStore?.…  ?? "a host"` short-circuits on a
        // nil STORE — so a record built after it had gone answered "a
        // host" about the machine whose name needs no store to know
        // ([[WI-2026-09-12-002]]).
        service.hostStore = nil
        XCTAssertEqual(service.machineName(of: here), "this Mac",
                       "the local machine was described as 'a host'")
        XCTAssertEqual(service.machineName(of: FileEndpoint(hostID: host.id, path: "/tmp/y")),
                       "a host",
                       "a remote host with no store to name it is the one case that is 'a host'")
    }
