...
final class HubLogLevelTests: XCTestCase {
    ...
    func testOperatedPortsCoverTheLocalHubAndAnsweringPeersOnly() {
        ...
        // A MACHINE WHOSE OWN HUB ADOPTION WAS REFUSED OPERATES NOTHING.
        // `hubPort` used to default to 9000, so this list always led with
        // a hub that might not exist — and the assertion compared the
        // function's output to the same property it reads, which agreed
        // with any answer ([[WI-2026-09-10-008]]).
        XCTAssertEqual(HubLogLevel.operatedPorts(), [],
                       "with no hub of its own and no peer answering, there is nothing to tell")

        tm.hubPort = 9000
        XCTAssertEqual(HubLogLevel.operatedPorts(), [9000],
        ...
        XCTAssertEqual(Set(HubLogLevel.operatedPorts()), Set([9000, port]))
    }
