...
/// WHAT A MACHINE IS CALLED, and where in a sentence it is being called.
final class MachineNamingTests: XCTestCase {

    /// THE DRIFT THIS FUNCTION'S OWN COMMENT NAMES. `displayName` says "A
    /// grant a human withdraws must read exactly as it did when they made
    /// it, so the two cannot be allowed to drift" — and the approval sheet
    /// rendered "This Mac → deskmac" from it while the line beneath said
    /// "this Mac already has a file with this name" from a private
    /// re-implementation, lowercase at the start of its own sentence.
    /// There were four such copies with three spellings of the local
    /// machine ([[WI-2026-09-11-010]]).
    @MainActor
    func testTheLocalMachineHasASentenceInitialFormAndAMidSentenceOne() {
        let store = HostStore()
        XCTAssertEqual(store.displayName(of: nil), "This Mac")
        XCTAssertEqual(store.displayNameInSentence(of: nil), "this Mac")
    }

    /// AND A HOST THE HUMAN NAMED IS WRITTEN THE WAY THEY WROTE IT,
    /// wherever it appears. Lowercasing "GPU" mid-sentence would be a
    /// different machine.
    @MainActor
    func testANamedHostReadsTheSameInEitherPosition() {
        let store = HostStore()
        let host = HostEntry(label: "GPU", address: "10.0.1.5", username: "u")
        store.hosts.append(host)
        XCTAssertEqual(store.displayName(of: host.id), "GPU")
        XCTAssertEqual(store.displayNameInSentence(of: host.id), "GPU")
        XCTAssertEqual(HostStore.inSentence(host), "GPU")
    }

    /// An id that resolves to nothing is not the local machine.
    @MainActor
    func testAnUnresolvableHostIsNeitherThisMacNorNamed() {
        let store = HostStore()
        XCTAssertEqual(store.displayName(of: UUID()), "a host")
        XCTAssertEqual(store.displayNameInSentence(of: UUID()), "a host")
    }

    /// A host with no label falls back to its address, in both forms.
    @MainActor
    func testAnUnlabelledHostIsItsAddress() {
        let store = HostStore()
        let host = HostEntry(label: "", address: "10.0.1.6", username: "u")
        store.hosts.append(host)
        XCTAssertEqual(store.displayName(of: host.id), "10.0.1.6")
        XCTAssertEqual(HostStore.inSentence(host), "10.0.1.6")
    }
}
...
final class PortForwardTests: XCTestCase {
    func testLocalForwardFlag() {
        ...
        XCTAssertEqual(fwd.wireSpec, "local:8080:localhost:80")
    }

    func testRemoteForwardFlag() {
        let fwd = PortForward(kind: .remote, listenPort: 9090, targetHost: "127.0.0.1", targetPort: 9000)
        XCTAssertEqual(fwd.wireSpec, "remote:9090:127.0.0.1:9000")
    }

    /// The empty target the model documents is a shape the editor
    /// produces, and it used to travel as an empty field that `synapty
    /// connect` dropped without a word while dialling the pane anyway.
    func testEmptyTargetIsTheLocalhostTheModelPromises() {
        let fwd = PortForward(kind: .local, listenPort: 8080, targetHost: "", targetPort: 80)
        XCTAssertEqual(fwd.wireTarget, "localhost")
        XCTAssertEqual(fwd.wireSpec, "local:8080:localhost:80")
        XCTAssertEqual(fwd.wireSpec, "local:8080:localhost:80")
    }

    func testWhitespaceOnlyTargetIsAlsoLocalhost() {
        let fwd = PortForward(kind: .local, listenPort: 8080, targetHost: "  ", targetPort: 80)
        XCTAssertEqual(fwd.wireSpec, "local:8080:localhost:80")
    }

    /// A colon separates the fields of the spec and also the groups of the
    /// address, so the address is bracketed out of the ambiguity the same
    /// way ssh itself documents.
    func testIPv6TargetIsBracketed() {
        let fwd = PortForward(kind: .local, listenPort: 8080, targetHost: "::1", targetPort: 80)
        XCTAssertEqual(fwd.wireSpec, "local:8080:[::1]:80")
        let already = PortForward(kind: .local, listenPort: 8080, targetHost: "[fe80::1]", targetPort: 80)
        XCTAssertEqual(already.wireSpec, "local:8080:[fe80::1]:80")
    }

    func testOrdinaryTargetIsUntouched() {
        let fwd = PortForward(kind: .remote, listenPort: 9090, targetHost: "10.0.0.2", targetPort: 9000)
        XCTAssertEqual(fwd.wireSpec, "remote:9090:10.0.0.2:9000")
    }
    ...
    func testHostEntryCodableWithNewFields() throws {
        ...
        XCTAssertEqual(decoded.forwardings[0].wireSpec, "local:8080:localhost:80")
    }
}
