...
final class SyncMonitorTests: XCTestCase {
    ...
    func testAWorkingSyncShowsNothing() async {
        ...
        // THE RENDERING THAT SHIPS ([[SyncPreflight]] `humanDescription`),
        // which is what `HubStatusPopover` and `ContentView` read. Two more
        // renderings of this same status lived on `SyncMonitor` with no
        // production caller between them ([[WI-2026-09-11-011]]).
        XCTAssertEqual(m.status.humanDescription, "Syncing with iCloud")
    }
    ...
    func testAStoppedSyncShowsSomethingSpecific() async {
        ...
        // SOMETHING SPECIFIC, and the same words every surface shows.
        let said = m.status.humanDescription
        XCTAssertNotEqual(said, "Syncing with iCloud")
        XCTAssertFalse(said.isEmpty, "a stopped sync said nothing at all")

        // AND SPECIFIC MEANS DISTINCT, which is what this test is named
        // for and what it stopped asserting. It pinned one literal, the
        // literal moved, and the repair was to require only a non-empty
        // string that is not the working one — which six identical
        // sentences would also satisfy. The remedies differ per state
        // (sign in, turn it on, wait for a network, reinstall), so a
        // sentence shared between two of them sends somebody to the wrong
        // one ([[WI-2026-09-12-002]]).
        let stopped: [SyncPreflight.Status] = [
            .notSignedIn, .restricted, .networkUnavailable, .schemaMissing,
            .notEntitled("no profile"), .failed("CKError 9"),
        ]
        var seen: Set<String> = []
        for state in stopped {
            let sentence = state.humanDescription
            XCTAssertFalse(state.isSyncing, "\(state) rendered as a working sync")
            XCTAssertTrue(seen.insert(sentence).inserted,
                          "two states a human fixes differently say the same thing: \(sentence)")
        }
    }
