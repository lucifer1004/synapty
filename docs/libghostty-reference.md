...
- GhosttyKit xcframework built from Ghostty submodule via `zig build -Demit-xcframework=true -Dxcframework-target=universal -Doptimize=ReleaseFast`
...
- Ghostty submodule + patch adds `src/terminal/c/terminal.zig` (C API wrapper)
- Build: `zig build lib-vt -Dtarget=<native>` for headless use
...
Programs (vim, tmux) send DSR/DA queries. The terminal generates responses that must be written back to the PTY. The daemon must queue and relay these — without this, modal editors and capability-querying programs will not function correctly.
