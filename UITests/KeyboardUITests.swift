...
final class KeyboardUITests: UITestCase {
    ...
    func testClearScreenIsAnActionAndNotTypedIntoTheShell() {
        ...
        app.typeKey("k", modifierFlags: .command)
        XCTAssertTrue(app.textFields["Search hosts, or type user@host:port"].waitForExistence(timeout: 5),
    ...
    func testAWorkbenchChordStillRunsFromInsideTheFindField() {
        ...
        app.typeKey("k", modifierFlags: .command)
        let palette = app.textFields["Search hosts, or type user@host:port"]
