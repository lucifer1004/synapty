...
test: build
    ...
    ./scripts/test-reconnect-e2e.sh
    ./scripts/test-setup-host-master.sh
    ./scripts/test-host-setup.sh
...
# THE VERSION BOTH GATES RUN, named once.
#
# CI pinned a tag and nothing told a developer which version to install,
# so `just verify` and the CI governance job were free to run different
# binaries and neither would report that they had — measured: CI on
# v0.19.1 against a local 0.19.2 ([[WI-2026-09-11-006]]). `.govctl-version`
# is what the workflow installs and what this checks, so a divergence is
# a message rather than a difference in results.
#
# THERE IS NO SECOND PLACE TO EDIT, and the message below used to send
# people looking for one. `ci.yml` derives its release tag from this file
# rather than naming a version of its own — which is the whole point of
# the change above, so the advice outlived the thing it described.
[private]
_govctl:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    WANT=$(tr -d ' \n' < .govctl-version)
    HAVE=$({{ GOVCTL }} --version | awk '{print $2}')
    if [ "$HAVE" != "$WANT" ]; then
        echo "govctl $HAVE is installed; this project and CI run $WANT." >&2
        echo "  cargo install govctl --version $WANT --force" >&2
        echo "  (or change .govctl-version — CI reads its tag from that file)" >&2
        exit 1
    fi
...
[group("build")]
deploy-all:
    # ONE STEP, WHICH CANNOT GO ONE SHORT. This named all five, so adding
    # a sixth target to `build.zig` and forgetting this line produced a
    # deploy-all that quietly built four fifths of the fleet
    # ([[WI-2026-09-11-003]]).
    zig build deploy
...
[group("test")]
verify: _govctl test test-swift
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    #
    # AND THE `|| true` PUT IT BACK. It was there to let the grep below
    # print the errors rather than let `set -e` kill the run first, and it
    # swallowed every other way of not running: a govctl that is not
    # INSTALLED makes the shell say "command not found" on stderr, which
    # does not begin with "error", so this gate printed "govctl: no
    # errors" about 171 clauses it had not read and exited 0. Measured
    # with `GOVCTL=govctl-does-not-exist just verify` ([[WI-2026-09-10-009]]).
    #
    # `_govctl` above catches the absent tool BEFORE the suites run — that
    # dependency is what `gov-check` has had all along and this recipe did
    # not — and the status check below catches a govctl that ran and
    # failed, whether it reported errors or crashed on its way to them.
    STATUS=0
    OUT=$({{ GOVCTL }} check 2>&1) || STATUS=$?
    if [ "$STATUS" -ne 0 ]; then
        printf 'govctl check exited %s:\n' "$STATUS" >&2
        printf '%s\n' "$OUT" >&2
        exit 1
    fi
    # AND A ZERO EXIT IS NOT AGREEMENT ON ITS OWN: warnings exit 0, so the
    # text is still read for anything that calls itself an error.
...
[group("gov")]
gov-lint:
    # THE CHECK'S OWN CASES FIRST. A lint whose rules nobody can change
    # without re-deriving their intent is one that gets loosened by
    # whoever it inconveniences next — and this one had already reported a
    # finding that could never be cleared ([[WI-2026-09-14-001]]).
    python3 scripts/gov-lint.py --selftest
...
package: deploy-all _xcodegen
    ...
    #
    # THE LIST COMES FROM THE BUILD ([[src/deploy_targets.zig]]). It was
    # spelled out here, in `deploy-all`, in `project.yml` and in two
    # tests; a sixth target added to `build.zig` and missed here ships a
    # DMG whose `.deploy` is one architecture short, with nothing saying
    # so ([[WI-2026-09-11-003]]).
    [ -f zig-out/deploy-targets.txt ] \
        || { echo "Error: zig-out/deploy-targets.txt not found. Run deploy-all first." && exit 1; }
    ...
    done < zig-out/deploy-targets.txt
