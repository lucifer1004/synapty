...
def find_review(name, flat, corpus):
    ...
    #
    # "THE LAST ONE" HAS A SECOND SENSE AND IT IS NOT A POINTER. RFC-0014
    # C-SESSION-NAME says "renaming replaces the name; the last one given
    # wins" — an ordering among INPUTS, which nothing inserted into the
    # document can move. This reported it on every run for as long as the
    # clause has existed, and a `review` finding that can never be cleared
    # is the thing the note above warns about, happening to this file:
    # "the ratio at which a check starts being ignored". The tell is the
    # participle — the positional sense names a thing, the temporal one
    # says who did it ([[WI-2026-09-14-001]]).
    temporal = re.compile(
        r"the last one (?:given|sent|said|offered|written|received|made|to arrive)\b")
    for m in re.finditer(
        ...
        span = " ".join(m.group(0).split())
        if temporal.search(span):
            continue
        out.append(("refers by position — an insertion moves it", span[:110]))
    return out
...
def stale_phases():
    """RFCs recorded at `spec` whose live clauses the code already cites.

    `phase` is hand-set and nothing relates it to anything, so `CLAUSE.md`'s
    rule — "a normative clause at `impl` is a commitment this project has
    made and implemented" — was false for three RFCs at once, two of them
    the most heavily cited specs in the tree ([[WI-2026-09-11-009]]).

    ADVICE, NEVER DAMAGE, and the asymmetry is deliberate. A citation in
    a comment is not proof of implementation, so this cannot assert that
    a phase is wrong; what it can say is that an RFC the code talks about
    constantly is still recorded as unbuilt, which is a thing worth a
    human's glance. The reverse — an `impl` RFC nobody cites — is NOT
    reported, because a clause can be honoured by code that never names
    it and this would fire on every one of them.

    A clause that is deprecated or superseded is skipped: those are
    history, and history is not evidence about a phase.
    """
    out = []
    for rfc_path in sorted(glob.glob("gov/rfc/*/rfc.toml")):
        raw = open(rfc_path).read()
        if not re.search(r'^\s*phase\s*=\s*"spec"', raw, re.M):
            continue
        if not re.search(r'^\s*status\s*=\s*"normative"', raw, re.M):
            continue
        rfc_id = rfc_path.split("/")[2]
        cited = []
        for clause_path in sorted(glob.glob(f"gov/rfc/{rfc_id}/clauses/*.toml")):
            body = open(clause_path).read()
            if re.search(r'^\s*status\s*=\s*"(deprecated|superseded)"', body, re.M):
                continue
            clause = clause_path.split("/")[-1][:-5]
            if clause.endswith("SUMMARY") or clause.endswith("NON-GOALS"):
                continue
            hits = subprocess.run(
                ["grep", "-rl", clause, "src", "Sources"],
                capture_output=True, text=True).stdout.split()
            if hits:
                cited.append(clause)
        if cited:
            out.append((rfc_id, cited))
    return out
...
def main():
    ...
    for rfc_id, cited in stale_phases():
        print(f"review  {rfc_id}: recorded at phase `spec`, but the code cites "
              f"{len(cited)} of its live clauses\n          "
              f"{', '.join(cited[:6])}{' …' if len(cited) > 6 else ''}")
...
def selftest():
    """WHAT THIS CHECK MUST AND MUST NOT SAY.

    A lint nobody can change without re-deriving its intent is a lint
    that gets loosened by whoever it inconveniences next. These are the
    two real catches this corpus produced and the false one that made the
    positional rule unclearable ([[WI-2026-09-14-001]]).
    """
    def kinds(text):
        return [k for k, _ in find_review("T", " ".join(text.split()), {})]

    must_flag = [
        "the frames are listed above and the last one is the restoration",
        "as four sentences earlier said, the id is minted once",
        "the bullet above governs this",
        "the first of these is the pane id",
    ]
    for text in must_flag:
        assert "refers by position — an insertion moves it" in kinds(text), text

    must_not_flag = [
        "renaming replaces the name; the last one given wins",
        "two clients may name it and the last one sent is kept",
    ]
    for text in must_not_flag:
        assert "refers by position — an insertion moves it" not in kinds(text), text

    print("gov-lint selftest: ok")
