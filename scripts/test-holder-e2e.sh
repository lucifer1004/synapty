...
# cwd, foreground command, shell cwd, attached-by label, human name.
#
# TEN, NOT EIGHT. Two were appended for [[RFC-0014]] C-CLIENT-LABEL and
# C-SESSION-NAME and this map was not; `commands.zig` records the growth
# where an editor of THIS file will not read it. A map narrower than the
# row is what someone consults before moving a column
# ([[WI-2026-09-11-002]]).
...
case "$SHELL_COL" in
    *shell-home) ;;
    *) fail "column 8 followed the command instead of the shell: $CWD_LINE" ;;
esac
...
# EVERY ROW OF THE SCREEN, ASKED OF THE BUILD RATHER THAN HARD-CODED.
#
# This line was `grep -qF $'\033[24;1H'`, and 24 was an unlinked copy of a
# constant on the other side — so raising the default would leave a full
# repaint still addressing row 24 on its way down and this still matching,
# silently demoted from "the last row was painted" to "row 24 was painted"
# ([[WI-2026-09-11-002]]).
#
# The first repair went the other way and dropped the property: it took
# the highest row addressed and required only that it be past the marker,
# which a repaint that paints rows 1-11 and stops also satisfies — while
# the line below still says every row. Re-derived instead
# ([[WI-2026-09-12-002]]): the build states the geometry a client with no
# terminal announces, and the side that cannot import it reads what was
# built, exactly as `deploy-targets.txt` and `tool-ladder.txt` do.
ROWS="$(sed -n 's/^rows=//p' zig-out/client-geometry.txt)"
[ -n "$ROWS" ] || fail "zig-out/client-geometry.txt has no rows — run \`zig build\`"
ADDRESSED="$(LC_ALL=C grep -oa $'\033\[[0-9]*;1H' "$TMP/repaint.out" \
    | tr -d $'\033[' | cut -d';' -f1 | sort -n | uniq)"
LAST_ROW="$(printf '%s\n' "$ADDRESSED" | tail -1)"
[ -n "$LAST_ROW" ] || fail "the repaint addressed no row at all"
[ "$LAST_ROW" -eq "$ROWS" ] \
    || fail "the repaint stopped at row $LAST_ROW of $ROWS — the rows past it were never painted"
# AND NOTHING IN BETWEEN WAS SKIPPED, which is the other half of the same
# property and the half the original defect actually produced: a
# DESCRIPTION carries only the rows with something on them.
MISSING=""
for r in $(seq 1 "$ROWS"); do
    printf '%s\n' "$ADDRESSED" | grep -qx "$r" || MISSING="$MISSING $r"
done
[ -z "$MISSING" ] \
    || fail "the repaint skipped row(s):$MISSING — that is a description, not a paint"
echo "ok: the restoration addressed all $ROWS rows, including the blank ones"
...
[ "$last_tick" -gt "$first_tick" ] || fail "the session stopped counting ($first_tick -> $last_tick)"
# AND NEVER WENT BACKWARDS, which is the half that names the failure.
# `last > first` cannot see a restart: a replayed stream reads TICK1..9
# then TICK1..20, and its last is still greater than its first. A
# COUNTER THAT DECREASES is the only unambiguous evidence of one
# ([[WI-2026-09-10-004]]).
restart=$(grep -o "TICK[0-9]*" "$TMP/local.out" | tr -d 'TICK' \
    | awk 'NR>1 && $1 < prev { r = prev" -> "$1 } { prev = $1 } END { print r }')
[ -z "$restart" ] || fail "the stream restarted instead of continuing ($restart)"
...
#
# THE RECORD GOES AND THE LOCK FILE STAYS. What a holder leaves is a
# record plus an unclaimed lock — the claim is taken on a file of its own
# and released by the kernel however the holder died ([[WI-2026-09-03-009]]).
# The sweep used to take both, and that put the original damage back: an
# flock lives on an INODE, so a start that has opened the lock file but not
# yet locked it, crossed by this unlink, takes its claim on an orphan — and
# a later start then creates a fresh file, locks THAT, and two processes
# hold one name ([[holder.Record.releaseClaim]], [[WI-2026-09-09-001]]).
#
# Leaving it costs one empty file per name ever used and decides nothing:
# `ownerGone` and `startWouldJoin` each read `.free` and `.absent` as the
# same answer, and `sessions` enumerates `.json`.
...
[ -f "${TOMB%.json}.lock" ] \
    || fail "the sweep took the lock file, which is how one name reaches two holders"
# AND THE NAME IS FREE TO BE TAKEN AGAIN, which is the only thing the
# lock file was ever removed to achieve.
$BIN run --hold --detach --id tombstone-e2e $HUB -- /bin/sh -c 'sleep 5' \
    > "$TMP/tomb-restart.log" 2>&1 \
    || fail "the swept name could not be started again (see $TMP/tomb-restart.log)"
$BIN end --id tombstone-e2e >/dev/null 2>&1 || true
echo "ok: a record whose process is gone is swept rather than listed, and its name is free"
