...
fn root(buf: []u8) ?[]const u8 {
    ...
    const home_env = sys.getenv("HOME");
    if (sys.getenv("SYNAPTY_CONFIG_ROOT")) |raw| {
        if (raw.len > 0 and raw[0] == '/') return std.fmt.bufPrint(buf, "{s}", .{raw}) catch null;
        // AND A LEADING TILDE, WHICH THE WORKBENCH ALSO EXPANDS. This side
        // tested only for `/`, so `SYNAPTY_CONFIG_ROOT=~/scratch` sent the
        // app to the scratch root and every `synapty` process it started
        // to the operator's REAL config — one variable, two destinations,
        // and the dangerous half silent ([[WI-2026-09-11-001]]).
        if (home_env) |h| {
            if (std.mem.eql(u8, raw, "~"))
                return std.fmt.bufPrint(buf, "{s}", .{h}) catch null;
            if (std.mem.startsWith(u8, raw, "~/"))
                return std.fmt.bufPrint(buf, "{s}/{s}", .{ h, raw[2..] }) catch null;
        }
    }
    const home = home_env orelse return null;
    return std.fmt.bufPrint(buf, "{s}/.config/synapty", .{home}) catch null;
}

/// A scratch file under the system temp directory, named by the caller.
///
/// ONE JOIN, BECAUSE SIX OF THEM WERE WRONG. The idiom
/// `getenv("TMPDIR") orelse "/tmp"` followed by `"{s}<name>"` was written
/// out at six call sites, and every copy assumed the value ends in a
/// slash. macOS `TMPDIR` does; the `/tmp` fallback does not, so with the
/// variable unset every one of them addressed `/tmpsynapty-…` at the root
/// of the filesystem. Measured: `env -u TMPDIR zig build test` was
/// 453/461 with six failures and two crashes, none of them about the code
/// under test. CI is macOS-only, where `TMPDIR` is always set, so the
/// suite was green about a platform it does not run on — while
/// `build.zig` ships three linux/musl deploy targets
/// ([[WI-2026-09-10-009]]).
pub fn tempFile(buf: []u8, comptime fmt: []const u8, args: anytype) ?[]const u8 {
    const dir = blk: {
        const t = sys.getenv("TMPDIR") orelse break :blk "/tmp";
        if (t.len == 0) break :blk "/tmp";
        break :blk if (t[t.len - 1] == '/') t[0 .. t.len - 1] else t;
    };
    var name_buf: [256]u8 = undefined;
    const name = std.fmt.bufPrint(&name_buf, fmt, args) catch return null;
...
/// MACHINE, NOT SHARED, and the two sides disagreed about it for months.
///
/// The fragment is DERIVED — from the person's settings and from this
/// machine's `$SHELL` — so it is regenerated wherever it lands and nothing
/// in it is the human's own edit. Carried as a record it became a second
/// copy of the same intent that could disagree with the first, and did: a
/// fresh machine wrote its default fragment before sync fetched, the
/// remote copy lost to "keep ours" for a non-JSON file, and the terminal
/// ignored the synced theme ([[WI-2026-09-02-005]]).
///
/// That work moved the workbench's answer and left this one behind. This
/// entry has no reader here — it exists to be migrated — so what the
/// mismatch did was file a flat-layout fragment into `shared/`, the half
/// that syncs, which is precisely the artefact that work removed
/// ([[WI-2026-09-09-013]]).
pub const ghostty_config: Entry = .{ .kind = .machine, .name = "ghostty.conf" };
...
test "the shared set is what a human would expect to find on a new Mac" {
    ...
    try testing.expectEqual(Kind.machine, ghostty_config.kind);
}
