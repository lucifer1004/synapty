...
pub const MailStore = struct {
    ...
    comptime {
        // THE FILE MUST HOLD WHAT THE MAILBOX WILL. `state_store` bounds
        // the queue it RESTORES at its own ceiling and stops there — which
        // is a partially-decoded mailbox, the one thing that module's
        // header says it never does ("half a queue is worse than none,
        // because it looks complete"). The two numbers happened to be
        // equal, so the promise held by coincidence; raising this one
        // alone would silently discard the difference on every restart,
        // for messages whose senders were told `queued`
        // ([[WI-2026-09-10-009]]).
        if (state_store.max_messages_per_identity < max_per_queue) @compileError(
            "C-MAILBOX: the restore ceiling must be >= the queue bound, or a " ++
                "restart truncates mail a sender was told was queued",
        );
    }
    ...
    /// UNDO THE LAST APPEND, for a message the hub turned out not to be
    /// able to keep. `queued` is a promise of durability
    /// ([[RFC-0008]] C-MAILBOX), so a queue that was written and not
    /// persisted must not be left holding a message nobody was told
    /// about: the sender is answered a failure, and a sender that
    /// retries would otherwise find its message queued twice
    /// ([[WI-2026-09-13-002]]).
    pub fn dropLast(self: *MailStore, id: []const u8) void {
        const list = self.map.getPtr(id) orelse return;
        if (list.items.len == 0) return;
        self.allocator.free(list.pop().?);
    }
...
pub const HubState = struct {
    ...
    /// accepted the bytes. Keys owned; guarded by `forward_mutex`.
    ///
    /// THE ORDER IS `presence -> forward`, AND THIS USED TO SAY THE
    /// OPPOSITE. It read "taken alone — never with presence_mutex held",
    /// which is false as written: `forwardToPeerLocked` runs under
    /// `presence_mutex` and calls both `registerForward` and
    /// `pendingKey`, each of which takes this one. The code has always
    /// followed `presence -> forward` consistently; it was the map that
    /// was wrong, which is the dangerous direction — a reader trusting it
    /// and moving a `forward_mutex` acquisition to satisfy it inverts the
    /// order ([[WI-2026-09-12-001]]).
    ///
    /// The RATIONALE behind the old wording still holds and is the point:
    /// the five-second `awaitForward` runs OUTSIDE `presence_mutex`, so a
    /// peer that never answers cannot freeze the hub.
    ...
    pub fn registerAgentTracked(self: *HubState, agent_id: []const u8, conn: *Connection) !u64 {
        ...
        _ = self.event_log.append(.{ .kind = .agent_registered, .agent = agent_id, .generation = gen }) catch 0;
        // ADVERTISE FIRST, THEN MINT — the order the durable-bind path
        // has always used, and this one had backwards.
        //
        // `wakeCandidateLocked` ends in `relayPresenceLocked`, so both
        // frames go onto the link in the order they are written here. A
        // peer that receives the presence first has no directory entry to
        // apply it to and refuses it `not_in_directory`; the advertise
        // that follows then creates the entry with `pending_wake = null`,
        // which renders as `no_evidence`. Nothing re-sends until the
        // agent's STATUS moves — and the case is an agent registering with
        // mail already queued, which is exactly the agent likely to sit
        // still ([[WI-2026-09-12-002]]).
        //
    ...
    fn wakeCandidateLocked(self: *HubState, agent: []const u8, gen: u64) void {
        ...
        // A PENDING EDGE IS ITS OWN EDGE. Mail arriving does not move the
        // agent's status, so relaying `pending_wake` only when presence
        // changes would leave a peer's row saying "nothing waiting" for as
        // long as the agent's state held still — which for a `waiting`
        // agent may be forever ([[WI-2026-09-10-007]]).
        self.relayPresenceLocked(agent, self.statusOfLocked(agent));
    }

    /// This hub's merged conclusion for a local identity, or `unknown`.
    fn statusOfLocked(self: *HubState, agent: []const u8) protocol.Status {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        const info = self.agent_registry.get(agent, arena.allocator()) orelse
            return .unknown;
        return info.status;
    }
    ...
    pub const BindOutcome = enum {
        ...
        /// (RFC-0008 C-IDENTITY).
        collision,
        /// This hub already holds every durable identity its state file
        /// can carry: binding REFUSED, caller keeps the pane identity.
        at_capacity,
    };

    /// Whether the caller keeps its pane identity.
    ///
    /// ASKED ONCE, HERE. The one production caller tested `!= .collision`
    /// and adopted the durable id for everything else, so a second
    /// refusal would have been read as a success — silently, and with the
    /// agent answering to a name this hub never bound.
    pub fn bindRefused(outcome: BindOutcome) bool {
        return switch (outcome) {
            .collision, .at_capacity => true,
            .bound, .rehomed, .refreshed => false,
        };
    }
    ...
    pub fn bindDurableIdTracked(
        ...
        // THE FILE MUST HOLD WHAT THE BIND WILL, and nothing made that
        // true. `state_store` refuses a store whose identity list exceeds
        // `max_identities` — so a hub past the ceiling wrote a file that
        // its own next restart rejects IN FULL: every durable identity and
        // all queued mail, rather than the overflow. The direction is from
        // partial to nothing, for messages whose senders were told
        // `queued`.
        //
        // THE MAILBOX HAS A PROOF AND THIS HAD ONLY THE SAME SENTENCE.
        // `MailStore.max_per_queue` carries a comptime assertion showing
        // no store this hub writes can exceed the restore bound, which is
        // what lets that loop say "anything reaching here came from
        // another build or a hand". Identities had no equivalent: grep
        // found `max_identities` in `state_store.zig` and nowhere on a
        // write path ([[WI-2026-09-12-002]]).
        //
        // REFUSED HERE, where it is one agent being told to prune, rather
        // than discovered at a restart that has already lost everything.
        // Before any mutation: the steps below rebind the connection, take
        // the routing entry and mint a generation, and a refusal after
        // those is a half-applied bind.
        if (!self.durable_ids.contains(durable_id) and
            self.durable_ids.count() >= state_store.max_identities)
        {
            log.warn(
                "durable identity ceiling reached ({d}) — refusing to bind {s}; " ++
                    "a store past this bound is refused whole on restart",
                .{ state_store.max_identities, durable_id },
            );
            _ = self.event_log.append(.{
                .kind = .identity_rejected,
                .agent = durable_id,
        ...
        // `registerAgentTracked` and `teardownAgent`, and the upgrade
        // path goes through neither: it removes the pane id from the
        // routing table directly rather than through `teardownAgent`.
        //
        ...
        // THE IDENTITY SET AND ANY MERGED MAIL ARE BOTH PROMISES, AND
        // THIS ANSWER IS STILL DISCARDED. Said plainly because the line
        // above reads as a contradiction otherwise.
        //
        // The two places a promise is made about ONE message refuse when
        // they cannot keep it ([[mailboxDeliver]], [[spoolAfterSilence]]).
        // This is a promise about an IDENTITY, and undoing it here would
        // mean unwinding a rebind, a routing entry, a generation, two
        // events and a directory broadcast — while the honest outcome is
        // already visible: `persistLocked` logs that nothing is durable,
        // and on such a hub every send is refused, so an agent cannot act
        // on a name it is about to lose. Recorded rather than changed on
        // a guess ([[WI-2026-09-15-001]]).
        _ = self.persistLocked();
    ...
    fn relayPresenceLocked(self: *HubState, agent_id: []const u8, status: protocol.Status) void {
        ...
        const pending = self.wake_pending.contains(agent_id);
        var with_pending = json.ObjectMap.empty;
        var without_pending = json.ObjectMap.empty;
        with_pending.put(a, "state", .{ .string = status.toString() }) catch return;
        with_pending.put(a, "pending_wake", .{ .bool = pending }) catch return;
        without_pending.put(a, "state", .{ .string = status.toString() }) catch return;
        ...
        while (it.next()) |link| {
            ...
            // AND THE FIELD IS OMITTED RATHER THAN SENT AS `false` to a
            // peer that did not declare `pending-wake`. It would arrive
            // as a fact the peer has no rule for, and the honest shape of
            // "this build does not report that" is an absent field
            // ([[RFC-0011]] C-DEFAULT, [[WI-2026-09-10-007]]).
            const payload = if (link.caps.has(.pending_wake)) with_pending else without_pending;
    ...
    pub fn relayPresenceAllTo(self: *HubState, conn: *Connection, peer_caps: federation.CapabilitySet) void {
        ...
        for (ids) |id| {
            ...
            const pending = self.wake_pending.contains(id);
            // A FRESH LINK STARTS CONSISTENT, and `unknown` with mail
            // waiting is something to say. This skipped every agent that
            // had never signalled, so an identity whose only fact was
            // "it has unread mail" reached the peer as nothing at all.
            if (info.status == .unknown and !pending) continue;
            ...
            if (peer_caps.has(.pending_wake))
                payload.put(a, "pending_wake", .{ .bool = pending }) catch continue;
    ...
    pub fn completeForward(self: *HubState, forward_id: []const u8, ok: bool, reason: []const u8) void {
        ...
        if (ok) {
            ...
            // NO LITERAL IN AN OWNED FIELD. This was `catch "unavailable"`,
            // which puts a static string where the entry's own allocation
            // goes — and both `deinit` and `dropPendingLocked` free what is
            // there. An empty reason is the absence, and the sentence a
            // human reads is chosen at the READ site instead, where it
            // costs nothing and owns nothing ([[WI-2026-09-12-001]]).
            const owned = self.allocator.dupe(u8, reason) catch &.{};
        ...
        while (true) {
            {
                ...
                if (self.pending_forwards.getEntry(forward_id)) |e| {
                    switch (e.value_ptr.*) {
                        ...
                        .nacked => |r| {
                            // Copied out before the entry goes; the drop
                            // frees the entry's own copy, here as on the
                            // deadline path below.
                            const copy = if (r.len == 0)
                                "unavailable"
                            else
                                (arena.dupe(u8, r) catch "unavailable");
                            self.dropPendingLocked(forward_id);
                            return .{ .nacked = copy };
                        },
                    }
    ...
    ///
    /// FREES BOTH HALVES OF WHAT THE ENTRY OWNED. This freed only the key,
    /// and a `.nacked` value owns its reason — so a nack that arrived in
    /// the gap between `awaitForward`'s last look and its final lock
    /// acquisition leaked the string. `deinit` had the rule right; this
    /// did not, and the two callers that see the value first are what hid
    /// it ([[WI-2026-09-12-001]]).
    fn dropPendingLocked(self: *HubState, forward_id: []const u8) void {
        if (self.pending_forwards.fetchRemove(forward_id)) |kv| {
            if (kv.value == .nacked and kv.value.nacked.len > 0) {
                self.allocator.free(kv.value.nacked);
            }
            self.allocator.free(kv.key);
        }
    }
    ...
    pub fn mailboxDeliver(
        ...
        if (!known_locally) {
            ...
            if (self.directory.lookup(target)) |entry| {
                ...
                // THE SAME PROMISE, THE SAME ANSWER. `spooled` tells the
                // sender this hub holds the message and will forward it;
                // a copy that is not on disk does not survive the restart
                // that makes holding it mean anything
                // ([[WI-2026-09-13-002]]).
                if (!self.persistLocked()) {
                    self.spool.dropLast(entry.peer);
                    return error.NotDurable;
                }
        ...
        //
        // AND THE ANSWER IS READ. The order was right and the outcome was
        // thrown away, so a hub that could not write its state file said
        // `delivered` anyway — the word C-MAILBOX defines as "queued in
        // the local mailbox" ([[persistLocked]], [[WI-2026-09-13-002]]).
        //
        // UNDONE, NOT LEFT BEHIND. Failing to hold the copy is this hub
        // failing, which the spool path above already says is an ERROR
        // and not one of C-DELIVERY's six outcomes. A queue left holding
        // a message the sender was told did not arrive is a message
        // delivered twice the moment they retry.
        if (!self.persistLocked()) {
            self.mailbox.dropLast(target);
            if (was_empty) self.wakeCancelLocked(target);
            return error.NotDurable;
        }
        if (hosted_conn) |hc| {
            var nudge_arena = std.heap.ArenaAllocator.init(self.allocator);
    ...
    pub fn relayPresence(
        ...
        entry.status = status;
        // ONLY WHERE THE PEER SPOKE. A frame carrying no `pending_wake`
        // is a peer that does not have the capability, and writing
        // `false` here would turn "cannot say" into "nothing waiting" —
        // which is precisely the decode [[RFC-0011]] C-DEFAULT forbids.
        if (pending) |p| entry.pending_wake = p;
        _ = self.event_log.append(.{
    ...
    fn flushSpoolToLocked(self: *HubState, peer: []const u8) usize {
        ...
        if (sent > 0) _ = self.persistLocked();
        return sent;
    }
    ...
    pub fn federationSweep(self: *HubState, now_ms: i64) void {
        ...
        if (self.spool.expire(a, now_ms)) |dropped| {
            ...
            if (dropped.len > 0) _ = self.persistLocked();
        } else |_| {}
    ...
    pub fn adoptMintedPeerId(self: *HubState, suggested_label: ?[]const u8) void {
        // UNDER THE LOCK, like every other reader of this field. Called
        // once at startup before any thread exists, so no interleaving is
        // reachable today — which is the argument that stops being true
        // the moment anything else calls it, and the guard reads as
        // protection either way.
        self.presence_mutex.lock(io_mod.get()) catch unreachable;
        const taken = self.peer_id != null;
        self.presence_mutex.unlock(io_mod.get());
        if (taken) return;
        var buf: [128]u8 = undefined;
        const id = switch (@import("identity_store.zig").ensure(&buf, suggested_label)) {
            .id => |id| id,
            .unreadable => {
                log.err("identity: the peer id file exists and could not be read — not minting a new one over it; this hub will not federate until it can be read (`synapty hub --remint` replaces it deliberately)", .{});
                return;
            },
        };
    ...
    /// Shut down every relay link; the reader loops then exit and each
    /// tombstones its peer's identities on the way out.
    ///
    /// CALLED ONLY BY `src/e2e_test.zig` TODAY. The comment here used to
    /// say "used on hub teardown and by operators dropping federation",
    /// and neither caller exists — which matters because the bug below
    /// was latent in exactly the gap between what the comment claimed and
    /// what ran ([[WI-2026-09-12-001]]).
    ///
    /// RETAINED WHILE THE POINTER IS HELD. Every other reader of
    /// `peer_links` — `relayPresenceLocked`, `broadcastDirectoryLocked`,
    /// `forwardToPeerLocked`, `advertiseAllTo` — uses `link.conn` UNDER
    /// `presence_mutex`, and that is what makes holding no reference
    /// sound: `peerLinkDown` removes the entry under the same lock and
    /// the connection is released only after. This function has to step
    /// outside the lock (a shutdown wakes the reader, which calls
    /// `peerLinkDown` and would deadlock against a held
    /// `presence_mutex`), so it is the one caller that must hold a
    /// reference of its own. Without it: the reader sees EOF, runs
    /// `peerLinkDown`, and its `defer conn.release()` takes the count to
    /// zero and destroys the connection — while this loop still has the
    /// pointer and shuts down whatever fd number is now in freed memory.
    pub fn closeAllPeerLinks(self: *HubState) void {
        ...
        while (it.next()) |link| {
            link.conn.retain();
            links.append(self.allocator, link.conn) catch link.conn.release();
        }
        self.presence_mutex.unlock(io_mod.get());
        for (links.items) |link| {
            link.interruptStream();
            link.release();
        }
    }
    ...
    /// ([[RFC-0009]] C-DELIVERY).
    /// Hold a forwarded message whose acknowledgement never came, and say
    /// so if it could not be held. Null means held.
    ///
    /// THIS RETURNED NOTHING AND HAD THREE SILENT WAYS OUT, under a caller
    /// that answers `spooled` — a word C-DELIVERY defines as the hub
    /// holding the message and forwarding it later. The test beside this
    /// one states the obligation: "a `spooled` answer has to mean durable,
    /// or it is a promise the hub cannot keep." The directory entry could
    /// be gone, the push could fail, and the persist could fail, and each
    /// left the sender told the message was held while nothing was
    /// ([[WI-2026-09-15-001]]).
    ///
    /// THE SENTENCE IS DIFFERENT FROM THE OTHER SPOOL PATH's, because the
    /// situation is. This message ALREADY WENT OUT once — that is what the
    /// forward id is — so whether the peer received it is genuinely
    /// unknown, and a refusal that said "nothing was queued" would be
    /// claiming more than this hub can see.
    pub fn spoolAfterSilence(
        ...
        self.presence_mutex.lock(io_mod.get()) catch unreachable;
        defer self.presence_mutex.unlock(io_mod.get());
        const entry = self.directory.lookup(target) orelse
            return "the peer hosting this is no longer known here, so nothing is held; " ++
                "whether it received the copy already sent is unknown";
        ...
        const push = self.spool.push(
            ...
            std.Io.Timestamp.now(io_mod.get(), .real).toMilliseconds(),
            forward_id,
        ) catch return "the hub could not hold a copy of this; " ++
            "whether the peer received the copy already sent is unknown";
        if (push == .dropped_oldest) {
            log.err(
                "spool for {s} full — DROPPED the oldest message, which a sender was already told was held",
                .{entry.peer},
        ...
        if (!self.persistLocked()) {
            self.spool.dropLast(entry.peer);
            return "the hub could not make this durable — its state file is not writable; " ++
                "whether the peer received the copy already sent is unknown";
        }
        return null;
    }
    ...
    pub fn mailboxDrainFor(self: *HubState, conn: *Connection, arena: Allocator) ![]const []const u8 {
        ...
        // DELIVERED MAIL MUST NOT COME BACK AFTER A RESTART, and this
        // answer is discarded — which makes the delivery AT-LEAST-ONCE
        // rather than lost: a drain that was not written is mail the
        // agent reads a second time. C-MAILBOX promises durability, not
        // exactly-once, so the degradation is inside the contract; the
        // line above is what made it look like it was not
        // ([[WI-2026-09-15-001]]).
        if (drained.len > 0) _ = self.persistLocked();
        return drained;
    }
    ...
    /// WHETHER THE STATE ON DISK NOW MATCHES THE STATE IN MEMORY.
    ///
    /// This returned nothing, and the words that ride on it are promises:
    /// `delivered` is defined as "hosted locally; queued in the local
    /// mailbox" and the call below it says why the order matters —
    /// "persist BEFORE the caller acks: queued has to mean durable, or
    /// the ack is a promise the hub cannot keep across a restart". The
    /// ORDER was right and the ANSWER was discarded, so a hub whose state
    /// file could not be written went on telling senders `delivered`
    /// ([[WI-2026-09-13-002]]). Measured against a live hub with its
    /// state path under a regular file: every write logged "mail is NOT
    /// durable" and every send was answered `delivered`.
    ///
    /// True when there is nothing to persist. A hub started `--no-state`
    /// was told by its operator not to keep anything, so it promises
    /// nothing across a restart and breaks no promise by not writing.
    fn persistLocked(self: *HubState) bool {
        const path = self.state_path orelse return true;
        ...
        while (it.next()) |e| {
            ...
            boxes.append(a, .{ .id = e.key_ptr.*, .messages = e.value_ptr.items }) catch {
                log.err("could not assemble hub state — mail is NOT durable", .{});
                return false;
            };
        }
        var ids = std.ArrayList([]const u8).empty;
        var id_it = self.durable_ids.keyIterator();
        while (id_it.next()) |k| ids.append(a, k.*) catch {
                log.err("could not assemble hub state — mail is NOT durable", .{});
                return false;
            };
        ...
        while (spool_it.next()) |e| {
            ...
            queues.append(a, .{ .peer = e.key_ptr.*, .messages = msgs.items }) catch {
                log.err("could not assemble hub state — mail is NOT durable", .{});
                return false;
            };
        }
    ...
    pub fn deinit(self: *HubState) void {
        ...
        self.forward_seen.deinit();
        {
            ...
            while (pf.next()) |e| {
                if (e.value_ptr.* == .nacked and e.value_ptr.nacked.len > 0) {
                    self.allocator.free(e.value_ptr.nacked);
                }
                self.allocator.free(e.key_ptr.*);
            }
...
test "RFC-0009 C-PRESENCE: a relayed status is stored, served, and withdrawn when the peer goes dark" {
    ...
    state.relayPresence("remotehost", "claude-remote01", .waiting, null);
...
test "RFC-0009 C-PRESENCE: relayed status bypasses the local merge path" {
    ...
    state.relayPresence("remotehost", "claude-remote01", .working, null);
    var saw = false;
    for (state.event_log.entries.items) |e| {
    ...
    state.relayPresence("build-box", "claude-remote01", .idle, null);
    try std.testing.expectEqual(
...
test "federationSweep releases tool parks the workbench never answered" {
    ...
    // refusal is the `no_workbench` fast path.
...
test "C-DELIVERY: a second silence does not lose the message" {
    ...
    try std.testing.expect(state.spoolAfterSilence(
        "claude-remote01",
        "m-silent",
        "{\"type\":\"dm\",\"id\":\"m-silent\"}",
        "1000-7",
...
test "C-DELIVERY: `forwarded` waits for the peer, and silence becomes `spooled`" {
    ...
    // A NACK THAT ARRIVES WITH NOBODY LEFT TO READ IT. `completeForward`
    // runs on the relay reader thread, so it can land AFTER
    // `awaitForward`'s last look — and the entry is then dropped by the
    // deadline path while it is `.nacked`. `dropPendingLocked` freed only
    // the key, so the peer's own words leaked, once per late answer
    // ([[WI-2026-09-12-001]]).
    //
    // DRIVEN THROUGH THE DROP ITSELF. The window is between two
    // statements on another thread and cannot be opened deterministically
    // from here; what CAN be pinned is the rule the drop owes, which is
    // the rule `deinit` twenty lines away already had right.
    // `std.testing.allocator` is what reports the failure.
    state.registerForward("f-late");
    state.completeForward("f-late", false, "not_hosted");
    {
        state.forward_mutex.lock(io_mod.get()) catch unreachable;
        defer state.forward_mutex.unlock(io_mod.get());
        state.dropPendingLocked("f-late");
    }
...
test "C-WAKE-TRIGGER: a peer is told the identity exists before it is told mail waits for it" {
    // `wakeCandidateLocked` ends in `relayPresenceLocked`, so registering
    // an identity with mail already queued puts TWO frames on the link.
    // They went out presence-first, and a peer applies presence by looking
    // the identity up in its directory: no entry yet, so the frame is
    // refused `not_in_directory` and logged. The advertise that follows
    // then creates the entry with `pending_wake = null` — `no_evidence` —
    // and nothing re-sends until the agent's STATUS moves, which for an
    // agent that just registered and is sitting on its mail may be never.
    //
    // The durable-bind path advertises first and always did, which is what
    // made this look like an oversight rather than a choice
    // ([[WI-2026-09-12-002]]).
    //
    // ASSERTED ON THE WIRE, because the order is the whole defect: both
    // frames are sent either way, and a test that only checked they were
    // both present passed before the fix.
    std.testing.log_level = .err;
    const sys_mod = @import("sys");
    var state = HubState.init(std.testing.allocator);
    defer state.deinit();
    ...
    // MAIL BEFORE THE AGENT, which is the whole case and is reached by a
    // HUB RESTART: the state file restores queued mail for an identity
    // that has no generation yet, and the pane then comes back and
    // registers fresh ([[RFC-0008]] C-MAILBOX). Appended directly here
    // because that is what the restore does — `mailboxDeliver` would
    // refuse an identity it does not host, which is a different case.
    try state.mailbox.append(
        "sleeper-9f21",
        "{\"type\":\"dm\",\"id\":\"m1\",\"source\":\"someone\",\"target\":\"sleeper-9f21\",\"payload\":{}}",
    );
    ...
    var agent_conn = Connection.init(std.testing.allocator, afd, @ptrCast(&dummy), noopRelease);
    defer agent_conn.deinit();
    _ = try state.registerAgentTracked("sleeper-9f21", &agent_conn);

    var advertised_at: ?usize = null;
    var presence_at: ?usize = null;
    for (pconn.outbound.items[before..], 0..) |frame, i| {
        if (mem.indexOf(u8, frame, "sleeper-9f21") == null) continue;
        if (mem.indexOf(u8, frame, "relay_advertise") != null and advertised_at == null) {
            advertised_at = i;
        }
        if (mem.indexOf(u8, frame, "relay_presence") != null and presence_at == null) {
            presence_at = i;
        }
    }
    const adv = advertised_at orelse return error.TestExpectedAdvertise;
    const pres = presence_at orelse return error.TestExpectedPresenceRelay;
    try std.testing.expect(adv < pres);

    // AND THE PRESENCE IS THE ONE THAT MATTERS: it carries the pending
    // edge, which is the fact the refusal was dropping.
    try std.testing.expect(
        mem.indexOf(u8, pconn.outbound.items[before + pres], "\"pending_wake\":true") != null,
    );
}

test "C-MAILBOX: a hub refuses the identity it could not restore, rather than writing a store it will reject whole" {
    // `state_store` refuses a file whose `durable_ids` list exceeds
    // `max_identities`, and nothing bounded the write side — so a hub past
    // the ceiling wrote a state file that its own next restart rejects IN
    // FULL. Not the overflow identities: everything, including every
    // queued message whose sender was told `queued`.
    //
    // The mailbox's equivalent ceiling is safe because a comptime
    // assertion proves no store this hub writes can reach it. Identities
    // had the same sentence and no such proof ([[WI-2026-09-12-002]]).
    std.testing.log_level = .err;
    const sys_mod = @import("sys");
    var state = HubState.init(std.testing.allocator);
    defer state.deinit();

    // Fill to the ceiling the way a long-lived hub does, one identity at a
    // time. Inserted directly: the point is the COUNT the restore will
    // meet, and driving 4096 real registrations tests the loop rather than
    // the bound.
    var i: usize = 0;
    while (i < state_store.max_identities) : (i += 1) {
        var buf: [32]u8 = undefined;
        const id = try std.fmt.bufPrint(&buf, "filler-{d}", .{i});
        const key = try std.testing.allocator.dupe(u8, id);
        state.durable_ids.put(key, {}) catch {
    ...
    var conn = Connection.init(std.testing.allocator, fd, @ptrCast(&dummy), noopRelease);
    defer conn.deinit();
    _ = try state.registerAgentTracked("pane-9f21", &conn);

    const outcome = try state.bindDurableIdTracked(&conn, "claude-abc12345", "abc12345-6789");
    try std.testing.expectEqual(HubState.BindOutcome.at_capacity, outcome);
    // AND THE CALLER KEEPS ITS PANE IDENTITY. The one production caller
    // asked `!= .collision`, so a second refusal would have been adopted
    // as a success and the agent would answer to a name this hub never
    // bound.
    try std.testing.expect(HubState.bindRefused(outcome));
    try std.testing.expect(!state.durable_ids.contains("claude-abc12345"));
    // Nothing half-applied: the routing table still has only the pane id.
    try std.testing.expect(state.routing_table.lookup("claude-abc12345") == null);
    try std.testing.expect(state.routing_table.lookup("pane-9f21") == &conn);

    // AND IT IS THE CEILING, NOT A REFUSAL OF EVERYTHING. One identity
    // below the bound, the same bind succeeds.
    var freed: ?[]const u8 = null;
    {
        var it = state.durable_ids.keyIterator();
        if (it.next()) |k| freed = k.*;
    }
    const gone = freed orelse return error.TestSetupFailed;
    if (state.durable_ids.fetchRemove(gone)) |kv| std.testing.allocator.free(kv.key);
    try std.testing.expectEqual(
        HubState.BindOutcome.bound,
        try state.bindDurableIdTracked(&conn, "claude-abc12345", "abc12345-6789"),
    );
}

test "C-DELIVERY: `spooled` is only said when something is held" {
    // `spoolAfterSilence` returned nothing and had three silent ways out
    // — the directory entry gone, the push failing, the persist failing —
    // under a caller that then answered `spooled`, which C-DELIVERY
    // defines as the hub holding the message and forwarding it later. The
    // test beside this one already stated the obligation in as many
    // words: "a `spooled` answer has to mean durable, or it is a promise
    // the hub cannot keep."
    //
    // FOUND BY RE-READING THE FIX THAT MISSED IT. The spool path inside
    // `mailboxDeliver` was corrected first; this is the OTHER way a
    // message enters the spool, and it kept the defect
    // ([[WI-2026-09-15-001]]).
    //
    // THE ENTRY-GONE CASE IS THE ONE A UNIT TEST CAN DRIVE. The other two
    // are an allocation failure and a write failure, and the write
    // failure logs at `err`, which this runner counts and fails on — it
    // is pinned in `scripts/test-send-answer-e2e.sh` instead.
    std.testing.log_level = .err;
    var state = HubState.init(std.testing.allocator);
    defer state.deinit();

    // No directory entry anywhere: the peer that hosted this is gone
    // between the forward going out and its silence being judged.
    const why = state.spoolAfterSilence(
        "claude-remote01",
        "m-silent",
        "{\"type\":\"dm\",\"id\":\"m-silent\"}",
        "1000-7",
    ...
    // AND IT SAYS WHAT IS UNKNOWN. This message already went out once —
    // that is what the forward id is — so a refusal claiming nothing was
    // sent would be claiming more than this hub can see.
    try std.testing.expect(mem.indexOf(u8, why.?, "unknown") != null);
    {
        state.presence_mutex.lock(io_mod.get()) catch unreachable;
        defer state.presence_mutex.unlock(io_mod.get());
        try std.testing.expectEqual(@as(usize, 0), state.spool.count("remotehost-4e84"));
    }
}
