...
pub const Directory = struct {
    pub const Entry = struct {
        ...
        rival: ?[]const u8 = null,
        /// Whether the hosting peer says this identity has a wake
        /// candidate outstanding ([[RFC-0011]] C-DEFAULT).
        ///
        /// NULL IS NOT FALSE, and the type is what makes that so. A peer
        /// that never declared `pending-wake` cannot report unread mail,
        /// and rendering its silence as "nothing waiting" is the misread
        /// the clause forbids — so the absence is a third value here
        /// rather than a default written over it
        /// ([[WI-2026-09-10-007]]).
        pending_wake: ?bool = null,
...
///
/// AND DERIVED FROM THE SPOOL, for the reason stated above
/// `directory_retention_ms` and for the same class of failure. A spooled
/// message keeps its `forward_id` and is retried under it for as long as
/// the spool holds it; the receiver deduplicates on that id for this
/// window. So this MUST be at least the spool's TTL, and it was the one
/// constant in this file written out independently — raising
/// `spool_ttl_ms` alone (a weekend-long outage is the obvious reason to)
/// would make a retry after 24 hours arrive as `.fresh`, be queued a
/// second time, and hand the agent the same message twice: the failure
/// `forward_id` exists to prevent ([[WI-2026-09-10-009]]).
pub const forward_window_ms: i64 = spool_ttl_ms;

comptime {
    // GUARDS THE DERIVATION, NOT THE VALUES. The line above already makes
    // this true, so this can only fire if somebody replaces it with a
    // literal — which is exactly what the defect was. Said here because
    // read as a check on two independent constants (the shape
    // `MailStore.max_per_queue` really has) it looks like a pin that pins
    // nothing ([[WI-2026-09-12-002]]).
    if (forward_window_ms < spool_ttl_ms) @compileError(
        "C-DELIVERY: the dedup window must be >= the spool TTL, or a retried " ++
            "forward outlives the memory of having accepted it and is delivered twice",
    );
}
...
pub const Spool = struct {
    ...
    /// UNDO THE LAST PUSH, for a copy the hub turned out not to be able
    /// to keep. `spooled` promises the message is held and will be
    /// forwarded ([[RFC-0009]] C-DELIVERY); a sender answered a failure
    /// instead must not leave one behind to be forwarded anyway
    /// ([[WI-2026-09-13-002]]).
    pub fn dropLast(self: *Spool, peer: []const u8) void {
        const q = self.map.getPtr(peer) orelse return;
        if (q.items.len == 0) return;
        self.freeMessage(q.pop().?);
    }
...
pub const Capability = enum {
    ...
    presence_relay,
    /// Relays whether a local identity has a wake candidate outstanding,
    /// and carries the `pending_wake` field on the presence row
    /// ([[RFC-0011]] C-DEFAULT). Absence is NOT `false`: a peer that never
    /// declares this cannot report unread mail, which is a different
    /// sentence from "this agent has nothing waiting", and a consumer
    /// that renders the two the same has misread the capability rather
    /// than declined it ([[WI-2026-09-10-007]]).
    pending_wake,

    /// THE SEPARATOR IS A HYPHEN, AND `presence_relay` IS THE EXCEPTION
    /// ([[RFC-0010]] C-CAPABILITIES names it as one rather than leaving it
    /// as a puzzle: it is the spelling the wire already carries, and
    /// renaming a shipped value to tidy a convention would make that
    /// clause's own warning come true). `@tagName` cannot say that, so
    /// the two directions are written out and tested against each other.
    pub fn toString(self: Capability) []const u8 {
        return switch (self) {
            .presence_relay => "presence_relay",
            .pending_wake => "pending-wake",
        };
    }

    pub fn fromString(s: []const u8) ?Capability {
        inline for (@typeInfo(Capability).@"enum".fields) |f| {
            const c = @field(Capability, f.name);
            if (mem.eql(u8, s, c.toString())) return c;
        }
        return null;
    }
};
...
pub const local_capabilities = [_]Capability{ .presence_relay, .pending_wake };
...
test "this build declares what it actually implements" {
    ...
    // AND THE HYPHEN, which `@tagName` cannot produce. [[RFC-0010]]
    // C-CAPABILITIES fixes the convention and names `presence_relay` as
    // the one exception, so the two spellings are not a slip to be tidied
    // away — they are the rule and its exception, and both are pinned.
    try testing.expect(CapabilitySet.local().has(.pending_wake));
    try testing.expectEqualStrings("pending-wake", Capability.pending_wake.toString());
    try testing.expectEqual(Capability.pending_wake, Capability.fromString("pending-wake").?);
    try testing.expect(Capability.fromString("pending_wake") == null);

    // Every capability round-trips through the spelling it declares.
    inline for (@typeInfo(Capability).@"enum".fields) |f| {
        const c = @field(Capability, f.name);
        try testing.expectEqual(c, Capability.fromString(c.toString()).?);
    }
}
