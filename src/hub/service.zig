...
test "discovery file round-trips and is removed only by its writer" {
    ...
    const scratch = @import("paths").tempFile(&path_buf, "synapty-svc-test-{d}.json", .{std.c.getpid()}).?;
...
pub fn spawnDetached(exe_path: [*:0]const u8, extra: []const ?[*:0]const u8) bool {
    ...
    argv_buf[n] = "hub";
    n += 1;
    // NO `--state-path`, BECAUSE THE DEFAULT IS THE DEFAULT. This passed
    // the literal `default`, which used to be an in-band sentinel meaning
    // "the machine path" — a value the flag now takes as a RELATIVE FILE
    // of that name, which is what the sentinel made impossible to ask for
    // ([[WI-2026-09-11-005]]). Omitting the flag is the same request,
    // said in the shape the parser has.
    for (extra) |a| {
...
test "probeRunning refuses a stale discovery file rather than reporting a hub" {
    const io = io_mod.get();
    var path_buf: [256]u8 = undefined;
    const scratch = @import("paths").tempFile(&path_buf, "synapty-probe-test-{d}.json", .{std.c.getpid()}).?;
    discovery_path_override = scratch;
    defer discovery_path_override = null;
    defer std.Io.Dir.cwd().deleteFile(io, scratch) catch {};
...
test "a minted identity is never overridden by a suggestion" {
    ...
    const scratch_path = @import("paths").tempFile(&pbuf, "synapty-svcid-{d}.json", .{std.c.getpid()}).?;
