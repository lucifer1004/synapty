    ...
    if (root.get("mailboxes")) |mb| {
        ...
        while (it.next()) |entry| {
            ...
            for (v.array.items) |m| {
                ...
                // SAID, THOUGH NOT AS A BROKEN PROMISE. Truncating the
                // FILE is this ceiling's job — it is a guard against a
                // hostile or runaway store, and the test below pins that
                // the store still loads. What was missing is that it
                // happened silently: with the comptime assertion beside
                // `MailStore.max_per_queue` in place, no store this hub
                // WROTE can reach this branch, so anything that does came
                // from another build or a hand, and an operator wants to
                // know that ([[WI-2026-09-10-009]]).
                if (msgs.items.len >= max_messages_per_identity) {
                    log.warn(
                        "state file holds more than {d} messages for {s} — restored the first {d} and dropped the rest",
                        .{ max_messages_per_identity, entry.key_ptr.*, max_messages_per_identity },
                    );
                    break;
                }
    ...
    if (root.get("durable_ids")) |dv| {
        if (dv == .array) {
            for (dv.array.items) |v| {
                ...
                // THE SAME CEILING, THE SAME ANSWER. The two loops on
                // either side of this one reject the store when they meet
                // `max_identities`; this one broke out silently, so a
                // hub past the bound forgot some of its durable
                // identities on restart and their queued mail became
                // addressed to nothing — while the identical overflow in
                // a mailbox refused the whole file and said so. One
                // ceiling, one answer ([[WI-2026-09-10-009]]).
                if (ids.items.len >= max_identities) {
                    return .{ .outcome = .rejected, .reason = "state file exceeds the identity ceiling" };
                }
    ...
    if (root.get("spool")) |sv| {
        ...
        while (it.next()) |entry| {
            ...
            for (v.array.items) |m| {
                ...
                // Said out loud for the same reason as the mailbox above:
                // a spooled message is a promise to a sender too.
                if (msgs.items.len >= max_messages_per_identity) {
                    log.err(
                        "state file holds more than {d} spooled messages for {s} — RESTORED THE FIRST {d} and dropped the rest",
                        .{ max_messages_per_identity, entry.key_ptr.*, max_messages_per_identity },
                    );
                    break;
                }
...
test "per-identity message ceiling truncates the FILE, not the promise" {
    // AND THE PROMISE IS WHAT THE COMPTIME ASSERTION KEEPS. This ceiling
    // may be larger than `MailStore.max_per_queue` and never smaller; the
    // assertion beside that constant is what makes "not the promise" true
    // rather than a coincidence of two 256s ([[WI-2026-09-10-009]]).
...
test "file round-trip through an atomic write" {
    ...
    const scratch = @import("paths").tempFile(&pbuf, "synapty-state-test-{d}.json", .{std.c.getpid()}).?;
