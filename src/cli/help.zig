...
pub fn printSubcommand(name: []const u8) !void {
    inline for (types.subcommands) |sc| {
        if (mem.eql(u8, sc.name, name)) {
            ...
            // AND EACH VERB'S OWN SHAPE, for a subcommand that dispatches
            // on one. These flags lived inside `parseArgs` where nothing
            // could print them, so `synapty task list --help` answered
            // with this banner and never named `--project`
            // ([[WI-2026-09-15-002]]).
            inline for (sc.verbs) |v| {
                try io_mod.stdoutWriteAll(
                    "\n  synapty " ++ sc.name ++ " " ++ v.name
                        ++ (if (v.usage.len > 0) " " ++ v.usage else "")
                        ++ (if (comptime hasFlags(v.params)) " [options]" else "") ++ "\n");
                if (comptime hasFlags(v.params)) {
                    var vbuf: [4096]u8 = undefined;
                    var vwriter = std.Io.Writer.fixed(&vbuf);
                    try clap.help(&vwriter, clap.Help, comptime flagsOf(v.params), .{});
                    try io_mod.stdoutWriteAll(vwriter.buffered());
                }
            }
