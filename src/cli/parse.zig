...
pub fn parseArgs(allocator: Allocator, args: []const []const u8) !Subcommand {
    ...
    if (mem.eql(u8, sub, "account")) return parseAccount(rest);
    if (mem.eql(u8, sub, "connect")) return parseConnect(allocator, rest);
    if (mem.eql(u8, sub, "host-master")) return parseHost(allocator, rest, .master);
    if (mem.eql(u8, sub, "host-setup")) return parseHost(allocator, rest, .setup);
...
fn parseTask(allocator: Allocator, args: []const []const u8) !Subcommand {
    ...
    if (mem.eql(u8, action, "list")) {
        const params = comptime clap.parseParamsComptime(types.verbParamsFor("task", "list"));
        var iter = clap.args.SliceIterator{ .args = rest };
        var res = clap.parseEx(clap.Help, &params, clap.parsers.default, &iter, .{
            .allocator = allocator,
        }) catch |err| return usageError(err);
    ...
    if (mem.eql(u8, action, "create")) {
        ...
        const title = rest[0];
        const params = comptime clap.parseParamsComptime(types.verbParamsFor("task", "create"));
...
/// The `host-*` verbs, which share a shape: which host, and how to reach
/// it. Hand-rolled for the same reason `connect` is — `--forward` repeats.
fn parseHost(allocator: Allocator, args: []const []const u8, which: enum { master, setup }) !Subcommand {
    if (wantsHelp(args)) return ParseError.HelpRequested;
    var out: types.HostArgs = .{};
    var forwards: std.ArrayList([]const u8) = .empty;
    errdefer forwards.deinit(allocator);
    var i: usize = 0;
    while (i < args.len) : (i += 1) {
        const a = args[i];
        if (mem.eql(u8, a, "--print")) {
            out.print = true;
            continue;
        }
        ...
        i += 1;
        const v = args[i];
        if (mem.eql(u8, a, "--host")) {
            out.host = v;
        } else if (mem.eql(u8, a, "--user")) {
            out.user = v;
        } else if (mem.eql(u8, a, "--port")) {
            out.port = std.fmt.parseInt(u16, v, 10) catch return ParseError.MissingArgument;
        } else if (mem.eql(u8, a, "--key")) {
            out.key = v;
        } else if (mem.eql(u8, a, "--jump")) {
            out.jump = v;
        } else if (mem.eql(u8, a, "--socket")) {
            out.socket = v;
        } else if (mem.eql(u8, a, "--tunnel-port")) {
            out.tunnel_port = std.fmt.parseInt(u16, v, 10) catch return ParseError.MissingArgument;
        } else if (mem.eql(u8, a, "--peer-id")) {
            out.peer_id = v;
        } else if (mem.eql(u8, a, "--forward")) {
            try forwards.append(allocator, v);
        } else {
            return ParseError.UnknownSubcommand;
        }
    }
    if (out.host.len == 0 or out.user.len == 0) return ParseError.MissingArgument;
    out.forwards = try forwards.toOwnedSlice(allocator);
    return switch (which) {
        .master => .{ .host_master = out },
        .setup => .{ .host_setup = out },
    };
}

/// `synapty connect`.
///
/// HAND-ROLLED RATHER THAN THROUGH clap, for one reason: `--forward` may
/// be given several times, and what the workbench passes is a list. The
/// rest is a long flat run of `--flag value` pairs, which is the shape
/// clap would have produced anyway.
///
/// THE SLICES POINT INTO argv, which outlives every use of them; the
/// forwarding LIST is the one allocation, and it lives as long as the
/// process that is about to become ssh.
fn parseConnect(allocator: Allocator, args: []const []const u8) !Subcommand {
    if (wantsHelp(args)) return ParseError.HelpRequested;
    var out: types.ConnectArgs = .{};
    var forwards: std.ArrayList([]const u8) = .empty;
    errdefer forwards.deinit(allocator);
    var i: usize = 0;
    while (i < args.len) : (i += 1) {
        const a = args[i];
        if (mem.eql(u8, a, "--transport")) {
            out.transport = true;
            continue;
        }
        if (mem.eql(u8, a, "--print")) {
        ...
        if (mem.eql(u8, a, "--print-remote")) {
            out.print_remote = true;
            continue;
        }
        // EVERY FLAG BELOW TAKES A VALUE, so a missing one is a mistake
        // rather than an empty string quietly standing in for it.
        if (i + 1 >= args.len) return ParseError.MissingArgument;
        i += 1;
        const v = args[i];
        if (mem.eql(u8, a, "--id")) {
            out.agent_id = v;
        } else if (mem.eql(u8, a, "--host")) {
            out.host = v;
        } else if (mem.eql(u8, a, "--user")) {
            out.user = v;
        } else if (mem.eql(u8, a, "--dest")) {
            out.dest = v;
        } else if (mem.eql(u8, a, "--port")) {
            out.port = std.fmt.parseInt(u16, v, 10) catch return ParseError.MissingArgument;
        } else if (mem.eql(u8, a, "--key")) {
            ...
            out.jump = v;
        } else if (mem.eql(u8, a, "--remote")) {
            out.remote = v;
        } else if (mem.eql(u8, a, "--fresh-id")) {
            out.fresh_id = v;
        } else if (mem.eql(u8, a, "--cwd")) {
            out.cwd = v;
        } else if (mem.eql(u8, a, "--socket")) {
            out.socket = v;
        } else if (mem.eql(u8, a, "--socket-file")) {
            out.socket_file = v;
        } else if (mem.eql(u8, a, "--pid-file")) {
            out.pid_file = v;
        } else if (mem.eql(u8, a, "--forward")) {
            try forwards.append(allocator, v);
        } else {
    ...
    // A DIAL NEEDS A HOST AND A USER; a transport has already been given
    // the answer they make.
    if (out.transport) {
        if (out.dest.len == 0) return ParseError.MissingArgument;
    } else if (out.host.len == 0 or out.user.len == 0) {
...
/// `synapty account <kind> [text]`.
///
/// POSITIONAL AND NOT FLAGGED, because the two callers are shell functions
/// that already have the pieces as words, and a flag they had to quote is
/// one more place for a line to be assembled wrongly ([[ADR-0020]]).
///
/// THE TEXT IS ONE ARGUMENT. Both callers already quote it (`"$1"`), and
/// accepting a spread of words would mean joining them with a separator
/// this side chose — which is how a line comes to differ from the one the
/// caller meant. Trailing words are a mistake rather than noise, on the
/// same reasoning as `identify` ([[WI-2026-09-02-021]]).
fn parseAccount(args: []const []const u8) !Subcommand {
    if (wantsHelp(args)) return ParseError.HelpRequested;
    if (args.len == 0) return ParseError.MissingArgument;
    // A TRANSPORT'S STATUS INSTEAD OF A KIND. The two forms are exclusive
    // because they answer the same question by different means: one names
    // the line, the other derives it ([[commands.transportEnd]]).
    if (mem.eql(u8, args[0], "--transport-status")) {
        if (args.len != 2) return ParseError.MissingArgument;
        const status = std.fmt.parseInt(u8, args[1], 10) catch
            return ParseError.MissingArgument;
        return .{ .account = .{ .kind = "end", .text = "", .transport_status = status } };
    }
    if (args.len > 2) return ParseError.TooManyArguments;
    return .{ .account = .{
        .kind = args[0],
        .text = if (args.len > 1) args[1] else "",
    } };
}
...
fn parseRun(allocator: Allocator, args: []const []const u8) !Subcommand {
    ...
    // launched wrapper), resolve like a bare CLI: SYNAPTY_HUB_PORT, then
    // the discovery file, then 9000 (WI-2026-08-11-017).
    //
    // AND `--hub none` FOR A MACHINE THAT HAS NONE. Omitting the flag
    // cannot say that: it resolves, and what it resolves to on a workbench
    // whose adoption was REFUSED is the very hub the refusal was about
    // ([[run.HubTarget]], [[WI-2026-09-12-002]]). `none` rather than a
    // port of 0 or an empty host, because it is a word a human reading
    // `ps` can act on.
    var hub: ?types.HubTarget = .{
        .addr = "127.0.0.1",
        .port = @import("transport.zig").resolveHubPort(),
    };
    if (res.args.hub) |hub_str| {
        if (mem.eql(u8, hub_str, "none")) {
            hub = null;
        } else {
            ...
            hub = .{
                .addr = host,
                .port = std.fmt.parseInt(u16, port_str, 10) catch return ParseError.MissingArgument,
            };
        }
    }
...
fn parseHub(allocator: Allocator, args: []const []const u8) !Subcommand {
    ...
    if (res.args.help != 0) return ParseError.HelpRequested;
    // ONE JOB PER INVOCATION, AND TWO NAMED IS A USAGE ERROR. These were
    // three independent bools read in a fixed order, so `hub --ensure
    // --log` printed the log and dropped `--ensure` in silence — in
    // either order ([[WI-2026-09-11-005]]).
    var action: types.HubArgs.Action = .serve;
    var named: usize = 0;
    if (res.args.log != 0) {
        action = .log;
        named += 1;
    }
    if (res.args.remint != 0) {
        action = .remint;
        named += 1;
    }
    if (res.args.ensure != 0) {
        action = .ensure;
        named += 1;
    }
    if (named > 1) return ParseError.ConflictingActions;
    ...
    return .{ .hub = .{
        ...
        .state_path = if (res.args.@"no-state" != 0)
            .none
        else if (res.args.@"state-path") |sp|
            .{ .at = sp }
        else
            .machine,
    } };
...
fn parseChannelCreate(allocator: Allocator, args: []const []const u8) !Subcommand {
    // THE POSITIONAL IS APPENDED, because the table's `params` is the
    // FLAGS and the positional shape lives in `usage` — which is what
    // `--help` prints for it ([[types.VerbInfo]]).
    const params = comptime clap.parseParamsComptime(
        types.verbParamsFor("channel", "create") ++
            \\<str>
            \\
    );
    var iter = clap.args.SliceIterator{ .args = args };
...
test "the three answers to where durable state lives are three, not a string among two" {
    // `state_path` was `?[]const u8` with the literal `"default"`
    // standing in for the third answer, so a file actually NAMED
    // `default` could not be asked for — and `cli.zig` fell back to that
    // sentinel AS A PATH when the machine root would not resolve, writing
    // durable mailboxes to `./default` ([[WI-2026-09-11-005]]).
    const a = testing.allocator;

    const bare = try parseArgs(a, &.{"hub"});
    try testing.expectEqual(types.HubArgs.StatePath.machine, std.meta.activeTag(bare.hub.state_path));

    const off = try parseArgs(a, &.{ "hub", "--no-state" });
    try testing.expectEqual(types.HubArgs.StatePath.none, std.meta.activeTag(off.hub.state_path));

    const named = try parseArgs(a, &.{ "hub", "--state-path", "/srv/state.json" });
    try testing.expectEqualStrings("/srv/state.json", named.hub.state_path.at);

    // THE WORD THAT USED TO BE A SENTINEL IS A FILENAME. Nothing about it
    // is special any more, which is the point.
    const literal = try parseArgs(a, &.{ "hub", "--state-path", "default" });
    try testing.expectEqualStrings("default", literal.hub.state_path.at);
}
