...
///
/// WHY A SEND FAILED, IN THE HUB'S OWN WORDS, OR NULL IF IT DID NOT.
///
/// BROADER THAN THE NAME, since the hub refuses on two shapes: a delivery
/// VERDICT carries its words at `data.reason`, and everything else — a
/// missing target, a recipient that went away, a state file that cannot
/// be written — at `error`. Both are failures of the same send and both
/// exit non-zero here ([[WI-2026-09-13-002]]).
///
...
pub fn deliveryFailureOfEnvelope(arena: Allocator, hub_envelope: []const u8) ?[]const u8 {
    ...
    // A REFUSAL HAS TWO SHAPES AND THIS READ ONE.
    //
    // A DELIVERY verdict carries its words at `data.reason` — the peer's
    // own code, carried through. Everything else the hub refuses carries
    // them at `error`: a missing target, a recipient that went away
    // mid-send, and a hub that could not make the message durable. All of
    // those arrived at a human as "the hub did not accept it", which is
    // [[RFC-0015]] C-FAILURE on a reason the hub had already written down
    // ([[WI-2026-09-13-002]]).
    if (payload.object.get("data")) |data| {
        if (data == .object) {
            if (data.object.get("reason")) |reason| {
                if (reason == .string) return reason.string;
            }
        }
    }
    if (payload.object.get("error")) |e| {
        if (e == .string and e.string.len > 0) return e.string;
    }
    // A refusal with no words is still a refusal; saying nothing would
    // put the caller back where it started.
    return "the hub did not accept it";
}
...
///
/// BOUNDED WELL ABOVE THE HUB'S OWN BOUND, and DERIVED from it
/// ([[protocol.send_answer_wait_ms]]). A cross-machine send waits for a
/// peer's acknowledgement before the hub can answer at all, so a bound
/// under that would turn every forwarded message into "the hub did not
/// answer" — a relation that lived in this paragraph while the two
/// numbers sat in two modules ([[WI-2026-09-11-015]]).
fn readSendAnswer(arena: Allocator, fd: sys.fd_t) !?[]const u8 {
    sys.setRecvTimeout(fd, protocol.send_answer_wait_ms) catch {};
...
/// THE ONE SENTENCE A HUMAN CAN ACT ON WHEN THERE IS NO HUB.
///
/// Three verbs opened this connection and each carried its own copy of
/// the message. [[RFC-0003]] C-CLI-TOOLS asks for a readable failure, and
/// what it replaced was `error: ConnectionRefused` plus eight frames of
/// this repository's own source paths — so a reworded copy here is a
/// caller reading a different sentence about the same condition, and the
/// shell suite loops over exactly these three verbs expecting one
/// ([[WI-2026-09-11-018]]).
///
/// THE EXIT CODE STAYS THE CALLER'S. `wait` answers 1 for a transport
/// failure and a tool verb answers `refused`, which are different
/// contracts with different readers; what is shared is the sentence.
fn hubOrSayWhy(exit_code: u8) sys.fd_t {
    return transport.connectToHub(transport.hub_addr, transport.resolveHubPort()) catch {
        io_mod.stderrWriteAll("error: cannot reach the hub (is Synapty running?)\n") catch {};
...
/// 1 transport, 2 usage, 9 an outcome this build does not know
/// ([[WI-2026-09-11-014]]).
...
/// AN OUTCOME THIS BUILD DOES NOT KNOW IS NOT SUCCESS.
///
/// This fell through to `return 0` for every unrecognised word — and the
/// producing side, Swift's `ExecOutcome`, is an OPEN enum: adding a case
/// there costs nothing and takes no test red. So an agent running
///
///     synapty exec run --pane p --cmd "make" && deploy
///
/// proceeded on a refusal it never saw. The exit code is the only
/// machine-readable answer this verb gives — `{"outcome":"refused"}` on
/// stdout is prose to a shell — and it was the half that answered
/// "success" for everything it had not been told about
/// ([[WI-2026-09-11-014]]).
///
/// THE DIRECTION OF THE CHANGE IS DELIBERATE. A workbench newer than the
/// CLI now fails LOUDLY at the agent rather than appearing to work. An
/// agent chaining on `&&` is the caller this verb exists for, and a
/// mixed-build pane surfacing an error is the lesser harm.
pub const exec_unknown_outcome: u8 = 9;

fn execExitCode(resp: json.Value) u8 {
    const pl = resp.object.get("payload") orelse return exec_unknown_outcome;
    if (pl != .object) return exec_unknown_outcome;
    const data = pl.object.get("data") orelse return exec_unknown_outcome;
    if (data != .object) return exec_unknown_outcome;
    const outcome = data.object.get("outcome") orelse return exec_unknown_outcome;
    if (outcome != .string) return exec_unknown_outcome;
    ...
    return exec_unknown_outcome;
}

test "every outcome the workbench can send has an exit code, and an unknown one is not success" {
    // THE ONLY MACHINE-READABLE ANSWER THIS VERB GIVES, and nothing
    // measured it: `grep -rn execExitCode src/` returned the definition
    // and its one call site, and the verb has no e2e coverage at all. The
    // producing side is Swift's `ExecOutcome`, whose nine strings are
    // pinned against themselves and against nothing here
    // ([[WI-2026-09-11-014]]).
    const cases = [_]struct { outcome: []const u8, code: u8 }{
        .{ .outcome = "opened", .code = 0 },
        .{ .outcome = "ran", .code = 0 },
        .{ .outcome = "closed", .code = 0 },
        .{ .outcome = "matched", .code = 0 },
        .{ .outcome = "timed-out", .code = 3 },
        .{ .outcome = "target-gone", .code = 5 },
        .{ .outcome = "ownership-lost", .code = 6 },
        .{ .outcome = "disarmed", .code = 7 },
        .{ .outcome = "refused", .code = 8 },
    };
    ...
    const build = struct {
        fn resp(al: Allocator, word: []const u8) !json.Value {
            var data = json.ObjectMap.empty;
            try data.put(al, "outcome", .{ .string = word });
            var pl = json.ObjectMap.empty;
            try pl.put(al, "data", .{ .object = data });
            var top = json.ObjectMap.empty;
            try top.put(al, "payload", .{ .object = pl });
            return .{ .object = top };
        }
    };

    for (cases) |c| {
        const got = execExitCode(try build.resp(a, c.outcome));
        if (got != c.code) {
            std.debug.print("outcome {s} exited {d}, not {d}\n", .{ c.outcome, got, c.code });
            return error.WrongExitCode;
        }
    }

    // AND THE DEFECT WAS THE DEFAULT. A word this build has never heard
    // of must not read as success to a shell chaining on `&&`.
    try std.testing.expectEqual(
        exec_unknown_outcome,
        execExitCode(try build.resp(a, "a-word-a-newer-workbench-sends")),
    );
    // Nor may a malformed answer.
    var bare = json.ObjectMap.empty;
    try bare.put(a, "payload", .{ .string = "not an object" });
    try std.testing.expectEqual(exec_unknown_outcome, execExitCode(.{ .object = bare }));
}
...
fn toolExchange(
    ...
    // reader can act on.
    const fd = hubOrSayWhy(@intFromEnum(ToolOutcome.refused));
    defer sys.close(fd);
...
const AttachState = struct {
    fd: sys.fd_t,
    /// WHOLE FRAMES ONTO THE HOLDER'S SOCKET ([[holder.FrameWriter]]).
    /// `attachInputLoop` writes `.input` and `attachSizeLoop` writes
    /// `.resize` to this same fd, and a frame is a header write and then
    /// a payload write — so resizing the window during a paste could put
    /// three bytes of a resize header into the child as keystrokes, and
    /// leave the holder reading the rest as a frame with a length out of
    /// somebody else's payload.
    writer: holder.FrameWriter = .{},
    running: holder.Flag = .{ .value = true },
...
fn attachHere(allocator: Allocator, args: types.AttachArgs) !AttachOutcome {
    ...
    var ws = sys.winsize{
        .ws_row = protocol.assumed_rows,
        .ws_col = protocol.assumed_cols,
    };
    if (is_tty) {
        var waited: usize = 0;
    ...
    // THE FIRST THREAD IS JOINED IF THE SECOND CANNOT START
    // ([[WI-2026-09-23-004]]). It holds `&state`, which lives in this
    // frame, and a bare `try` returned with it still running — the shape
    // `hubServe` and `FakeDaemon` were both fixed for. The through path
    // below has always done this; the local one did not. The input loop
    // polls its flag, so the join is bounded by one poll interval.
    const size_thread = std.Thread.spawn(.{}, attachSizeLoop, .{ &state, stdin_fd, ws }) catch |err| {
    ...
    while (state.running.get()) {
        ...
        switch (frame.kind) {
            ...
            .gap => {
                ...
                //
                // INTO THE SCREEN, WHICH IS ONLY RIGHT HERE. This is the
                // local attach: there is no workbench, so the terminal
                // the human is watching is the only surface there is. In
                // a pane the same fact goes on the channel as a `hole`
                // and the workbench draws it above the grid
                // ([[WI-2026-09-16-002]]). The SENTENCE is one either
                // way — it has an owner ([[progress.Hole]]).
                screenNotice(progress_mod.Hole.catch_up_outrun.text()) catch {};
            },
            .exit => {
...
fn runRelay(agent_id: []const u8) !void {
    ...
    sys.connect(fd, &addr, addr.len()) catch {
        ...
        try io_mod.stderrWriteAll("\n");
        // THE STATUS IS THE MESSAGE. The line above is for a human reading
        // a terminal; this is what crosses ssh to the client and is the
        // only thing that tells "no session there" from "no host there"
        // ([[transportVerdict]]).
        std.process.exit(relay_no_session_status);
    };
...
fn attachInputLoop(state: *AttachState, stdin_fd: sys.fd_t) void {
    ...
    while (state.running.get()) {
        ...
        if (step.forward.len > 0) {
            state.writer.write(state.fd, .input, step.forward) catch break;
        }
        if (step.detach) {
...
fn attachSizeLoop(state: *AttachState, stdin_fd: sys.fd_t, initial: sys.winsize) void {
    ...
    while (state.running.get()) {
        ...
        state.writer.write(state.fd, .resize, &p) catch return;
    }
}
...
pub fn holdRefused(agent_id: []const u8) bool {
    ...
    // any client.
    //
    // FOUR ANSWERS, AND THE REFUSAL TAKES THREE OF THEM. This comment used
    // to enumerate three states of what is now a four-state enum —
    // "`absent` and `free` both mean nothing is holding the name, and only
    // `held` is a session a start would be joining" — and `unknown` fell
    // silently into "proceed". Which of the two mistakes to make is
    // [[holder.startWouldJoin]]'s to argue; the short of it is that
    // refusing a start costs one command and admitting one strands a live
    // session for good ([[WI-2026-09-08-005]]).
    return holder.startWouldJoin(agent_id);
}

/// `synapty account <kind> [text]` — one line of the account this
/// connection is writing.
///
/// WHY A VERB AND NOT A FUNCTION EACH CALLER COPIES. Four processes write
/// this format and none can import another, so the shape has been a
/// cross-process convention rather than a function ([[progress]]'s header
/// says so). Three of the four are this binary. The fourth was two shell
/// scripts, which each carried their own `printf` of the format and their
/// own `perl` call for a millisecond stamp — because macOS ships bash 3.2
/// with no `EPOCHREALTIME` and BSD `date` has no `%N`. [[ADR-0020]] draws
/// the line: shell only where the far side's shell is the only thing
/// running, and this runs on the machine the binary is already on.
///
/// REFUSED RATHER THAN WRITTEN WHEN THE KIND IS NOT ONE. A reader shows an
/// unknown kind as free text ([[progress.Kind]]), so `pain` instead of
/// `paint` would arrive as narration and the pane would wait out its
/// silence deadline instead of being given back. The caller is a script,
/// and a script's typo has no other reader.
pub fn runAccount(args: types.AccountArgs) !void {
    // A TRANSPORT'S STATUS, READ HERE RATHER THAN CLASSIFIED IN SHELL.
    // Which number means what is a fact about ssh ([[transportEnd]]), and
    // it was an `if [ "$rc" -eq 255 ]` in a script that also has to get
    // `set -e` right to reach it at all ([[ADR-0020]]).
    if (args.transport_status) |status| {
        const word = transportEnd(@as(c_int, status) << 8) orelse return;
        var p = progress_mod.Progress.fromEnv();
        defer p.close();
        p.say(.end, word);
        return;
    }
    const kind = std.meta.stringToEnum(progress_mod.Kind, args.kind) orelse {
        try io_mod.stderrWriteAll("synapty account: not a kind: ");
        try io_mod.stderrWriteAll(args.kind);
        try io_mod.stderrWriteAll("\n");
        std.process.exit(2);
    };
    // NOTHING WRITTEN AND NO COMPLAINT WHEN NOBODY NAMED A CHANNEL. That
    // is a connection a human is running in their own terminal, and
    // [[progress]]'s standing rule is that an account which cannot be
    // written is an account that is off — a connection is not worth
    // failing because the record of it could not be kept.
    var p = progress_mod.Progress.fromEnv();
    defer p.close();
    p.say(kind, args.text);
}

/// SSH'S OWN FAILURES WEAR ONE NUMBER. ssh(1): "ssh exits with the exit
/// status of the remote command or with 255 if an error occurred."
pub const ssh_own_failure: u8 = 255;

/// WHAT BECAME OF A TRANSPORT NOBODY WAS RELAYING FOR, from the status it
/// exited with; null where the status says nothing.
///
/// THIS IS THE WHOLE OF THE [[RFC-0014]] C-OPT-OUT ANSWER. On a host that
/// keeps no session the transport IS the pane's child, so a dropped link
/// and a human typing `exit` arrive as the same event — and libghostty's
/// exit code cannot separate them, because every pane is spawned through
/// `/usr/bin/login`, which reports 0 for a child that exits 42, is killed,
/// or does not exist. ssh's own status can, and this is the mapping
/// ([[WI-2026-09-09-004]]).
///
/// A SIGNAL SAYS NOTHING AND IS REPORTED AS NOTHING. A killed transport is
/// almost always the pane going away, and neither answer would be true of
/// it: the link was not severed, and no child of the human's finished.
/// Silence leaves the pane behaving as a terminal does, which is what
/// closing one should do.
pub fn transportEnd(status: c_int) ?[]const u8 {
    if ((status & 0x7f) != 0) return null;
    const code: u8 = @intCast((status >> 8) & 0xff);
    return if (code == ssh_own_failure) "link_severed" else "child_exited";
}

test "a transport's own failure is told apart from the shell it was carrying" {
    const exited = struct {
        fn s(code: u8) c_int {
            return @as(c_int, code) << 8;
        }
    };
    // SSH SAYING THE FAILURE WAS ITS OWN: a link that dropped, a host that
    // stopped answering, a connection it could not make.
    try std.testing.expectEqualStrings("link_severed", transportEnd(exited.s(255)).?);
    // AND THE REMOTE SHELL'S OWN STATUS, PASSED THROUGH UNCHANGED, which
    // is the human leaving.
    try std.testing.expectEqualStrings("child_exited", transportEnd(exited.s(0)).?);
    try std.testing.expectEqualStrings("child_exited", transportEnd(exited.s(1)).?);
    // A remote command that exits 255 itself is the one collision, and for
    // the interactive login shell on this path it is not a thing that
    // happens. Pinned so the collision is a decision rather than a
    // surprise.
    try std.testing.expectEqualStrings("link_severed", transportEnd(exited.s(255)).?);
    // KILLED: neither answer is true, so neither is given.
    try std.testing.expect(transportEnd(9) == null);
    try std.testing.expect(transportEnd(15) == null);
}
...
pub fn runHold(allocator: Allocator, server: *run.RunServer, args: types.RunArgs) !void {
    ...
    // THE NAME THE SESSION ACTUALLY HAS, taken back out of the path it is
    // served on rather than from what was typed. `--id Foo` and `--id foo`
    // are one session ([[holder.canonical]]), and a child told `Foo` while
    // its socket, lock and record all say `foo` would be an agent whose id
    // does not name its own session ([[WI-2026-09-08-014]]).
    env_store[0] = try std.fmt.allocPrintSentinel(
        allocator, "SYNAPTY_AGENT_ID={s}", .{holder.nameOf(path)}, 0);
    ...
    // AND UNWOUND IF THE START FAILS. `init` has taken the name and bound
    // the socket by the time `start` runs, so a `start` that cannot spawn
    // — a thread limit, a pty exhausted — left both on disk with no
    // record: invisible to `sessions` and unsweepable, accumulating for
    // any name never used again ([[WI-2026-09-08-013]]).
    errdefer h.stop();
    // THE SAME ASSUMPTION AS A CLIENT WITH NO TERMINAL ([[protocol.assumed_rows]]):
    // this sizes the pty before anyone has attached to say otherwise.
    try h.start(argv.items, &env, .{
        .ws_row = protocol.assumed_rows,
        .ws_col = protocol.assumed_cols,
    });
    defer h.stop();
...
pub fn runSessions(allocator: Allocator) !void {
    ...
    for (rows[0..found]) |*row| {
        ...
        // A COLUMN ANSWERS ITS OWN QUESTION OR SAYS NOTHING. The sixth is
        // WHERE THE SESSION IS STANDING ([[RFC-0014]] C-PWD), and these two
        // rows used to put a sentence there instead — "pid 38694", "no path
        // for its socket" — because it was the only free space on the line.
        //
        // A READER CANNOT KNOW IT IS NOT A PATH. The workbench takes that
        // column as the session's directory and shows its last component as
        // the row's title, so a dead session appeared in a human's sidebar
        // as "Studio · pid 38694" — a pid presented as the folder the work
        // was in ([[WI-2026-09-07-005]]).
        //
        // `-` IS ALREADY THE WORD FOR "WOULD NOT SAY", and it is the true
        // answer here: nothing answered, so nothing knows where this
        // session was standing. The workbench then titles the row by its
        // NAME, which is what a human needs to end it anyway.
        const text = if (row.socket_missing or !row.reachable)
            std.fmt.bufPrint(&line, "{s}\tunreachable\t-\t-\t-\t-\t-\t-\t-\t{s}\n", .{ row.name(), dash(row.humanName()) }) catch continue
        else
...
const LocalSession = struct {
    ...
    /// THE FAR SIDE ANSWERED THIS ATTEMPT. Set where the welcome is read
    /// and cleared by the caller before each attempt, so an outage ends
    /// when the session was REACHED rather than when it had something new
    /// to show ([[WI-2026-09-09-002]]).
    reached: bool = false,
};
...
test "every word an ended session can be reported by is one the workbench reads" {
    // THE ACCOUNT IS A WIRE BETWEEN TWO LANGUAGES. `account.say(.end,
    // @tagName(outcome))` puts these names on it and the workbench matches
    // them as strings ([[LinkState]] `AccountEnd`), so renaming a case here
    // is a protocol change no compiler on either side can see. Three of
    // these had no test anywhere against what this file actually writes.
    //
    // The list is written out rather than derived, on purpose: a test that
    // asked the enum what it is called would agree with any answer.
    const reads = [_][]const u8{
        // Retried rather than reported — the only outcome that writes no
        // `end` at all, and the reason this is a list and not a count.
        "transport_lost",
        "child_exited",
        "displaced",
        "stopped",
    ...
    const fields = @typeInfo(AttemptEnd).@"enum".fields;
    try std.testing.expectEqual(reads.len, fields.len);
    inline for (fields, 0..) |f, i| try std.testing.expectEqualStrings(reads[i], f.name);

    // `link_severed` is the seventh word and belongs to no client: the
    // connection itself writes it, on a host that keeps no session, where
    // ssh IS the pane's child. It is pinned where it is decided, against
    // the exit statuses that produce it.
}
...
pub fn runAttachThrough(allocator: Allocator, args: types.AttachArgs) !void {
    ...
    // WHEN THE LINK WENT, so the budget below is spent in wall-clock and
    // not in attempts. A budget counted in attempts silently changes how
    // long the client tries for as soon as the wait between them backs
    // off ([[WI-2026-09-07-004]]).
    var down_since: ?i64 = null;
    while (true) {
        // WHETHER THIS ATTEMPT REACHED THE FAR SIDE. If it did, the
        // outage it ended is over: the next one starts its own ramp and
        // its own budget.
        //
        // NOT "DID MORE BYTES ARRIVE". That was the test, and it is a
        // different question with the same answer only for a session that
        // is producing output. A resumed attach sets `rendered` from
        // `welcome.position`, and an IDLE session's position is identical
        // on every reconnect — so `rendered` never moved, the ramp kept
        // doubling and the budget kept running from the first loss.
        // Measured: five kills against a live holder running `sleep 1` in
        // a loop reconnected every time, at 1s, 2s and 4s, and after half
        // an hour it would have paused saying "stopped dialling after 30
        // minutes down" having reached the session on every attempt
        // ([[WI-2026-09-09-002]]).
        session.reached = false;
        ...
        switch (outcome) {
            ...
            .transport_lost => {
                attempts += 1;
                // HOW LONG THE LINK HAS BEEN DOWN, on the clock that measures
                // spans ([[sys.upMillis]]). On the wall clock an NTP step
                // forward — which is what a waking laptop does, and waking is
                // when reconnects happen — could spend the whole budget on the
                // first loss and drop straight into `waitToBeTold`.
                if (down_since == null) down_since = sys.upMillis();
                const down_for: u64 = @intCast(@max(sys.upMillis() - down_since.?, 0));
                ...
                // SPENT, SO IT CAN RUN OUT. A tight loop would spend the
                // whole outage failing to dial — 3600 attempts an hour,
                // none of which could succeed sooner than the network
                // allowed — so the wait backs off, and after half an hour
                // down the client stops dialling and waits to be told
                // ([[progress.Retry]]).
                if (!progress_mod.Retry.keepTrying(down_for)) {
                    if (!waitToBeTold(&account, down_for, is_tty)) {
                        account.say(.end, "stopped");
                        break;
                    }
                    attempts = 0;
                    down_since = sys.upMillis();
                    continue;
                }
                io_mod.get().sleep(
                    std.Io.Duration.fromMilliseconds(
                        @intCast(progress_mod.Retry.delayMs(attempts)),
                    ),
                    .awake,
                ) catch {};
            },
...
/// STOPPED DIALLING, AND WAITING TO BE TOLD TO START AGAIN.
///
/// THE PROCESS STAYS, AND THAT IS THE WHOLE POINT. Exiting here would be
/// simpler and would cost the human their pane: this process owns the pty,
/// so its screen and the scrollback above it live exactly as long as it
/// does, and a pane rebuilt around a new one starts empty. A resumed
/// attach repaints over what is there ([[RFC-0014]] C-RESTORE); a
/// restarted pane has nothing to repaint over.
///
/// TWO WAYS BACK, because there are two people who might want it. The
/// workbench presses a button and leaves a marker beside the account
/// ([[progress.RetrySignal]]); a human sitting at the pane presses a key,
/// which is ghostty's own idiom for a terminal waiting on them and needs
/// no explaining.
///
/// AND ONE WAY OUT. The detach key still works, or the human would be shut
/// in a pane whose only exit is closing it — the lesson
/// [[WI-2026-09-02-020]] already paid for once.
///
/// Returns true to dial again, false when the human detached.
fn waitToBeTold(
    ...
    const text = std.fmt.bufPrint(
        &msg,
        "stopped dialling after {d} minutes down",
        .{down_for_ms / 60_000},
    ) catch "stopped dialling";
    account.say(.paused, text);
    if (!account.listening()) {
        screenNotice("stopped reconnecting — press any key to try again") catch {};
    }
    while (true) {
        if (progress_mod.RetrySignal.take()) break;
        // THE REST IS THE ONLY THING IN THIS LOOP, so it happens whatever
        // else does. It used to be inside `if (is_tty and …)`, and `and`
        // short-circuits: with no terminal the body was a marker check and
        // nothing else, spinning a core forever. The comment there claimed
        // the guard PREVENTED that; putting it in front of the loop's only
        // blocking call guaranteed it ([[WI-2026-09-08-001]]).
        //
        // Non-interactive is not exotic — it is how the e2e script runs
        // this, and any piped or wrapped invocation.
        const ready = if (is_tty) sys.waitReadable(0, 400) catch false else blk: {
            io_mod.get().sleep(std.Io.Duration.fromMilliseconds(400), .awake) catch {};
            break :blk false;
        };
        if (!ready) continue;

        // NOTHING ELSE READS STDIN BETWEEN ATTEMPTS — the forwarding
        // thread lives and dies inside `attachOnce` — so a byte here is
        // the human's, and it is theirs to interpret.
        var b: [64]u8 = undefined;
        const n = sys.read(0, &b) catch 0;
        // END OF INPUT IS NOT A KEY TO WAIT FOR. A hung-up terminal polls
        // ready forever and reads zero every time, so treating this as
        // "nothing yet" was the second way this spun. There is nobody left
        // to press anything.
        if (n == 0) {
            if (!account.listening()) screenNotice("detached") catch {};
            return false;
        }
        if (std.mem.indexOfScalar(u8, b[0..n], detachKey()) != null) {
            if (!account.listening()) screenNotice("detached") catch {};
            return false;
        }
        break;
    }
    account.say(.resumed, "dialling again");
    return true;
}

const ThroughState = struct {
    /// The same pair as [[AttachState]]: `throughInputLoop` and
    /// `throughSizeLoop` both write to `out_fd`. The `defer` below says
    /// in as many words that both of these threads write to it — and the
    /// conclusion drawn from that was only to order the CLOSE.
    writer: holder.FrameWriter = .{},
...
/// WHAT THE TRANSPORT'S DEATH MEANT, from the one fact that carries the
/// answer end to end.
///
/// THE RELAY SAYS "no session named X" AND EXITS 2, and ssh propagates a
/// remote command's exit status unchanged — so that 2 arrives here having
/// crossed the network, while ssh's own failures arrive as 255 and a
/// transport that could not run at all arrives as whatever it chose. The
/// status was being discarded and the answer inferred from a read failure
/// instead.
///
/// THE DEFAULT IS THE ONE THAT RETRIES. The two mistakes are not
/// symmetrical: calling a missing session a lost link costs one more dial
/// and a second's delay, where calling a lost link a missing session ends
/// a pane somebody meant to come back to — which is the asymmetry
/// [[RFC-0014]] C-LIVENESS states for its own determination and the
/// reasoning holds here unchanged.
///
/// IT MUST NOT BLOCK, AND AN EARLIER VERSION OF THIS DID.
///
/// The comment here used to argue that `wait` is only reached because the
/// read failed, "which for a pipe means the writer is gone". The function
/// it depended on says otherwise: `holder.readWelcome` answers
/// `error.Closed` for ANY frame that is not a welcome, and `readFrame`
/// answers `error.TooLarge` whenever the length it reads exceeds the
/// caller's buffer. Neither is a statement about the writer.
///
/// SO A CHATTY TRANSPORT HUNG THE PANE OUTRIGHT. An ssh whose far side
/// prints a MOTD, or echoes from a `.bashrc` on a non-interactive shell —
/// the misconfiguration that classically breaks `scp` — writes bytes that
/// are not a welcome and then goes on living. `wait` then blocked until
/// that process exited, which is never; and the input thread is not
/// spawned until after the handshake, so the detach key was not there
/// either. The only way out was closing the pane, which is the failure
/// `waitToBeTold` exists to avoid. Reproduced against the built binary:
/// twelve seconds, no `lost`, no retry ([[WI-2026-09-08-001]]).
///
/// SO: ASK, DO NOT WAIT. A transport that has already gone is reaped and
/// its status read; one that is still there is abandoned — killed, since
/// nothing will read from it again — and reported as the dead link it now
/// is.
///
/// AND HOW LONG TO ASK FOR IS THE CALLER'S FACT, NOT A CONSTANT. See
/// [[ReadEnd]].
const ReadEnd = enum {
    /// THE TRANSPORT CLOSED ITS STDOUT. Nothing more will ever come from
    /// it, and a process that has finished writing is a process on its way
    /// out — so its status is worth waiting a moment for, and two seconds
    /// is a moment against a dial that is over either way.
    stream_ended,
    /// IT IS ALIVE AND NOT SPEAKING. A MOTD, a `.bashrc` echoing on a
    /// non-interactive shell, a frame that is not a welcome, or the
    /// handshake deadline running out with the pipe still open. There is
    /// nothing to wait for: this process has not decided to exit and may
    /// never. Killing it at once is also FASTER than the fifth of a second
    /// that used to be spent here on every such attempt.
    still_talking,

    fn waitMs(self: ReadEnd) i64 {
        return switch (self) {
            .stream_ended => 2000,
            .still_talking => 0,
        };
...
fn transportVerdict(child: *std.process.Child, reaped: *bool, ended: ReadEnd) AttemptEnd {
    const pid = child.id orelse return .transport_lost;

    // A CHILD ON ITS WAY OUT HAS NOT FINISHED GOING, and one WNOHANG
    // cannot tell that from a child that is staying. The write end of its
    // stdout closes during exit, BEFORE it becomes reapable — so at the
    // instant the handshake sees end-of-stream, `waitpid` may still answer
    // "running" for a process that has already chosen its status. Killing
    // it there discards exit 9, which is the one fact this function exists
    // to carry: `no_session` is terminal and tells the human the session is
    // gone, where `transport_lost` retries forever against a host that will
    // never have it ([[WI-2026-09-08-004]]).
    //
    // A FIFTH OF A SECOND WAS A RACE DRESSED AS A BOUND. It was chosen for
    // "a process that is already exiting", and how long that takes is not
    // ours to know: measured, a transport differing from another only by
    // the gap between closing stdout and exiting lost its status outright.
    //
    //     sh -c 'exec 1>&-; exit 9'            end no_session at +18ms
    //     sh -c 'exec 1>&-; sleep 5; exit 9'   killed at 200ms, 9 gone
    //
    // So the window is asked of the READ that sent us here, which already
    // knows which case this is ([[WI-2026-09-09-003]]).
    var waited: i64 = 0;
    while (waited < ended.waitMs()) : (waited += 10) {
        var status: c_int = 0;
        const r = std.c.waitpid(pid, &status, 1); // WNOHANG
        if (r == pid) {
            reaped.* = true;
            closeTransportPipes(child);
            return verdictOfStatus(status);
        }
        // ECHILD — NOT "STILL RUNNING". `r != pid` used to cover this too,
        // and the branch below then signalled a pid this process no longer
        // owns, which the system is free to have given to somebody else.
        if (r < 0) {
            reaped.* = true;
            closeTransportPipes(child);
    ...
    // STILL THERE. This is a transport that is alive and not speaking the
    // protocol — the chatty MOTD. Killing it is not impatience: the
    // attempt is over either way, and one nobody will read from holds a
    // pty and a connection open for nothing. `Child.wait` then does the
    // cleanup, and is bounded because SIGKILL cannot be ignored.
...
/// WHAT A REAPED TRANSPORT'S STATUS MEANS — the decision on its own, with
/// no process in it, so it can be put a case to.
///
/// It could not be before: the caller takes a `*std.process.Child`, so
/// asking it about exit 255 meant spawning something that exits 255. The
/// e2e only ever produces 1 (an unreachable host) and the relay's own
/// code, and 255 is ssh's own failure — the commonest real input, and the
/// one the "default is the outcome that retries" argument is entirely
/// about ([[WI-2026-09-08-004]]).
///
/// The low seven bits carry a signal; zero there means the next byte up is
/// an ordinary exit code ([[holder.childExitLoop]] reads it the same way).
fn verdictOfStatus(status: c_int) AttemptEnd {
    if ((status & 0x7f) != 0) return .transport_lost;
    const code: u8 = @intCast((status >> 8) & 0xff);
    return if (code == relay_no_session_status) .no_session else .transport_lost;
}

test "only the relay's own word ends the retrying" {
    const exited = struct {
        fn s(code: u8) c_int {
            return @as(c_int, code) << 8;
        }
    };
    // THE RELAY SAID SO, and it is the only thing that may.
    try std.testing.expectEqual(AttemptEnd.no_session, verdictOfStatus(exited.s(relay_no_session_status)));

    // SSH'S OWN FAILURE — a host that is down, a key refused, DNS that
    // did not resolve. The commonest input there is, and the one that
    // must retry rather than tell the human their session is gone.
    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(exited.s(255)));

    // THE CLI'S USAGE-ERROR CODE. This WAS the relay's word for "no
    // session", so a transport invoked wrongly ended the pane
    // ([[WI-2026-09-08-001]]).
    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(exited.s(2)));

    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(exited.s(0)));
    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(exited.s(1)));

    // KILLED. A transport that died on a signal said nothing, and a
    // signal number that happens to equal the relay's code says nothing
    // either — which is what reading the low bits first is for.
    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(9));
    try std.testing.expectEqual(AttemptEnd.transport_lost, verdictOfStatus(15));
}

/// THE PIPES GO WITH THE REAP, WHOEVER DID IT.
///
/// `std.process.Child.wait` closes stdin, stdout and stderr on its way out
/// — `childWaitPosix` does it in a `defer`. Reaping with a raw `waitpid`
/// instead skips that, so every attempt that ended here leaked two or
/// three descriptors, in a loop that retries for as long as a host is
/// down: a few hours at one dial per thirty seconds is on the order of a
/// thousand ([[WI-2026-09-08-004]]).
///
/// AND OF ALL THE PLACES TO LEAK THEM. Descriptor exhaustion is exactly
/// the condition [[SessionRecord]]'s `unknown` claim state was added to
/// survive; these two are the same failure met from opposite ends.
fn closeTransportPipes(child: *std.process.Child) void {
    if (child.stdin) |f| {
        sys.close(@intCast(f.handle));
        child.stdin = null;
    }
    if (child.stdout) |f| {
        sys.close(@intCast(f.handle));
    ...
    // NOT STDERR. Another thread is reading it ([[progress.pumpLines]]),
    // and closing a descriptor out from under a reader is a race the
    // system will happily lose in the worst way: the number is free the
    // instant it is closed, so the next open in this process hands it to
    // somebody else and the pump reads their file. `attachOnce` closes it
    // after that thread has been stopped and joined.
}

/// The relay's word for "nothing is holding that name". A number rather
/// than a message because it is the only thing that survives ssh.
///
/// NOT 2, WHICH THIS WAS. Two is the CLI's universal usage-error code
/// ([[WI-2026-09-02-021]], `cli.zig`) — a missing argument, an unparseable
/// flag, the attach chooser without a tty. A relay invoked wrongly would
/// have been read as a relay reporting that the session was gone, which is
/// the terminal answer that ends a pane ([[WI-2026-09-08-001]]).
const relay_no_session_status: u8 = 9;
...
fn attachOnce(
    ...
    // for both.
    //
    // AND REAPED ONCE. The status is a fact worth having — it is what
    // tells a host that is not there from a session that is not there
    // ([[transportVerdict]]) — so the handshake path takes it, and this
    // takes it only if nothing else did.
    var reaped = false;
    defer if (!reaped) {
        _ = std.process.Child.wait(&child, io_mod.get()) catch {};
    };
    ...
    var pump_stop = std.atomic.Value(bool).init(false);
    if (child.stderr) |errf| {
        stderr_thread = std.Thread.spawn(
            .{},
            progress_mod.pumpLines,
            .{ account, errf.handle, &pump_stop },
        ) catch null;
    }
    // STOPPED, JOINED, AND ONLY THEN CLOSED, in that order and in one
    // place. The pipe does not reach end-of-stream while anything the
    // transport spawned still holds its write end, so the thread has to be
    // TOLD to stop rather than waited out ([[progress.pumpLines]]); and the
    // descriptor cannot be closed while it is being read, because the
    // number is reusable the instant it is closed.
    defer {
        pump_stop.store(true, .release);
    ...
    var theirs: u8 = 0;
    // BOUNDED, BECAUSE THE FAR SIDE MAY NOT BE SPEAKING THE PROTOCOL AT
    // ALL. Thirty seconds is far longer than a handshake over a working
    // link and far shorter than forever, which is what this was
    // ([[holder.readWelcomeWithin]], [[WI-2026-09-08-001]]).
    const welcome = holder.readWelcomeWithin(from_relay, &wbuf, &theirs, 30_000) catch |err| {
        if (err == error.VersionMismatch) {
            reportVersionMismatch(theirs) catch {};
            return .version_mismatch;
        }
        // WHY THE HANDSHAKE FAILED IS THE TRANSPORT'S TO SAY, NOT OURS TO
        // ASSUME. Every failure here used to be called `no_session` — a
        // transport that died, a link that dropped mid-handshake, an ssh
        // that could not resolve the host, a VPN that was off. None of
        // those is a statement about whether a session exists, and
        // `no_session` is TERMINAL, so a host that was merely down ended
        // the pane on its first attempt without ever retrying
        // ([[WI-2026-09-07-006]]).
        // AND THE READ SAYS WHICH KIND OF FAILURE THIS WAS. End of stream
        // means the transport has finished writing and is going; a
        // deadline, a MOTD or a frame that is not a welcome means it is
        // alive and not speaking ([[ReadEnd]]). A partial frame is a
        // writer that stopped mid-sentence, which is far more often a
        // transport dying than one stalling, and the cost of being wrong
        // is two seconds on an attempt that is over anyway.
        return transportVerdict(&child, &reaped, switch (err) {
            error.EndOfStream, error.PartialFrame => .stream_ended,
            else => .still_talking,
        });
    };
    ...
    // THE HOLDER ANSWERED. Whatever happens to this transport afterwards,
    // the session was reached and the outage before it is over.
    ...
    if (answer == .unavailable) {
        ...
        // human's to know.
        //
        // A `hole`, NOT A `lost`. `lost` means "the transport died and
        // another attempt follows" ([[progress.Kind]]) and neither half
        // is true here: the link is up and the restoration is already on
        // its way. Saying it flashed "Link lost — reconnecting" in red on
        // the pane strip of a connection that had just succeeded
        // ([[WI-2026-09-11-020]]). It went to `note` next, which is where
        // the transport's own stderr goes and which only the dialling
        // placeholder reads, so it was kept and shown to nobody
        // ([[WI-2026-09-16-002]]).
        const hole = progress_mod.Hole.position_refused;
        account.say(hole.kind(), hole.text());
    }
    if (answer == .resumed) {
    ...
    while (true) {
        ...
        if (!state.running.get()) return if (state.detached) .stopped else .transport_lost;
        ...
        switch (frame.kind) {
            ...
            .gap => {
                // A HOLE IN THE SCROLLBACK IS NOT A DEAD LINK, and saying
                // `lost` here was a lie that never ended. `lost` sets
                // [[ConnectProgress]]`.lostSince`, which ONLY `paint` and
                // `live` clear — and after a gap neither ever comes: the
                // `.data` arm below says nothing, `.paint` is only
                // emitted from a `.restore`, and the `.live` for this
                // resumed handshake was already spent above. So a pane
                // whose output was streaming in front of the human wore
                // "Link lost — reconnecting · 14m" for the rest of the
                // attach, took the sidebar mark at sixty seconds, and
                // pulled them to a workspace to fix a host that had never
                // stopped answering ([[WI-2026-09-11-020]]).
                //
                // The link is UP and what follows is live. The fact
                // worth saying is the hole, and it has a channel of its
                // own: on `note` it reached only the placeholder a
                // painted pane no longer has ([[WI-2026-09-16-002]]).
                const hole = progress_mod.Hole.catch_up_outrun;
                account.say(hole.kind(), hole.text());
...
fn throughInputLoop(state: *ThroughState, stdin_fd: sys.fd_t) void {
    ...
    while (state.running.get()) {
        ...
        const step = detach.step(buf[0..n], &out);
        if (step.forward.len > 0) {
            state.writer.write(state.out_fd, .input, step.forward) catch break;
        }
        if (step.detach) {
            state.detached = true;
            state.running.set(false);
...
fn throughSizeLoop(state: *ThroughState, stdin_fd: sys.fd_t, initial: sys.winsize, follow: bool) void {
    ...
    while (state.running.get()) {
        ...
        std.mem.writeInt(u16, p[0..2], now.ws_row, .little);
        std.mem.writeInt(u16, p[2..4], now.ws_col, .little);
        state.writer.write(state.out_fd, .resize, &p) catch return;
    }
}
...
const FakeDaemon = struct {
    ...
    fn start(path: []const u8, reply: ?[]const u8) !FakeDaemon {
        ...
        // THE DESCRIPTOR BY VALUE, NOT A POINTER TO THIS FRAME.
        //
        // This passed `&server` — a local that this function RETURNS BY
        // VALUE, so the slot the thread was given died the moment `start`
        // did, and the copy the caller holds lives somewhere else. The
        // thread then called `accept` through a dangling pointer, and
        // whether it worked depended on whether the stack had been
        // reused: green on this machine and `timed_out` on CI, which is
        // what the test above records having seen once before and fixed
        // only as far as "fails rather than crashes"
        // ([[WI-2026-09-15-003]]).
        //
        // `accept` wants nothing but the listener, so handing it the fd
        // ends the lifetime question rather than answering it.
        const thread = try std.Thread.spawn(.{}, serve, .{ server.listener_fd, reply });
        return .{ .server = server, .thread = thread };
    }

    fn serve(listener_fd: sys.fd_t, reply: ?[]const u8) void {
        const conn = sys.accept(listener_fd) catch return;
