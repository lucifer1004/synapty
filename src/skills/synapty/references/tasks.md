...
`synapty task` reaches the workbench the same way the presentation verbs
do, so it answers the same way: **`0` done, `2` you used it wrong or are
not in a pane, `3` a human has been asked and has not answered, `4` the
workbench refused** — with the JSON on stdout and the reason as a
sentence on stderr. A failed `claim` or `update` is not a `0`; check it
before reporting the work as coordinated.
