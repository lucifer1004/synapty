...
fn waitForUnregistered(server: *hub.HubServer, agent_id: []const u8, timeout_ms: u64) bool {
    ...
    const deadline = std.Io.Timestamp.now(io_mod.get(), .real).toMilliseconds() + @as(i64, @intCast(timeout_ms));
    while (std.Io.Timestamp.now(io_mod.get(), .real).toMilliseconds() < deadline) {
        // A TABLE THAT CANNOT BE READ ESTABLISHES NOTHING, and `true` here
        // means "established that the agent is gone". `waitForRegistered`
        // answers the same failure with `false` for the same reason: the
        // question is what can be shown, and an error shows neither
        // ([[WI-2026-09-10-004]]).
        const agents = server.registeredAgents(alloc) catch return false;
        defer {
            for (agents) |id| alloc.free(id);
...
test "e2e: RunServer IPC send through daemon to Hub delivers to target" {
    ...
    var server = try run.RunServer.init(alloc, "alice", .{ .addr = "127.0.0.1", .port = h.port });
    defer server.deinit();
...
test "e2e: wrapper survives hub restart — reconnect + durable identity re-claim (WI-2026-08-11-017)" {
    ...
    var server = try run.RunServer.init(alloc, "local-rcn1", .{ .addr = "127.0.0.1", .port = port });
    defer server.deinit();
    const threads = try server.startThreads();
    defer server.stopThreads(threads);
...
test "e2e: RunServer IPC channel create and send through daemon" {
    ...
    // Start RunServer for "alice".
    var srv_alice = try run.RunServer.init(alloc, "alice", .{ .addr = "127.0.0.1", .port = h.port });
...
test "e2e: RunServer.init survives the app-start race (hub binds late)" {
    ...
    const Ctx = struct {
        ...
        fn initLate(self: *@This()) void {
            self.server = run.RunServer.init(self.alloc, "late-agent", .{ .addr = "127.0.0.1", .port = self.port }) catch {
                self.failed = true;
                return;
            };
        }
...
test "e2e: recv --wait BLOCKS over the pane IPC path until a dm arrives (WI-2026-08-10-001)" {
    ...
    var server = try run.RunServer.init(alloc, "alice-blk", .{ .addr = "127.0.0.1", .port = h.port });
    defer server.deinit();
    const threads = try server.startThreads();
...
test "e2e: daemon shutdown wakes a parked recv --wait instead of hanging (WI-2026-08-10-001)" {
    ...
    var server = try run.RunServer.init(alloc, "alice-shut", .{ .addr = "127.0.0.1", .port = h.port });
    defer server.deinit();
    const threads = try server.startThreads();
...
test "e2e: identity_displaced terminates a parked recv --wait with the stable error (RFC-0008 C-MAILBOX)" {
    ...
    var server = try run.RunServer.init(alloc, "alice-disp", .{ .addr = "127.0.0.1", .port = h.port });
    defer server.deinit();
    const threads = try server.startThreads();
    defer server.stopThreads(threads);
...
/// Poll until this peer's spool has let go of everything it was holding.
///
/// WAITED FOR, BECAUSE DELIVERY IS NOT RELEASE. `flushSpoolToLocked` keeps
/// its copy on purpose — [[RFC-0009]] C-DELIVERY: "a hub holds its copy
/// until the acknowledgement arrives, then releases it" — so the peer's
/// mailbox filling up is the wrong thing to read this on. The order is: A
/// writes, B enqueues, B acknowledges, A releases; `waitForMail` returns
/// after the second of those and the spool is read after the fourth.
///
/// Measured: with six copies of this test running at once, the sample
/// taken straight after `waitForMail` was 2 in three runs of thirty-six
/// and 1 in one more — and every one of them had drained within a single
/// five-millisecond poll ([[WI-2026-09-09-011]]).
///
/// THE ASSERTION IS NOT WEAKENED BY WAITING. What it means to check is
/// that the copies are released rather than leaked, and a bounded wait
/// followed by the same equality still fails if they never are.
fn waitForSpoolDrained(server: *hub.HubServer, peer: []const u8, timeout_ms: u64) void {
    const deadline = std.Io.Timestamp.now(io_mod.get(), .real).toMilliseconds() + @as(i64, @intCast(timeout_ms));
    while (std.Io.Timestamp.now(io_mod.get(), .real).toMilliseconds() < deadline) {
        if (server.state.spool.count(peer) == 0) return;
        io_mod.get().sleep(std.Io.Duration.fromMilliseconds(2), .awake) catch {};
    }
}
...
test "e2e RFC-0009: two hubs federate — forward, spool on outage, flush in order, local A2A never stops" {
    ...
    // AND THEN FOR THE ACKNOWLEDGEMENTS, which are what release the held
    // copies ([[waitForSpoolDrained]]).
    waitForSpoolDrained(hub_a.server, "beta", 5000);
...
test "e2e ADR-0008: tool_request round-trips hub -> workbench -> agent, and fails fast with no workbench" {
    ...
    // THE WORD [[RFC-0007]] C-EXEC-SCOPE NAMES, not a paraphrase of it.
    // A hub with no workbench is the ordinary case for a server hosting
    // detached agents, so this is a refusal callers branch on.
    try std.testing.expect(mem.indexOf(u8, refusal, "no_workbench") != null);
...
test "every state the shipped manifests name is one this protocol has" {
    // THE VOCABULARY CROSSES THREE LANGUAGES AND WAS PINNED IN NONE OF
    // THEM AGAINST ANOTHER. `Status`'s own doc says "Tag names ARE the
    // protocol strings — every surface uses them verbatim", and the five
    // tags are restated in Swift three times over and in `detect/*.json`
    // 122 times. Nothing read the manifests against this enum.
    //
    // AND THE MANIFEST COPY FAILS BY DROPPING RULES. `AgentDetector`
    // compiles a manifest with `guard passiveStates.contains(rule.state)
    // … else { continue }` — an unknown state does not error, does not
    // log and does not fail the load; the manifest simply compiles with
    // fewer rules than it has. Rename a tag here and detection degrades
    // across the whole fleet while every test stays green and every
    // manifest still "loads" ([[WI-2026-09-11-014]]).
    //
    // `skip` is the one extra word: it is accepted at compile time and
    // never leaves the classifier, so it is a manifest concept rather
    // than a wire state.
    const io = io_mod.get();
    var dir = std.Io.Dir.cwd().openDir(io, "detect", .{ .iterate = true }) catch {
        // The suite is run from the repo root by `zig build test`; if the
        // directory is not there this is not the thing under test.
    ...
    while (try it.next(io)) |entry| {
        ...
        // Read the field rather than parsing the whole manifest: what is
        // under test is the WORD, and a shape change here would be a
        // different failure with its own message.
        const key = "\"state\":";
        var at: usize = 0;
        while (mem.indexOfPos(u8, raw, at, key)) |found| {
            var i = found + key.len;
            ...
            const word = raw[start..i];
            at = i;
            checked += 1;
            if (protocol.Status.fromString(word) != null) continue;
            if (mem.eql(u8, word, "skip")) continue;
            std.debug.print(
                "detect/{s} names the state \"{s}\", which is neither a Status tag nor `skip` — " ++
                    "AgentDetector would drop that rule without a word\n",
                .{ entry.name, word },
            );
            return error.ManifestNamesAnUnknownState;
        }
    }
    // A PASS THAT READ NOTHING IS NOT A PASS.
    try std.testing.expect(checked > 0);
}

test "e2e WI-2026-08-12-008: peer_connect makes the local hub dial its peer" {
    // Stands in for the real path: `synapty host-setup` brings up a hub on the
