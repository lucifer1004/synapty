...
// ---------------------------------------------------------------------------
// Where a pane's socket lives
// ---------------------------------------------------------------------------

/// THE PANE IPC SOCKET'S SHAPE, STATED ONCE.
///
/// FOUR AUTHORS AND NO OWNER. `run.zig` created it from
/// `"/tmp/synapty-{d}.sock"`, swept `/tmp` for `"synapty-"` + `".sock"`
/// and sliced the pid back out by those two lengths, and `mcp.zig`
/// discovered it from `"/tmp/synapty-"` plus `"{s}{d}.sock"`. Three
/// independent statements of one convention, plus a fourth in the shell
/// suites ([[WI-2026-09-11-017]]).
///
/// WHAT DRIFT COSTS. `synapty mcp-serve` falls back to the process-tree
/// walk when `SYNAPTY_SOCK` is absent; a changed prefix means it logs "no
/// IPC socket found" and every `synapty_send`, `synapty_recv` and
/// `synapty_notify` stops working while the pane itself is fine. The
/// sweeper independently stops recognising its own files and leaks one
/// socket per dead pane forever.
///
/// STILL IN `/tmp`, AND THAT IS A SEPARATE QUESTION FROM THIS ONE. The
/// holder's sockets moved out of it for a named reason ([[paths]]: "a
/// directory the operating system is free to sweep and any user is free
/// to empty ... the holder and its child went on running with nothing
/// able to name them"). This is the one path still there, and moving it
/// would break the `SYNAPTY_SOCK` contract with every running pane and
/// every deployed binary — a product decision, not a tidy-up. Naming the
/// convention is what makes that decision a one-line change when it is
/// taken.
pub const pane_socket = struct {
    pub const dir = "/tmp";
    pub const prefix = "synapty-";
    pub const suffix = ".sock";

    /// The socket a pane wrapper of this pid serves.
    pub fn path(buf: []u8, pid: i32) ![]const u8 {
        return std.fmt.bufPrint(buf, "{s}/{s}{d}{s}", .{ dir, prefix, pid, suffix });
    }

    /// The prefix a discoverer joins a pid onto — the same string, without
    /// the pid, for the one caller that builds it that way.
    pub fn pathPrefix(buf: []u8) ![]const u8 {
        return std.fmt.bufPrint(buf, "{s}/{s}", .{ dir, prefix });
    }

    /// The pid a directory entry names, or null when the entry is not one
    /// of ours. THE SWEEPER'S HALF OF THE SAME CONVENTION: it sliced the
    /// digits out by two hard-coded lengths, so a prefix change made it
    /// silently stop recognising the files it had just written.
    pub fn pidOf(entry_name: []const u8) ?[]const u8 {
        if (!mem.startsWith(u8, entry_name, prefix)) return null;
        if (!mem.endsWith(u8, entry_name, suffix)) return null;
        const digits = entry_name[prefix.len .. entry_name.len - suffix.len];
        return if (digits.len == 0) null else digits;
    }
};

test "what the pane socket is called, read back the way the sweeper reads it" {
    var buf: [128]u8 = undefined;
    const p = try pane_socket.path(&buf, 4242);
    try std.testing.expectEqualStrings("/tmp/synapty-4242.sock", p);

    // The name alone is what the sweeper sees, and it must recover the pid
    // the creator put there ([[WI-2026-09-11-017]]).
    const name = p[pane_socket.dir.len + 1 ..];
    try std.testing.expectEqualStrings("4242", pane_socket.pidOf(name).?);

    // And anything that is not ours is not ours to judge.
    try std.testing.expect(pane_socket.pidOf("synapty-.sock") == null);
    try std.testing.expect(pane_socket.pidOf("other-4242.sock") == null);
    try std.testing.expect(pane_socket.pidOf("synapty-4242.json") == null);
}
