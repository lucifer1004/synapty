...
/// HOW LONG A SENDER WAITS FOR AN ANSWER THE HUB CANNOT GIVE YET.
///
/// The same ladder one module along. A cross-machine send waits for a
/// peer's acknowledgement before the hub can answer at all
/// (`handlers.forward_ack_bound_ms`), so the CLI's own read bound must
/// outlast that or every forwarded message reads as "the hub did not
/// answer". That relation was stated in prose above `readSendAnswer` and
/// nowhere else, with the two numbers — 5000 and 10_000 — written out in
/// two modules ([[WI-2026-09-11-015]]).
///
/// DERIVED, so the relation is the code rather than a paragraph about it,
/// which is exactly what [[WI-2026-08-30-010]] did for the rung above.
/// THE TERMINAL GEOMETRY TO ASSUME WHERE NOBODY HAS MEASURED ONE.
///
/// Here rather than beside any one caller because it goes ON THE WIRE, in
/// the hello, and the holder produces a full-screen restoration for
/// exactly these dimensions. Three places need it and each had its own
/// copy: the local attach and the `through` path when there is no tty to
/// ask, and `run --hold` sizing the pty before any client has attached. A
/// shell test asserting that a repaint reached the LAST row had a fourth,
/// unlinked — so raising the number would have left that assertion green
/// while silently demoting what it meant ([[WI-2026-09-12-002]]).
///
/// `holder.ClientEntry` used to carry a fifth as a field default. It is
/// built in exactly one place and that place always supplies the size
/// from the hello, so the default could never be reached; it is gone
/// rather than kept in step ([[WI-2026-09-15-001]]).
pub const assumed_rows: u16 = 24;
pub const assumed_cols: u16 = 80;

pub const forward_ack_bound_ms: i64 = 5_000;
pub const send_answer_wait_ms: i64 = forward_ack_bound_ms * 2;

comptime {
    // GUARDS THE DERIVATION, NOT THE VALUES — the line above already makes
    // this true. It fires only if the derivation is replaced by a literal,
    // which is the mistake it exists for ([[WI-2026-09-12-002]]).
    if (send_answer_wait_ms <= forward_ack_bound_ms) @compileError(
        "C-DELIVERY: a sender must outlast the hub's own acknowledgement bound, " ++
            "or every forwarded message reads as a hub that did not answer",
    );
}
