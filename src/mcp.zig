...
fn handleInitialize(allocator: Allocator, id: json.Value) ![]const u8 {
    ...
    // THE VERSION THIS BUILD IS, not a literal beside it. `build.zig.zon`
    // is the one place it lives — the app, the DMG and the cask all read
    // it — and this told every MCP client `0.1.0` from a string of its
    // own ([[WI-2026-09-11-004]]).
    return std.fmt.allocPrint(allocator,
        \\{{"jsonrpc":"2.0","id":{s},"result":{{"protocolVersion":"2025-03-26","capabilities":{{"tools":{{}}}},"serverInfo":{{"name":"synapty","version":"{s}"}}}}}}
    , .{ id_str, @import("version").string });
}
...
fn discoverSocket(allocator: Allocator) ?[]const u8 {
    ...
    // THE ONE PLACE THE CONVENTION IS STATED ([[ipc.pane_socket]]). A
    // prefix that drifted from the creator's meant this logged "no IPC
    // socket found" and every tool call went unavailable while the pane
    // itself was fine ([[WI-2026-09-11-017]]).
    var pref_buf: [64]u8 = undefined;
    const prefix = ipc.pane_socket.pathPrefix(&pref_buf) catch "/tmp/synapty-";
    return discoverSocketFrom(allocator, pid, prefix);
}
...
fn discoverSocketFrom(allocator: Allocator, start_pid: i32, prefix: []const u8) ?[]const u8 {
    ...
    while (pid > 1 and depth < 16) : (depth += 1) {
        ...
        const path = std.fmt.bufPrint(
            &path_buf, "{s}{d}{s}", .{ prefix, pid, ipc.pane_socket.suffix }) catch return null;
...
test "handleRequest: initialize returns correct protocolVersion and serverInfo" {
    ...
    // THE VERSION THIS BUILD IS, asked of the same place the server
    // reads. This asserted the literal `0.1.0`, so bumping the manifest
    // left the MCP server telling clients the old number and this test
    // green about it ([[WI-2026-09-11-004]]).
    var vbuf: [64]u8 = undefined;
    const want = try std.fmt.bufPrint(&vbuf, "\"version\":\"{s}\"", .{@import("version").string});
    try std.testing.expect(mem.indexOf(u8, r, want) != null);
    // AND IT IS A VERSION, not an empty string wearing the shape of one.
    try std.testing.expect(@import("version").string.len > 0);
