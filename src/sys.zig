...
/// Milliseconds since this machine came up, INCLUDING TIME IT SPENT
/// ASLEEP.
///
/// FOR MEASURING DURATIONS, WHICH THE WALL CLOCK CANNOT DO. Every deadline
/// and every elapsed-time answer in this tree used `nowMillis`, and a wall
/// clock is a number a human, an admin or NTP can move. A step BACKWARD
/// extends `deadline - now` by the size of the step, re-opening the very
/// hang `readWelcomeWithin` exists to close; a step FORWARD makes `left`
/// non-positive at once and aborts a healthy handshake, or makes the link
/// read as down past its budget on the very first loss. And forward steps
/// cluster exactly where this code runs: a laptop waking and re-syncing
/// NTP is when reconnects happen ([[WI-2026-09-09-003]]).
///
/// SLEEP COUNTS, DELIBERATELY. `CLOCK_BOOTTIME` on Linux and Darwin's
/// `CLOCK_MONOTONIC` — which, unlike Linux's, goes on ticking while the
/// system is asleep — are the same clock by different names. That is the
/// semantics [[progress.Retry]] already argues for: a laptop closed for a
/// weekend should wake having spent its budget, not begin it.
///
/// NOT THE WALL CLOCK, AND NEVER MIXED WITH IT. `nowMillis` stamps the
/// account, which another process reads as a date; this measures spans
/// inside one process. A deadline taken from one and compared against the
/// other is nonsense, which is why the fallbacks below are ordered rather
/// than ending at `nowMillis`.
pub fn upMillis() i64 {
    const preferred: std.c.clockid_t = if (comptime builtin.target.os.tag.isDarwin())
        .MONOTONIC
    else
        .BOOTTIME;
    var ts: std.c.timespec = undefined;
    if (std.c.clock_gettime(preferred, &ts) == 0) return millisOf(ts);
    // A MACHINE THAT REFUSES THE FIRST STILL GETS ONE CLOCK, not a mixture
    // of two: this is chosen the same way on every call, so two readings
    // within a measurement always come from the same source.
    if (std.c.clock_gettime(.MONOTONIC, &ts) == 0) return millisOf(ts);
    return 0;
}

fn millisOf(ts: std.c.timespec) i64 {
...
/// WHY A CLAIM COULD NOT BE OPENED, WHICH IS NOT ONE ANSWER.
///
/// This used to be `?fd_t`, and every `open(2)` failure collapsed into
/// `null` — ENOENT alongside EACCES, EMFILE, ENFILE, ELOOP, EIO. The
/// caller read that single `null` as "no lock file", which is what
/// [[holder.Record.ownerGone]] then treats as a dead holder and sweeps.
///
/// SO AN FD-EXHAUSTED WORKBENCH WOULD HAVE SWEPT EVERY LIVE SESSION IT
/// HAD, and it runs out of descriptors precisely when it has the most
/// surfaces open, which is when it has the most live sessions to be wrong
/// about. [[RFC-0014]] C-LIVENESS: "THE DETERMINATION MUST NOT REPORT A
/// RUNNING HOLDER AS GONE… a test that can answer 'gone' for a holder
/// that is merely busy is inadmissible however well it behaves in the
/// ordinary case" ([[WI-2026-09-07-008]]).
pub const ClaimOpen = union(enum) {
    /// Opened. The caller owns the descriptor.
    fd: fd_t,
    /// The lock file is not there. NOTHING CLAIMS THIS NAME.
    missing,
    /// It may be there and it may not; this process could not find out.
    /// The only safe reading is that nothing is known.
    unknown,
};

pub fn openForClaim(path: []const u8) ClaimOpen {
    const path_z = posix.toPosixPath(path) catch return .unknown;
    const fd = system.open(&path_z, .{ .ACCMODE = .RDONLY }, @as(std.c.mode_t, 0));
    if (fd >= 0) return .{ .fd = @intCast(fd) };
    return if (posix.errno(fd) == .NOENT) .missing else .unknown;
}
...
pub const Pty = struct {
    ...
    /// NOTHING OPEN. What an owner carries before it has opened one and
    /// after it has closed one, so neither state is `undefined` and a
    /// second release cannot close a descriptor number the kernel has
    /// since handed to something else ([[WI-2026-09-23-001]]).
    pub const none: Pty = .{ .master = -1, .slave = -1 };
...
/// BECOME `argv`. Returns only on failure — there is no process left to
/// return into otherwise.
///
/// EXEC RATHER THAN SPAWN-AND-WAIT, where the caller is a transport. A
/// process sitting between a client and its ssh is a process the client's
/// pipes pass through, whose death the client reads as the transport's,
/// and which a kill aimed at the transport does not reach.
/// ALLOCATED, NOT BOUNDED BY A BUFFER. The first version copied each
/// argument into a fixed 256 bytes and returned silently when one did not
/// fit — and the interesting argument on this path is the script that runs
/// on the far side, which is a page and a half. The caller reported "could
/// not run the transport" and the reason was a size guard nobody could
/// see. Nothing is freed: this call either does not return or has replaced
/// the process that owned the memory.
pub fn execInto(alloc: std.mem.Allocator, argv: []const []const u8) void {
    if (argv.len == 0) return;
    const ptrs = alloc.alloc(?[*:0]const u8, argv.len + 1) catch return;
    for (argv, 0..) |a, i| {
        const z = alloc.dupeZ(u8, a) catch return;
        ptrs[i] = z.ptr;
    }
    ptrs[argv.len] = null;
    _ = execvp(ptrs[0].?, @ptrCast(ptrs.ptr));
}
...
test "a duration is measured on a clock a human cannot move" {
    // THE WALL CLOCK IS A NUMBER SOMEBODY CAN SET. Deadlines and elapsed
    // times used it, so an NTP step could abort a healthy handshake or
    // spend a reconnect's whole budget on its first loss
    // ([[WI-2026-09-09-003]]).
    const wall = nowMillis();
    const up = upMillis();
    // NOT THE EPOCH. No machine has been up since 1970, so a reading
    // anywhere near the wall clock's would mean the two are one clock.
    const a_year: i64 = 365 * 24 * 60 * 60 * 1000;
    try std.testing.expect(up < wall - a_year);
    try std.testing.expect(up > 0);

    // AND IT ADVANCES, MEASURED AGAINST THE OTHER CLOCK so that neither
    // is asked to prove itself. Spinning on the wall clock terminates
    // whatever this one does; asserting on this one then says it moved.
    // (A busy wait rather than a sleep: this file has no `Io` by design.)
    const before = upMillis();
    const until = nowMillis() + 20;
    while (nowMillis() < until) {}
    try std.testing.expect(upMillis() >= before + 10);
}
...
test "ThreadReaper joins what has finished and keeps only what is live (WI-2026-09-02-015)" {
    ...
    const deadline = upMillis() + 5_000;
    const pause: std.c.timespec = .{ .sec = 0, .nsec = 1_000_000 };
    while (upMillis() < deadline) {
        _ = reaper.reap();
