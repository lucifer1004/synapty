...
///
/// ONE OPTIONAL RATHER THAN AN ADDRESS PLUS A FLAG THAT MUST AGREE — and
/// `null` is a fact the wire could not carry before. Omitting `--hub`
/// resolves to `SYNAPTY_HUB_PORT`, then the discovery file, then 9000, so
/// the workbench leaving the flag out said exactly what passing
/// `127.0.0.1:9000` said. [[WI-2026-09-10-008]] took every OTHER holder
/// of the hub port off a `?? 9000` fallback — the wake reports, the exec
/// receipts, the tool receipts and the detector edges all genuinely stop
/// now — and left the pane half unchanged, because the omission was a
/// change to the command STRING and not to where the pane dials
/// ([[WI-2026-09-12-002]]).
///
/// 9000 is the one number that must not stand in for "no hub": it names
/// the hub an adoption may have just REFUSED. A pane pointed there
/// registers this workbench's agents into a hub of a build it may not
/// use, while the workbench tells the human A2A is unavailable.
pub const HubTarget = struct {
    addr: []const u8,
    port: u16,
};
...
pub const RunServer = struct {
    allocator: Allocator,
    agent_id: []const u8,
    /// NULL WHEN THIS PANE HAS NO HUB ([[HubTarget]]). Everything the
    /// pane is on its own — the pty, the held session, its own IPC socket
    /// — works without one; what is absent is the registration and the
    /// mail.
    hub_fd: ?sys.fd_t,
    ...
    pub fn init(
        ...
        // THE ONE PLACE THE CONVENTION IS STATED ([[ipc.pane_socket]]).
        // The creator, the sweeper and `mcp-serve`'s discoverer each held
        // their own copy of it ([[WI-2026-09-11-017]]).
        var sock_buf: [128]u8 = undefined;
        const socket_path = try allocator.dupe(u8, try ipc.pane_socket.path(&sock_buf, pid));
        ...
        //
        // NO TARGET IS NOT A FAILED DIAL ([[HubTarget]]). A pane on a
        // machine whose hub was refused holds its session and serves its
        // own socket; what it does not do is register into the hub the
        // refusal was about. There is no retry either, because nothing
        // here is waiting for that hub to appear.
        var sa: sys.sockaddr_in = undefined;
        var hub_fd: ?sys.fd_t = null;
        if (hub) |target| {
            const addr4 = std.Io.net.Ip4Address.parse(target.addr, target.port) catch {
                return error.InvalidHubAddress;
            };
            sa = sys.sockaddr_in.init(@bitCast(addr4.bytes), target.port);
            const retry_step_ms = 250;
            const max_wait_ms = 8_000;
            var waited_ms: u64 = 0;
            hub_fd = blk: while (true) {
        ...
        errdefer if (hub_fd) |fd| sys.close(fd);
    ...
    pub fn deinit(self: *RunServer) void {
        ...
        if (self.hub_fd) |fd| sys.close(fd);
    ...
    fn shutdownHubFd(self: *RunServer) void {
        ...
        if (self.hub_fd) |fd| sys.shutdown(fd, sys.SHUT.RDWR);
    }
...
fn hubReaderThread(srv: *RunServer) void {
    // A PANE WITH NO HUB HAS NOTHING TO READ ([[HubTarget]]). Returning
    // here rather than not spawning keeps the start and stop plumbing one
    // shape: the thread is joined the same way whether or not it had work
    // to do, and every failure path that joins it still does.
    if (srv.hub_fd == null) return;
    ...
    while (@atomicLoad(bool, &srv.running, .acquire)) {
        // READ FRESH EACH TIME, because `reconnectHub` swaps it under
        // `hub_write_mutex` and this loop is the sole reader.
        const fd = srv.hub_fd orelse return;
...
fn writeToHub(srv: *RunServer, data: []const u8) !void {
    ...
    // A PANE WITH NO HUB REFUSES RATHER THAN PRETENDS. The callers all
    // treat this as the failure it is, which is what makes `synapty
    // register` inside such a pane say so instead of exiting zero having
    // done nothing ([[HubTarget]]).
    const fd = srv.hub_fd orelse return error.NoHub;
        ...
        if (sys.connect(fd, &srv.hub_sa, @sizeOf(sys.sockaddr_in))) |_| {
            {
                srv.hub_write_mutex.lock(io_mod.get()) catch unreachable;
                defer srv.hub_write_mutex.unlock(io_mod.get());
                if (srv.hub_fd) |old_fd| sys.close(old_fd);
                srv.hub_fd = fd;
            }
...
fn sweepStaleSockets() void {
    sweepStaleSocketsIn(ipc.pane_socket.dir);
}
...
fn sweepStaleSocketsIn(dir_path: []const u8) void {
    ...
    while (it.next(io) catch null) |entry| {
        ...
        // READ BACK THE WAY IT WAS WRITTEN ([[ipc.pane_socket]]). This
        // sliced the digits out by two hard-coded lengths, so a prefix
        // change here would have silently stopped it recognising the
        // files the creator had just written ([[WI-2026-09-11-017]]).
        const digits = ipc.pane_socket.pidOf(entry.name) orelse continue;
