...
pub fn readWelcome(fd: sys.fd_t, buf: []u8, theirs: *u8) !Welcome {
    ...
    return welcomeFrom(frame, theirs);
}

/// THE SAME, WITH A DEADLINE — for a caller whose transport is a pipe to a
/// process that may say something that is not a frame and then go on
/// living.
///
/// A HANDSHAKE THAT CANNOT TIME OUT IS A PANE THAT CANNOT BE RECOVERED.
/// `readExactly` blocks until it has a whole frame, so a transport that
/// writes fewer bytes than a header — an ssh whose far side echoes three
/// characters from a `.bashrc` — left the client blocked in the read with
/// no retry, no detach key (the input thread is not spawned until after
/// the handshake) and no way out but closing the pane. The variant that
/// ERRORS on non-protocol bytes was found first and fixed first; this is
/// the same hang reached by writing less ([[WI-2026-09-08-001]]).
/// END OF STREAM IS ITS OWN ANSWER, and not `error.Closed`.
///
/// `welcomeFrom` below answers `error.Closed` for any frame that is not a
/// welcome — a live transport printing a MOTD reaches it just as a dead
/// one does. Collapsing the two loses the only fact that distinguishes a
/// transport that has FINISHED WRITING and is on its way out from one that
/// is alive and not speaking the protocol, which is exactly what
/// [[commands.transportVerdict]] has to decide ([[WI-2026-09-09-003]]).
pub fn readWelcomeWithin(fd: sys.fd_t, buf: []u8, theirs: *u8, ms: i32) !Welcome {
    const frame = (try readFrameWithin(fd, buf, ms)) orelse return error.EndOfStream;
    return welcomeFrom(frame, theirs);
}

/// The parse, with no I/O in it, so the two readers above cannot come to
/// disagree about what a welcome is.
fn welcomeFrom(frame: anytype, theirs: *u8) !Welcome {
...
/// ONE WRITER PER FD. This is a header write and then a payload write,
/// so a socket with two threads writing to it needs [[FrameWriter]]
/// instead — every fd in this codebase with more than one writer has one
/// ([[WI-2026-09-12-001]]).
...
/// A STOP FLAG WRITTEN BY ONE THREAD AND READ IN ANOTHER'S LOOP.
///
/// PLAIN LOADS GET HOISTED OUT OF LOOPS, and this codebase has the bug to
/// prove it: `end` reported success, the reply went out, and the session
/// ran on, because the loop testing the flag had cached it. Both of this
/// holder's flags carry that scar, and so do the three in the CLI's
/// client loops — which survive only because each loop body contains an
/// opaque call (`sys.waitReadable`, `sys.read`) the optimiser cannot see
/// through. That is a property of the loop BODIES rather than of the
/// code's intent: one refactor that inlines a poll changes it.
///
/// ONE TYPE RATHER THAN TWO SPELLINGS. The holder wrote this as raw
/// `@atomicLoad`/`@atomicStore` beside its fields and the CLI as a struct
/// of its own, with the reasoning above copied into both — so a third
/// flag would be written whichever way its author happened to read first
/// ([[WI-2026-09-15-001]]).
pub const Flag = struct {
    /// NO DEFAULT, AND THAT IS THE POINT. The two users start from
    /// opposite states — a holder is not running until `start`, a client
    /// loop is running from the moment its state exists — and unifying
    /// them under one default silently stopped every client loop before
    /// its first read. The holder e2e caught it on the first run; a
    /// declaration that must say which way it starts cannot be got wrong
    /// that way twice ([[WI-2026-09-15-001]]).
    value: bool,

    pub fn get(self: *const Flag) bool {
        return @atomicLoad(bool, &self.value, .acquire);
    }

    pub fn set(self: *Flag, v: bool) void {
        @atomicStore(bool, &self.value, v, .release);
    }
};

/// A SOCKET THAT ONLY EVER CARRIES WHOLE FRAMES, however many threads are
/// writing to it.
///
/// `writeFrame` is a header write and then a payload write, and
/// `sys.writeAll` loops on short writes — so two threads sharing a socket
/// can each land bytes inside the other's frame. `readFrame` takes the
/// next five bytes as a header, so the reader then takes a length out of
/// somebody else's payload: it refuses the frame and drops the session,
/// or hands the child bytes that were never typed.
///
/// SERIALISING THE WRITERS IS THE FIX, not buffering the frame into one
/// call. A single `writeAll` still loops when the socket buffer is full,
/// and a partial write is exactly the gap this closes.
///
/// ONE PER SOCKET WITH MORE THAN ONE WRITER, rather than one for the
/// process: a write is bounded by the send timeout it already carries,
/// and a wedged client must not be able to hold up an answer on an
/// unrelated connection.
pub const FrameWriter = struct {
    mutex: std.Io.Mutex = .init,

    pub fn write(self: *FrameWriter, fd: sys.fd_t, kind: Frame, payload: []const u8) !void {
        try self.mutex.lock(io_mod.get());
        defer self.mutex.unlock(io_mod.get());
        try writeFrame(fd, kind, payload);
    }
};
...
/// Read one frame, giving the whole of it a deadline.
pub fn readFrameWithin(fd: sys.fd_t, buf: []u8, ms: i32) !?struct { kind: Frame, payload: []const u8 } {
    // ON THE CLOCK THAT MEASURES SPANS ([[sys.upMillis]]): a wall-clock
    // step backward would extend this deadline by the size of the step,
    // which is the hang it exists to close.
    const deadline = sys.upMillis() + ms;
    var hdr: [header_len]u8 = undefined;
    if (!try readExactlyWithin(fd, &hdr, deadline)) return null;
    const len = std.mem.readInt(u32, hdr[1..5], .little);
    if (len > buf.len) return error.TooLarge;
    if (len > 0 and !try readExactlyWithin(fd, buf[0..len], deadline)) return null;
    return .{ .kind = @enumFromInt(hdr[0]), .payload = buf[0..len] };
}

/// `readExactly` against a wall clock. A writer that sends part of a frame
/// and stops is the case this exists for: the read itself never returns,
/// because the pipe is open and the writer is alive.
fn readExactlyWithin(fd: sys.fd_t, buf: []u8, deadline_ms: i64) !bool {
    var got: usize = 0;
    while (got < buf.len) {
        const left = deadline_ms - sys.upMillis();
        if (left <= 0) return if (got > 0) error.PartialFrame else error.Timeout;
        const ready = sys.waitReadable(fd, @intCast(@min(left, 1000))) catch
            return if (got > 0) error.PartialFrame else error.Closed;
        if (!ready) continue;
...
pub const Retained = struct {
    ...
    pub fn deinit(self: *Retained, allocator: std.mem.Allocator) void {
        ...
        // EMPTY AFTERWARDS, so the `buf.len > 0` that guards a release
        // means what it says. It did not: the slice kept its length after
        // the free, and a second release freed it again
        // ([[WI-2026-09-23-001]]).
        self.buf = &.{};
    }
...
pub const ClientEntry = struct {
    ...
    /// NO DEFAULTS. This is built in exactly one place and that place
    /// always has the size from the hello, so a default here was two more
    /// copies of a number that lives in [[protocol.assumed_rows]] and
    /// could never be read ([[WI-2026-09-15-001]]).
    rows: u16,
    cols: u16,
};
...
/// could have found it.
/// WHAT MAY BE A SESSION'S NAME.
///
/// A NAME IS A NAME AND NOT A PATH. Every file a session has is built by
/// interpolating this into a directory — `{dir}/{name}.sock`, `.lock`,
/// `.json` — so a name carrying a separator is a name carrying a
/// DESTINATION. `--id ../../escaped` put the socket two directories above
/// the sessions directory and outside it altogether, which is outside the
/// 0700 that [[RFC-0014]] C-ENTITLEMENT requires of it: "Enumeration …
/// only its owner may enter it". Nothing validated this; there was no
/// check on `--id` anywhere in the tree ([[WI-2026-09-08-008]]).
///
/// AND IT SPLIT THE NAME IN TWO. `nameOf` takes the BASENAME of the socket
/// path, so `a/b` served a socket at `sessions/a/b.sock` while claiming
/// and recording under `b`. The guard asked about one name and the claim
/// took another — which the claim-before-destroy ordering caught, but only
/// by refusing a start whose socket path was never the one in question.
///
/// REFUSED RATHER THAN SANITISED. A name quietly rewritten is a session
/// the human cannot find under the name they gave it, and two names that
/// sanitise alike would collide silently.
pub fn validName(name: []const u8) bool {
    if (name.len == 0 or name.len > max_name_len) return false;
    if (std.mem.eql(u8, name, ".") or std.mem.eql(u8, name, "..")) return false;
    for (name) |c| {
        // A separator makes it a path; a NUL truncates it for the kernel;
        // a control character is not something a human typed on purpose
        // and is not something a listing can print.
        if (c == '/' or c == 0 or c < 0x20 or c == 0x7f) return false;
        // AND ASCII, BECAUSE THE FOLD IS BYTE-WISE AND UNICODE IS NOT.
        //
        // `canonical` maps bytes in 'A'...'Z'. That is not stable across
        // Unicode normalisation: `Ü` precomposed is C3 9C and survives it,
        // while the SAME character decomposed is 55 CC 88 — whose leading
        // byte is 'U', so it folds to `ü` and the two spellings become two
        // sessions a human cannot tell apart. Measured across the
        // boundary: Foundation converts `Process` arguments through the
        // file-system representation, which on Darwin normalises to NFD,
        // so the workbench held one spelling and the holder received the
        // other — the holder wrote `übercase.json` for a name this side
        // called `Übercase` ([[WI-2026-09-12-002]]).
        //
        // UNREPRESENTABLE RATHER THAN MIRRORED. The alternative is a
        // workbench that reproduces Foundation's normalisation before
        // folding, which fixes one launch path and not the pane's, and
        // leaves `--id Ü` typed at two shells still meaning two sessions.
        // "One string is one identifier" ([[WI-2026-09-09-001]]) is the
        // rule, and a byte-wise fold can only keep it over ASCII.
        if (c >= 0x80) return false;
    }
    return true;
}

/// THE CEILING IS THE KERNEL'S, NOT A ROUND NUMBER.
///
/// A socket whose full path is too long falls back to binding by BASENAME
/// from inside its directory ([[socketCall]]), and that basename is
/// `{name}.sock`. `sun_path` is 104 bytes, so the name has 104 − 5 for
/// `.sock` − 1 for the NUL: ninety-eight.
///
/// IT WAS 128, WHICH PROMISED THIRTY BYTES THAT DO NOT WORK. Measured, one
/// start per length: 98 starts, 99 and above fail — and fail late, in
/// `Holder.init`, with "the session did not come up" and no reason. A
/// validator that accepts what the next layer refuses has moved the error
/// rather than prevented it ([[WI-2026-09-08-013]]).
pub const max_name_len: usize = 98;

/// THE ONE SPELLING A NAME HAS.
///
/// `Foo` AND `foo` WERE ONE SESSION ON A MAC AND TWO ON A LINUX HOST,
/// because the default APFS volume is case-insensitive and ext4 is not.
/// For a tool whose whole point is spanning machines, a semantic that
/// differs by filesystem is not a semantic ([[WI-2026-09-08-014]]).
///
/// THIS SANITISES WHERE [[validName]] REFUSES, AND THE DIFFERENCE IS
/// PREDICTABILITY. A name carrying a slash is rewritten arbitrarily by any
/// sanitiser you could pick, and the human cannot guess which — so it is
/// refused. Case has exactly one obvious folding, and the collision it
/// creates is the collision we WANT: `Foo` and `foo` are meant to be one
/// session. The rule stays "refuse what has no single right answer".
///
/// ASCII ONLY, AND THAT IS A LIMIT AND NOT A CHOICE. Folding `İ` or `ẞ`
/// correctly is locale-dependent, and deciding whether `é` and `é` are one
/// name is Unicode normalisation — for which Zig's standard library has no
/// tables and this project has no room for a dependency ([[CLAUDE.md]]:
/// libghostty and zig-clap, nothing else at runtime). So a non-ASCII name
/// still means one session on a Mac and two on Linux. Said out loud rather
/// than left to be discovered.
///
/// THE RESULT BORROWS `buf` and lives exactly as long as it does. This is
/// `pub`, so that is a promise to callers rather than a property of the
/// current ones: a caller that lets the returned slice outlive the array
/// it passed reads a name that is no longer there.
pub fn canonical(buf: []u8, name: []const u8) ?[]const u8 {
...
pub fn socketPath(buf: []u8, agent_id: []const u8) ![]const u8 {
    ...
    const name = canonical(&nbuf, agent_id) orelse return error.InvalidSessionName;
    var dir: [1024]u8 = undefined;
    const d = paths.sessionsDir(&dir) orelse return error.NoStateDirectory;
    return std.fmt.bufPrint(buf, "{s}/{s}.sock", .{ d, name });
}
...
pub const Claim = enum {
    ...
    free,
    /// No lock file. NOTHING CLAIMS THIS NAME — it was never claimed, or
    /// it was swept.
    absent,
    /// THE QUESTION COULD NOT BE ASKED, which is not the same as an
    /// answer and must never be read as one. A descriptor limit, a
    /// permission, an I/O error, a state directory that would not
    /// resolve: all of them used to arrive here as `absent`, and `absent`
    /// is what sweeps a record ([[WI-2026-09-07-008]]).
    unknown,
};
...
///
/// AND THE ASYMMETRY HERE RUNS OPPOSITE TO [[Record.ownerGone]]'s, which
/// is why the fourth claim state had to be answered twice rather than
/// once. There, `unknown` must not be read as "gone", because sweeping a
/// live session's record loses a session. Here, `unknown` must be read as
/// "would join", because the alternative is worse in kind: refusing a
/// start is recoverable in one command, and what an admitted start does is
/// not recoverable at all.
///
/// WHAT IT DOES IS UNLINK THE LIVE SESSION'S SOCKET. [[Holder.init]]
/// removes the path and binds its own before anything claims the name, so
/// a start let through by mistake strands a running holder — alive, on a
/// pty, with its child, and permanently unreachable, while `sessions`
/// shows one healthy row describing the impostor. `claimState == .held`
/// let `unknown` fall silently into "proceed", and an EACCES on the lock
/// reproduced it ([[WI-2026-09-08-005]]).
pub fn startWouldJoin(name: []const u8) bool {
    return switch (claimState(name)) {
        .held, .unknown => true,
        .free, .absent => false,
    };
}
...
pub fn claimState(name: []const u8) Claim {
    ...
    // A STATE DIRECTORY THAT WILL NOT RESOLVE IS NOT AN EMPTY ONE. This
    // answered `absent`, so one unresolvable path made EVERY session on
    // the machine read as gone, in one call.
    const path = Record.lockPathFor(&buf, name) orelse return .unknown;
    const opened = sys.openForClaim(path);
    const fd = switch (opened) {
        .fd => |f| f,
...
pub const Record = struct {
    ...
    pub fn rewrite(name: []const u8, session_name: []const u8) void {
        ...
        // SAID, NOT SWALLOWED. This one cannot refuse the way `describe`
        // does — the session is already running and the name is already
        // set in memory, so it answers to it either way. What is lost is
        // the copy `sessions` reads for a holder it cannot reach, and a
        // rename that did not stick is worth a line rather than nothing
        // ([[WI-2026-09-09-003]]).
        writeBody(path, body[0..at]) catch |err| {
            log.err("could not record the session name: {s}", .{@errorName(err)});
        };
    }

    pub fn pathFor(buf: []u8, name: []const u8) ?[]const u8 {
        // THE SAME GATE AND THE SAME FOLDING AS THE SOCKET'S. These three
        // paths are built the same way from the same string, so a name
        // that may not be a socket may not be a record either, and one
        // that folds must fold identically or a session's three files stop
        // being one session's ([[canonical]]).
        var nbuf: [max_name_len]u8 = undefined;
        const folded = canonical(&nbuf, name) orelse return null;
        var dir: [1024]u8 = undefined;
        const d = paths.sessionsDir(&dir) orelse return null;
        return std.fmt.bufPrint(buf, "{s}/{s}.json", .{ d, folded }) catch null;
    }
    ...
    pub fn lockPathFor(buf: []u8, name: []const u8) ?[]const u8 {
        ...
        var dir: [1024]u8 = undefined;
        const d = paths.sessionsDir(&dir) orelse return null;
        return std.fmt.bufPrint(buf, "{s}/{s}.lock", .{ d, folded }) catch null;
    }
    ...
    /// TAKE THE NAME, AND NOTHING ELSE.
    ///
    /// SEPARATE FROM DESCRIBING IT so that a holder can take the name
    /// BEFORE it destroys anything. [[Holder.init]] unlinks the session's
    /// socket path and binds its own; that used to happen some seventy
    /// lines before the claim was attempted, so a start let through by a
    /// mistaken guard took a live session's socket and only afterwards
    /// discovered it could not have the name — leaving the original holder
    /// alive, on its pty, and permanently unreachable
    /// ([[WI-2026-09-08-005]], [[WI-2026-09-08-006]]).
    ///
    /// A GUARD IS A SECOND OPINION AND THIS IS THE FIRST. `startWouldJoin`
    /// asks whether a name is taken and can be wrong — it was, about
    /// `unknown`. This cannot: it is the claim itself, and the kernel
    /// answers it.
    ///
    /// The returned descriptor must stay open for as long as the session
    /// does: the kernel releasing it is how anything else learns the
    /// holder is gone, whatever killed it ([[sweepEnded]]).
    ///
    /// THE CLAIM IS ON A FILE OF ITS OWN ([[claimState]]). It is created
    ...
    /// not replaced — because an flock lives on an inode and a claim
    /// sharing a file with data is released by every writer of that data.
    /// HAND A CLAIM BACK. THE FILE STAYS.
    ///
    /// IT USED TO UNLINK, AND THAT PUT THE ORIGINAL DAMAGE BACK. A claim
    /// is an flock, and an flock lives on an INODE. A start that has
    /// opened the lock file but not yet locked it, crossed by an unlink of
    /// that path, takes its claim on an orphan: nothing names that inode
    /// any more, so a LATER start creates a fresh file, locks that, and
    /// both processes believe they hold the name — whereupon the second
    /// one's `Holder.init` unlinks the first's live socket. That is
    /// exactly the loss `claim` exists to prevent, re-entered through the
    /// cleanup path ([[WI-2026-09-09-001]]).
    ///
    /// So the identity of a claim is the PATH, and the cost is one empty
    /// file per name ever used — which is what a lock file is. The leak
    /// this used to fix ([[WI-2026-09-08-013]]) was a leak of files, not
    /// of meaning: a lock with no record and no socket beside it is
    /// invisible to `sessions` (which enumerates `.json`) and reads `free`
    /// to everything else, and `.free` and `.absent` are one answer to
    /// every caller there is — `ownerGone` and `startWouldJoin` each group
    /// them together.
    ///
    /// ONLY FOR A CLAIM THIS PROCESS HOLDS. `claim`'s own refusal path
    /// must not call this: the lock it could not take belongs to somebody
    /// who is using it.
    pub fn releaseClaim(fd: sys.fd_t) void {
        sys.close(fd);
    }

    pub fn claim(name: []const u8) ?sys.fd_t {
        var dir: [1024]u8 = undefined;
    ...
    /// Describe the session this process has already claimed.
    ///
    /// LOCK BEFORE RECORD, WHICH IS WHY THESE ARE TWO CALLS AND IN THIS
    /// ORDER. Between them there is a moment with a claim and no record,
    /// and that shape is inert: `sweepEnded` needs a record it can read
    /// before it will remove anything. The reverse order would leave a
    /// readable record with no claim beside it — indistinguishable from a
    /// tombstone, and swept as one.
    ///
    /// AND IT IS WRITTEN LATE ON PURPOSE, once the child is on the
    /// terminal, so a record never names a session that failed to start.
    /// Splitting the claim out did not move this; it moved only the taking
    /// of the name, which is the half that has to happen before any
    /// damage.
    /// AND IT SAYS SO WHEN IT CANNOT. This returned `void`, and its only
    /// failure mode was silence: `pathFor` answering null, or `createFile`
    /// meeting ENOSPC, EDQUOT or EACCES, each ended in a bare `return`.
    /// What that leaves is a claim held, a socket bound and NO RECORD — a
    /// session that is running and attachable and that `sessions` cannot
    /// list (it enumerates `.json`) and `sweepEnded` cannot remove (it
    /// needs a record it can read). A moment of full disk made a live
    /// session vanish from the only surface that could find it, for as
    /// long as the machine lasted ([[WI-2026-09-09-003]]).
    ///
    /// The caller refuses the start on this, which unwinds the claim and
    /// the socket with it ([[Holder.start]]).
    pub fn describe(name: []const u8) !void {
        var buf: [1024]u8 = undefined;
        const path = pathFor(&buf, name) orelse return error.NoStateDirectory;
        var body: [128]u8 = undefined;
        const text = try std.fmt.bufPrint(&body, "{{\"pid\":{d}}}\n", .{std.c.getpid()});
        try writeBody(path, text);
    }
    ...
    ///
    /// NOT HELD, RATHER THAN `free`. Both answers that are not `held` mean
    /// the owner is gone, and this asked for one of them — so a record
    /// beside an ABSENT lock was neither live nor sweepable, a row that
    /// could not be returned to and could not be removed, for as long as
    /// the machine lasted. A state that is neither is not a state
    /// ([[WI-2026-09-07-005]]).
    ///
    /// AND `absent` CANNOT BE A SESSION BEING BORN, which is what the
    /// previous rule was protecting and what the comment beside it
    /// asserted. [[Record.claim]] creates the lock and takes it before it
    /// writes the record, so a record exists only where a lock did. The
    /// ordering that IS real is the socket's — a holder binds before it
    /// writes its record — and [[sweepEnded]] respects it by refusing to
    /// touch anything without a record to read.
    ///
    /// The two orderings were conflated, in this comment and in the
    /// workbench's copy of it, and neither was checked against the write.
    pub fn ownerGone(name: []const u8) bool {
        return switch (claimState(name)) {
            // The lock was taken and released, or was never taken: either
            // way nothing holds this name.
            .free, .absent => true,
            // A live holder, or a question that could not be asked. The
            // second is not evidence of anything, and treating it as
            // evidence of DEATH is the inadmissible direction
            // ([[RFC-0014]] C-LIVENESS, [[WI-2026-09-07-008]]).
            .held, .unknown => false,
        };
    }

    /// THE RECORD GOES; THE CLAIM STAYS.
    ///
    /// It used to take the lock file too, on the reasoning that a lock
    /// left behind is a name that reads as a tombstone forever — `free`,
    /// never `absent`. That distinction no longer exists: `ownerGone` and
    /// `startWouldJoin` each treat `.free` and `.absent` as one answer, so
    /// the lock file decides nothing. What unlinking it DID decide was
    /// which inode a concurrent start would lock, and that is how a sweep
    /// could hand one name to two holders ([[releaseClaim]],
    /// [[WI-2026-09-09-001]]).
...
/// A session's name, taken back out of the socket path it is served on —
/// the name is what a client knows it by.
///
/// FREE OF THE HOLDER, because `init` has to name the session before there
/// is a holder to ask: the claim is taken before anything is destroyed
/// ([[WI-2026-09-08-006]]).
pub fn nameOf(socket_path: []const u8) []const u8 {
...
pub const Holder = struct {
    ...
    /// WHOLE FRAMES ONTO THE CLIENT'S SOCKET ([[FrameWriter]]). The
    /// counter above answers a different question — it stops the fd being
    /// CLOSED under a writer — and nothing ordered the writers against
    /// each other. Four threads reach a client fd: the pump, the
    /// handshake's restoration, the child reaper's `.exit`, and the next
    /// attach's `.displaced` to the client it takes the seat from.
    ///
    /// `catching_up` already serialises exactly one of those pairs, and
    /// that is the shape of the bug: the pump is held off while `catchUp`
    /// writes, and then `attach` publishes the fd with `catching_up ==
    /// false` BEFORE the restoration is written, so the largest write of
    /// all had nothing holding the pump off it.
    ///
    /// COVERS THE DISPLACED CLIENT'S FD TOO. That is a different socket,
    /// so this serialises a little more than it must; both writes are
    /// bounded by the send timeout, and a lifetime rule that depends on
    /// which fd you are holding is one every future caller has to be
    /// told.
    client_writer: FrameWriter = .{},
    ...
    name_buf: [label_max]u8 = undefined,
    name_len: u8 = 0,
    /// Written by whoever calls stop() and read by three threads whose
    /// loops are otherwise free to cache it ([[Flag]]). A join against a
    /// thread that never observed the change does not return, and a
    /// holder that cannot finish stopping keeps answering — which is a
    /// session a human has ended and can still see.
    running: Flag = .{ .value = false },
    ...
    /// to kill the child. Set on the accept thread and read by the thread
    /// that owns the holder's lifetime ([[Flag]]).
    ending: Flag = .{ .value = false },
    ...
    pub fn init(allocator: std.mem.Allocator, socket_path: []const u8) !Holder {
        ...
        // THE NAME IS TAKEN BEFORE ANYTHING IS DESTROYED, and that
        // ordering is the whole safety of this function.
        //
        // The unlink below removes whatever is at the socket path. It used
        // to happen first, with the claim attempted some seventy lines
        // later in `start`, after the pty child and three threads — so a
        // start let through by a mistaken guard took a LIVE session's
        // socket and only afterwards discovered it could not have the
        // name. The original holder stayed alive on its pty, with its
        // child, and permanently unreachable, because every client
        // reaching that path found the impostor; and `sessions` showed one
        // healthy row describing the impostor, so nothing told the human a
        // session had been stranded ([[WI-2026-09-08-005]]).
        //
        // A GUARD IS A SECOND OPINION AND THIS IS THE FIRST.
        // `startWouldJoin` asks whether a name is taken and can be wrong —
        // it was, about `unknown`, which is what made the damage above
        // reachable. This cannot be wrong: it IS the claim, and the kernel
        // answers it. So the guard is now an early, courteous refusal and
        // no longer the only thing between a mistake and a stranded
        // session ([[WI-2026-09-08-006]]).
        const claimed = Record.claim(nameOf(socket_path)) orelse return error.SessionNameHeld;
        // THE FILE GOES BACK TOO, not just the descriptor. Closing alone
        // left a lock nothing could see and nothing could sweep, one per
        // failed start ([[Record.releaseClaim]]).
        errdefer Record.releaseClaim(claimed);
    ...
    pub fn start(
        ...
        // NO ERRDEFERS IN HERE, AND THAT IS THE POINT ([[WI-2026-09-23-001]]).
        // Everything this acquires is released by `stop`, which the caller
        // already defers on failure — the sentence at the end of this
        // function says so. These used to be released here as well, which
        // made two owners of one set of resources: on a failure after the
        // threads below had started, this freed the retention buffer and
        // closed the pty while the pump was still using both, and then
        // `stop` joined the pump and freed and closed them a second time.
        // One owner, which joins before it frees, cannot do either.
        ...
        // names a session that failed to start. THE CLAIM IS ALREADY
        // HELD — `init` took it before it destroyed anything — and this is
        // only the describing ([[Record.claim]], [[Record.describe]]).
        //
        // AND A SESSION THAT CANNOT BE RECORDED IS NOT STARTED. It would
        // otherwise run, and be attachable, and be invisible to `sessions`
        // and unreachable by `sweepEnded` — a state nothing can find and
        // nothing can clear. Failing here unwinds the claim, the socket
        // and the pty through the caller's `errdefer`, so a full disk
        // costs an error message rather than a permanent orphan.
        try Record.describe(self.name());
    }
    ...
    pub fn name(self: *const Holder) []const u8 {
        return nameOf(self.socket_path);
    }

    pub fn stop(self: *Holder) void {
        // ONCE. The one caller holds it in an `errdefer` AND a `defer`, so
        // anything that fails after `start` succeeded runs both — and a
        // second pass joins threads that were already joined, closes a
        // listener whose number may belong to somebody else by now, and
        // frees the screen and the client threads twice
        // ([[WI-2026-09-23-001]]).
    ...
    fn retainAndDeliver(self: *Holder, bytes: []const u8) void {
        ...
        defer self.endWrite();
        self.client_writer.write(fd, .data, bytes) catch {
    ...
    fn sendRestoration(self: *Holder, fd: sys.fd_t, rows: u16, cols: u16) !void {
        ...
        try self.client_writer.write(fd, .restore, payload);
    }
    ...
    fn catchUp(self: *Holder, fd: sys.fd_t, gen: u64, from: u64) void {
        ...
        while (self.isCurrent(gen)) {
            ...
            if (!self.retained.has(at)) {
                ...
                self.client_writer.write(fd, .gap, &payload) catch {};
                return;
            }
            ...
            self.mutex.unlock(io_mod.get());
            self.client_writer.write(fd, .data, buf[0..n]) catch {
                self.detachClient(fd);
                sys.shutdown(fd, sys.SHUT.RDWR);
                self.mutex.lock(io_mod.get()) catch return;
                self.catching_up = false;
    ...
    fn unattachedFor(self: *Holder) u64 {
        ...
        if (since == 0) return 0;
        const elapsed = sys.upMillis() - since;
    ...
    fn attach(self: *Holder, fd: sys.fd_t, catching_up: bool, label: []const u8) u64 {
        ...
        if (previous) |old| {
            ...
            self.client_writer.write(old, .displaced, label[0..kept]) catch {};
    ...
    fn detachClient(self: *Holder, fd: ?sys.fd_t) void {
        ...
        if (self.client) |current| {
            if (fd == null or current == fd.?) {
                ...
                self.unattached_since = sys.upMillis();
            }
        }
    }
    ...
    fn deliver(self: *Holder, kind: Frame, payload: []const u8) void {
        ...
        defer self.endWrite();
        self.client_writer.write(target, kind, payload) catch {
...
const TestClient = struct {
    ...
    fn connectLabelled(path: []const u8, rows: u16, cols: u16, label: []const u8) !TestClient {
        ...
        // THROUGH THE ONE READER, like every shipping client
        // ([[WI-2026-09-11-002]]). This decoded the welcome by hand and
        // never looked at `payload[0]` — which is exactly the defect
        // `welcomeFrom` was extracted to make unrepresentable, and this
        // is the client EVERY holder test establishes a session with. A
        // change that made the real clients refuse a version mismatch
        // would have left the suite green.
        var buf: [64]u8 = undefined;
        var theirs: u8 = 0;
        const w = try welcomeFrom(
            (try readFrame(fd, &buf)) orelse return error.Closed, &theirs);
        return .{
            .fd = fd,
            .answer = w.answer,
...
test "a start that fails with its threads running is unwound once, joining before it frees" {
    // THE PATH `start` NAMES IN ITS OWN LAST COMMENT: the record cannot
    // be written — a full disk — after the child and all three threads
    // are up. Forced here by putting a DIRECTORY where the record goes,
    // so creating the file fails and nothing else does
    // ([[WI-2026-09-23-001]]).
    //
    // WHAT FAILED, AND HOW THIS SEES IT. `start` freed the retention
    // buffer and closed the pty on its way out while the pump was still
    // running on both, and then the caller's `stop` joined the pump and
    // freed and closed them again. The testing allocator reports the
    // second free; nothing else in this file could.
    var rbuf: [128]u8 = undefined;
    const root = try std.fmt.bufPrint(&rbuf, "/tmp/synapty-holder-unwind-{d}", .{std.c.getpid()});
    paths.root_override = root;
    defer paths.root_override = null;
    defer std.Io.Dir.cwd().deleteTree(io_mod.get(), root) catch {};

    var pbuf: [128]u8 = undefined;
    const path = try testSocketPath(&pbuf, "unwind");
    var h = try Holder.init(testing.allocator, path);

    var dbuf: [1024]u8 = undefined;
    const record = Record.pathFor(&dbuf, h.name()) orelse return error.NoRecordPath;
    try std.Io.Dir.cwd().createDirPath(io_mod.get(), record);

    if (h.start(echo_shell, &.{}, .{ .ws_row = 24, .ws_col = 80 })) |_| {
        return error.StartShouldHaveFailed;
    } else |_| {}

    // EXACTLY AS THE ONE CALLER HAS IT: the `errdefer` that answers a
    // failed start, and — for anything failing after a start that
    // succeeded — the `defer` beside it as well.
    h.endChild();
    h.stop();
    h.stop();
}
...
test "the directory holding sessions is owner-only, and so is a record" {
    ...
    const held = claimLiveForTest(name);
...
test "a record whose process is gone is swept, not kept" {
    ...
    const claim = claimLiveForTest("still-here");
    ...
    try testing.expect(claimLiveForTest("still-here") == null);
    ...
    // A RECORD WITH NO LOCK AT ALL IS STILL A TOMBSTONE, and this is the
    // case that hung a row on a real machine for six days: a holder from
    // before the claim moved to its own file left a record nothing could
    // sweep and nothing could return to, because `absent` satisfied
    // neither test ([[WI-2026-09-07-005]]).
    //
    // IT CANNOT BE A SESSION BEING BORN. [[Record.claim]] takes the lock
    // BEFORE it writes the record, so a record exists only where a lock
    // did; the ordering that would make this dangerous is the SOCKET's,
    // and `sweepEnded` already refuses anything it cannot read a record
    // for.
    try writeRecordForTest("no-lock", 0x7FFF_FFFF);
    var nolock: [1024]u8 = undefined;
    if (Record.lockPathFor(&nolock, "no-lock")) |lp| sys.unlink(lp);
    try testing.expect(claimState("no-lock") == .absent);
    try testing.expect(sweepEnded(testing.allocator, "no-lock"));
    try testing.expect(Record.read(testing.allocator, "no-lock") == null);

    // A CLAIM THAT COULD NOT BE ASKED IS NOT A CLAIM THAT SAID NO.
    //
    // Every `open(2)` failure used to arrive as `absent`, and `absent` is
    // what sweeps — so a workbench out of descriptors, or one that could
    // not resolve its state directory, would have deleted the record of
    // every live session it held. [[RFC-0014]] C-LIVENESS calls a test
    // that can answer "gone" for a holder that is merely busy
    // inadmissible "however well it behaves in the ordinary case"
    // ([[WI-2026-09-07-008]]).
    //
    // A SYMLINK LOOP IS THE ERROR THAT IS PORTABLE TO PROVOKE. ELOOP is
    // not the interesting one — EMFILE is — but the code cannot tell them
    // apart and must not: everything that is not ENOENT is the same
    // answer, which is no answer.
    try writeRecordForTest("cannot-ask", 0x7FFF_FFFF);
    var lbuf: [1024]u8 = undefined;
    const lock = Record.lockPathFor(&lbuf, "cannot-ask") orelse return error.NoStateDirectory;
    sys.unlink(lock);
    {
        var loop_buf: [1024]u8 = undefined;
        const other = try std.fmt.bufPrint(&loop_buf, "{s}.loop", .{lock});
        try std.Io.Dir.cwd().symLink(io_mod.get(), other, lock, .{});
        try std.Io.Dir.cwd().symLink(io_mod.get(), lock, other, .{});
    }
    try testing.expectEqual(Claim.unknown, claimState("cannot-ask"));
    try testing.expect(!Record.ownerGone("cannot-ask"));
    try testing.expect(!sweepEnded(testing.allocator, "cannot-ask"));
    try testing.expect(Record.read(testing.allocator, "cannot-ask") != null);
...
test "a record replaced under a live holder is not a tombstone" {
    ...
    const claim = claimLiveForTest("swapped");
    defer if (claim) |fd| sys.close(fd);
...
/// A record naming a pid of the test's choosing. `Record.claim` names the
/// process that calls it, which is by definition alive.
/// A live session, as a holder makes one: the name taken, then described.
/// The two are separate calls in production so the name can be taken
/// BEFORE anything is destroyed ([[Holder.init]]); a test that wants the
/// finished shape asks for both.
fn claimLiveForTest(name: []const u8) ?sys.fd_t {
    const fd = Record.claim(name) orelse return null;
    Record.describe(name) catch return null;
    return fd;
}
...
fn writeRecordForTest(name: []const u8, pid: i32) !void {
    ...
    // record with NO lock reads `absent` instead, which is a different
    // answer and — since [[Record.ownerGone]] stopped asking for `free`
    // specifically — no longer a different outcome. The case has its own
    // test below.
    var lbuf: [1024]u8 = undefined;
...
test "the claim exists before the record does, and a claim alone is inert" {
    // THE PREMISE THE WHOLE SWEEP RESTS ON, ASSERTED IN PROSE THREE TIMES
    // AND IN NO TEST.
    //
    // `Record.ownerGone` treats a record beside an absent lock as a
    // tombstone, and `SessionRecord.live()` sweeps on that answer. Both
    // are safe ONLY because a record cannot exist where a lock did not:
    // [[Holder.init]] claims before it does anything else, and
    // [[Record.describe]] runs later, once the child is on the terminal.
    // Three comments say so — here, in the workbench's copy, and in the
    // test beside this one — and nothing checked it.
    //
    // THAT IS EXACTLY HOW THE PREVIOUS WRONG PREMISE SURVIVED. "A record
    // with no lock is a session still starting up" was also written in two
    // places and verified in none, and it was false
    // ([[WI-2026-09-07-005]]). Three comments agreeing is one claim
    // written three times ([[WI-2026-09-08-012]]).
    var root: [128]u8 = undefined;
    const r = try std.fmt.bufPrint(&root, "/tmp/synapty-order-{d}", .{std.c.getpid()});
    std.Io.Dir.cwd().createDirPath(io_mod.get(), r) catch {};
    defer std.Io.Dir.cwd().deleteTree(io_mod.get(), r) catch {};
    paths.root_override = r;
    defer paths.root_override = null;
    // THE DIRECTORY EXISTS FIRST, because on any machine that has ever
    // run a session it does — and without it `describe` fails silently,
    // which masked a reversed ordering and left this test green against
    // the very defect it is for.
    var dbuf: [1024]u8 = undefined;
    if (paths.sessionsDir(&dbuf)) |d| makeSessionsDir(d);

    // THROUGH `Holder.init`, WHICH IS WHERE THE ORDERING NOW LIVES. A
    // first draft of this called `claim` and `describe` directly, in the
    // right order, and so pinned its own call and not the holder's:
    // reversing `init` left it green. The assertion has to be made on the
    // object that decides.
    var pbuf: [256]u8 = undefined;
    const path = try testSocketPath(&pbuf, "ordering");
    var h = try Holder.init(testing.allocator, path);
    defer h.stop();
    const nm = nameOf(path);

    // INIT HAS CLAIMED AND NOTHING HAS DESCRIBED. `start` writes the
    // record, once the child is on the terminal, and it has not run.
    try testing.expectEqual(Claim.held, claimState(nm));
    try testing.expect(Record.read(testing.allocator, nm) == null);

    // AND THE WINDOW IS INERT. `sweepEnded` needs a record it can read
    // before it removes anything, so a session being born cannot be
    // swept — which is the property the reverse ordering destroys, and
    // the whole reason the sweep may treat an absent lock as a tombstone.
    try testing.expect(!sweepEnded(testing.allocator, nm));
    try testing.expectEqual(Claim.held, claimState(nm));

    // AND ONCE DESCRIBED: both, and still held.
    try Record.describe(nm);
    try testing.expect(Record.read(testing.allocator, nm) != null);
    try testing.expect(!sweepEnded(testing.allocator, nm));
    Record.remove(nm);
}

test "one session has one spelling" {
    // `Foo` AND `foo` WERE ONE SESSION ON A MAC AND TWO ON LINUX, because
    // the default APFS volume is case-insensitive and ext4 is not. For a
    // tool whose point is spanning machines that is not a semantic
    // ([[WI-2026-09-08-014]]).
    var a: [max_name_len]u8 = undefined;
    var b: [max_name_len]u8 = undefined;
    try testing.expectEqualStrings("foo", canonical(&a, "Foo").?);
    try testing.expectEqualStrings("foo", canonical(&a, "FOO").?);
    try testing.expectEqualStrings("local-9ece", canonical(&a, "Local-9ECE").?);
    // IDEMPOTENT, which is what lets it be applied at more than one place
    // without the places having to agree about who applied it.
    try testing.expectEqualStrings(canonical(&a, "Foo").?, canonical(&b, canonical(&a, "Foo").?).?);

    // NON-ASCII IS NOT A NAME, AND THAT IS A REFUSAL NOW RATHER THAN A
    // DOCUMENTED LIMIT. This asserted that `é` folds to itself, with the
    // note that a non-ASCII name "is still one session on a Mac and two on
    // Linux" — which is the very thing the header of this file calls not a
    // semantic. It is worse than that across the boundary: the fold is
    // byte-wise, so a precomposed `Ü` survives it and the same character
    // decomposed folds to `ü`, and Foundation normalises `Process`
    // arguments on the way out ([[validName]], [[WI-2026-09-12-002]]).
    try testing.expect(canonical(&a, "é") == null);

    // WHAT MAY NOT BE A NAME MAY NOT BE FOLDED INTO ONE.
    try testing.expect(canonical(&a, "A/B") == null);
    try testing.expect(canonical(&a, "") == null);

    // THE THREE FILES ARE ONE SESSION'S. Folding in one builder and not
    // another would give a session a socket under one name and a claim
    // under another — which is the shape [[WI-2026-09-08-008]] was about.
    var p1: [1024]u8 = undefined;
    var p2: [1024]u8 = undefined;
    try testing.expectEqualStrings(try socketPath(&p1, "Foo"), try socketPath(&p2, "foo"));
    try testing.expectEqualStrings(Record.pathFor(&p1, "Foo").?, Record.pathFor(&p2, "foo").?);
    try testing.expectEqualStrings(Record.lockPathFor(&p1, "Foo").?, Record.lockPathFor(&p2, "foo").?);

    // AND THE ROUND TRIP CLOSES. The holder takes its name back out of the
    // path it is served on, so what it registers, records and reports is
    // the folded one — not what was typed.
    try testing.expectEqualStrings("foo", nameOf(try socketPath(&p1, "Foo")));
}

test "a claim handed back takes its file with it" {
    // CLOSING THE DESCRIPTOR RELEASES THE LOCK AND LEAVES THE FILE, and a
    // lock with no record and no socket beside it is invisible to
    // `sessions` (which enumerates `.json`), unsweepable by `sweepEnded`
    // (which needs a record it can read), and reads `free` to everything
    // else. One per failed start, forever ([[WI-2026-09-08-013]]).
    var root: [128]u8 = undefined;
    const r = try std.fmt.bufPrint(&root, "/tmp/synapty-release-{d}", .{std.c.getpid()});
    std.Io.Dir.cwd().createDirPath(io_mod.get(), r) catch {};
    defer std.Io.Dir.cwd().deleteTree(io_mod.get(), r) catch {};
    paths.root_override = r;
    defer paths.root_override = null;

    const fd = Record.claim("handed-back") orelse return error.CouldNotClaim;
    var lbuf: [1024]u8 = undefined;
    const lock = Record.lockPathFor(&lbuf, "handed-back") orelse return error.NoStateDirectory;
    (try std.Io.Dir.cwd().openFile(io_mod.get(), lock, .{})).close(io_mod.get());

    Record.releaseClaim(fd);

    // THE FILE STAYS, AND THAT IS THE POINT. An flock lives on an inode;
    // a claim whose file can be unlinked is a claim whose inode can be
    // swapped out from under it, and then two starts hold two different
    // inodes and both believe they hold the name (below). So the identity
    // of a claim is the PATH, and the cost of that is one empty file per
    // name ever used — which is what a lock file is.
    (try std.Io.Dir.cwd().openFile(io_mod.get(), lock, .{})).close(io_mod.get());
    // AND THE NAME IS FREE TO BE TAKEN, rather than reading as a tombstone
    // nothing can clear. `.free` and `.absent` are the same answer to
    // every caller — `ownerGone` and `startWouldJoin` both group them —
    // so leaving the file behind decides nothing it used to decide.
    try testing.expectEqual(Claim.free, claimState("handed-back"));
    const again = Record.claim("handed-back") orelse return error.CouldNotClaim;
    Record.releaseClaim(again);
}

test "a session that cannot be recorded is refused rather than run unfindable" {
    // THE SILENT FAILURE WAS THE WHOLE DEFECT. `describe` returned void,
    // and a full disk, a quota or a permission left a claim held, a socket
    // bound and no record — a session running and attachable that
    // `sessions` cannot list and `sweepEnded` cannot remove, for as long
    // as the machine lasts ([[WI-2026-09-09-003]]).
    var root: [128]u8 = undefined;
    const r = try std.fmt.bufPrint(&root, "/tmp/synapty-nodesc-{d}", .{std.c.getpid()});
    std.Io.Dir.cwd().createDirPath(io_mod.get(), r) catch {};
    defer {
        var d: [1024]u8 = undefined;
        if (paths.sessionsDir(&d)) |sd| sys.chmod(sd, 0o700) catch {};
        std.Io.Dir.cwd().deleteTree(io_mod.get(), r) catch {};
    }
    paths.root_override = r;
    defer paths.root_override = null;

    var dir: [1024]u8 = undefined;
    const sessions = paths.sessionsDir(&dir) orelse return error.NoStateDirectory;
    makeSessionsDir(sessions);
    // A DIRECTORY THAT CANNOT BE WRITTEN, which is what a full disk, a
    // quota or a lost permission all look like from here.
    try sys.chmod(sessions, 0o500);

    try testing.expectError(error.AccessDenied, Record.describe("unwritable"));
}

test "a sweep cannot hand one name to two holders" {
    // THE THREE-PARTY SHAPE, WITHOUT ANY TIMING IN THE TEST.
    //
    // A start opens the lock file. A sweep of the same name unlinks it.
    // The start then takes its claim — on an inode no path names any
    // more, so it succeeds and the process runs normally. A SECOND start
    // now finds no file, creates one, and takes a claim of its own. Both
    // hold the name, and the second one's `Holder.init` unlinks the
    // first's live socket: exactly the damage the claim exists to prevent,
    // re-entered through the cleanup path ([[WI-2026-09-09-001]]).
    var root: [128]u8 = undefined;
    const r = try std.fmt.bufPrint(&root, "/tmp/synapty-two-holders-{d}", .{std.c.getpid()});
    std.Io.Dir.cwd().createDirPath(io_mod.get(), r) catch {};
    defer std.Io.Dir.cwd().deleteTree(io_mod.get(), r) catch {};
    paths.root_override = r;
    defer paths.root_override = null;
    ...
    if (paths.sessionsDir(&dir)) |d| makeSessionsDir(d);
    var lbuf: [1024]u8 = undefined;
    const lock = Record.lockPathFor(&lbuf, "two-holders") orelse return error.NoStateDirectory;

    var first = try std.Io.Dir.cwd().createFile(io_mod.get(), lock, .{ .truncate = false });
    defer first.close(io_mod.get());
    Record.remove("two-holders");
    try testing.expect(sys.tryClaim(@intCast(first.handle)));

    try testing.expect(Record.claim("two-holders") == null);
}

test "a name is a name and not a path" {
    // EVERY FILE A SESSION HAS IS THIS STRING INTERPOLATED INTO A
    // DIRECTORY, so a name carrying a separator carries a destination.
    // `--id ../../escaped` put the socket two directories ABOVE the
    // sessions directory and outside the 0700 [[RFC-0014]] C-ENTITLEMENT
    // requires of it, and nothing checked: there was no validation of
    // `--id` anywhere in the tree ([[WI-2026-09-08-008]]).
    try testing.expect(!validName("../../escaped"));
    try testing.expect(!validName("a/b"));
    try testing.expect(!validName("/abs"));
    try testing.expect(!validName(".."));
    try testing.expect(!validName("."));
    try testing.expect(!validName(""));
    // A NUL ENDS THE STRING FOR THE KERNEL AND NOT FOR US, so a name
    // carrying one is a path whose tail this process cannot see.
    try testing.expect(!validName("be\x00ll"));
    try testing.expect(!validName("tab\there"));
    // AND A NAME THAT IS ONE IDENTIFIER. `canonical` folds BYTES, so a
    // precomposed `Ü` (C3 9C) survives it while the same character
    // decomposed (55 CC 88) folds to `ü` — two sessions for one name a
    // human typed. Both spellings are refused, and so is every other
    // multibyte one ([[WI-2026-09-12-002]]).
    try testing.expect(!validName("\xc3\x9cber"));
    try testing.expect(!validName("U\xcc\x88ber"));
    try testing.expect(!validName("\xe6\x97\xa5\xe6\x9c\xac"));

    // A PATH THAT CANNOT BE BUILT IS `unknown`, NOT `absent`. The comment
    // on that arm says one unresolvable path "made EVERY session on the
    // machine read as gone, in one call", and nothing drove it until a
    // name could be refused ([[WI-2026-09-08-012]]).
    try testing.expectEqual(Claim.unknown, claimState("a/b"));
    try testing.expectEqual(Claim.unknown, claimState("x" ** (max_name_len + 1)));
    try testing.expect(!Record.ownerGone("a/b"));

    // AND THE NAMES THIS PROJECT ACTUALLY MINTS STILL PASS.
    try testing.expect(validName("local-9ece"));
    try testing.expect(validName("gpu-4"));
    try testing.expect(validName("reconnect-e2e-12345"));
    try testing.expect(validName("a.name.with.dots"));

    // BOUNDED BY THE KERNEL'S `sun_path`, not by a round number. A socket
    // whose full path is too long binds by basename — `{name}.sock` — and
    // 104 bytes less five less a NUL is ninety-eight. Measured one start
    // per length: 98 comes up, 99 does not ([[WI-2026-09-08-013]]).
    try testing.expectEqual(@as(usize, 98), max_name_len);
    try testing.expect(validName("x" ** max_name_len));
    try testing.expect(!validName("x" ** (max_name_len + 1)));
    // AND THE SOCKET PATH IT PERMITS REALLY FITS. A validator that accepts
    // what the next layer refuses has moved the error, not prevented it.
    var sbuf: [1024]u8 = undefined;
    const at_ceiling = try socketPath(&sbuf, "x" ** max_name_len);
    try testing.expect(sys.sockaddr_un.init(std.fs.path.basename(at_ceiling)) != null);

    // THE THREE PATHS AGREE, because they are built the same way from the
    // same string: what may not be a socket may not be a record or a lock.
    var buf: [1024]u8 = undefined;
    try testing.expectError(error.InvalidSessionName, socketPath(&buf, "a/b"));
    try testing.expect(Record.pathFor(&buf, "a/b") == null);
    try testing.expect(Record.lockPathFor(&buf, "a/b") == null);
}

test "a second holder cannot take a live name, even with no guard in front of it" {
    // THE GUARD IS A SECOND OPINION AND THIS IS THE FIRST. `holdRefused`
    // asks `startWouldJoin` before a start is attempted, and it CAN be
    // wrong — it was, about `unknown`, and a start let through unlinked a
    // live session's socket before discovering it could not have the name
    // ([[WI-2026-09-08-005]]).
    //
    // This calls `Holder.init` DIRECTLY, with no guard anywhere in front
    // of it, which is what makes the assertion worth something: the claim
    // is taken before anything is destroyed, so being wrong upstream costs
    // a refusal instead of a stranded session ([[WI-2026-09-08-006]]).
    var root: [128]u8 = undefined;
    const r = try std.fmt.bufPrint(&root, "/tmp/synapty-claim-first-{d}", .{std.c.getpid()});
    std.Io.Dir.cwd().createDirPath(io_mod.get(), r) catch {};
    defer std.Io.Dir.cwd().deleteTree(io_mod.get(), r) catch {};
    paths.root_override = r;
    defer paths.root_override = null;
    ...
    var pbuf: [256]u8 = undefined;
    const path = try testSocketPath(&pbuf, "held-name");

    var first = try Holder.init(testing.allocator, path);
    defer first.stop();

    // WHAT THE SOCKET IS, so the second attempt can be shown not to have
    // replaced it. An inode is the identity of the file the first holder
    // is listening on; a new one at the same path is the theft.
    const before = try std.Io.Dir.cwd().statFile(io_mod.get(), path, .{});

    try testing.expectError(error.SessionNameHeld, Holder.init(testing.allocator, path));

    const after = try std.Io.Dir.cwd().statFile(io_mod.get(), path, .{});
    try testing.expectEqual(before.inode, after.inode);
    // AND THE FIRST HOLDER STILL HAS THE NAME. A refused second attempt
    // that handed the claim back on its way out would leave this `free`.
    // BY THE NAME THE SOCKET PATH GIVES, which is what the holder claims —
    // not the label this test passed in to build that path.
    try testing.expectEqual(Claim.held, claimState(nameOf(path)));
}
...
test "a start is refused by the claim, not by whether the socket answers" {
    ...
    const held = claimLiveForTest("in-use");
    defer if (held) |fd| sys.close(fd);
...
const RaceProbe = struct {
    ...
    fn run(self: *RaceProbe) void {
        var i: usize = 0;
        while (i < self.rounds) : (i += 1) {
            self.writer.write(self.fd, self.kind, self.payload) catch return;
        }
    }
};

/// Reads frames until end of stream, recording the FIRST thing it sees
/// that no writer could have sent.
///
/// A THREAD OF ITS OWN, AND IT NEVER STOPS EARLY. The first version of
/// this read on the main thread and broke out on the first bad frame —
/// which deadlocked the test it was meant to fail: the writers were
/// blocked on a socket buffer nobody was draining any more, so `join`
/// never returned and the case hung instead of reporting.
const RaceReader = struct {
    ...
    fn run(self: *RaceReader) void {
        ...
        while (self.bad_kind == null) {
            var hdr: [header_len]u8 = undefined;
            if (!(readExactly(self.fd, &hdr) catch break)) return;
            const len = std.mem.readInt(u32, hdr[1..5], .little);
            const ok = (hdr[0] == @intFromEnum(Frame.data) and len == self.long_len) or
                (hdr[0] == @intFromEnum(Frame.input) and len == self.short_len);
            if (!ok) {
                self.bad_kind = hdr[0];
                self.bad_len = len;
                break;
            }
            if (len > 0 and !(readExactly(self.fd, payload[0..len]) catch break)) return;
            const want: u8 = if (hdr[0] == @intFromEnum(Frame.data)) 'L' else 'S';
            for (payload[0..len]) |c| {
                if (c != want) {
                    self.bad_kind = hdr[0];
                    self.bad_len = c;
        ...
        // DRAIN, NEVER STOP. Once the stream is out of step there is
        // nothing left to read correctly — but a reader that returns here
        // leaves both writers blocked on a socket buffer nobody empties,
        // and the test hangs instead of failing. Verified: it did.
        while (true) {
            const n = sys.read(self.fd, &payload) catch return;
            if (n == 0) return;
        }
    }
};

test "two threads writing to one socket never interleave a frame" {
    // WHAT A FRAME IS ON THE WIRE, AND WHO MAY BE WRITING ONE.
    // `writeFrame` is TWO `writeAll` calls — header, then payload — and
    // four threads reach the attached client's fd: the pump delivering
    // the child's bytes (`retainAndDeliver`), the handshake writing the
    // restoration (`sendRestoration`), the child reaper delivering
    // `.exit` (`deliver`), and the next attach writing `.displaced` to
    // the client it takes the seat from. Nothing ordered them, so a whole
    // frame could land between another's header and its payload.
    //
    // `writes_in_flight` does NOT cover this. It stops the fd being
    // CLOSED under a writer ([[WI-2026-09-02-016]]) and says nothing
    // about two writers on a live one. `catching_up` serialises exactly
    // one of the pairs — the pump against `catchUp` — and `attach`
    // publishes the fd with `catching_up == false` BEFORE the restoration
    // is written, so the largest write of all is the least protected.
    //
    // WHAT THAT COSTS THE HUMAN: `readFrame` takes the next five bytes as
    // a header, so the reader takes a length out of somebody else's
    // payload and either refuses the frame or hands the child bytes that
    // were never typed. An interleaved `.exit` is the client never
    // learning the child is gone, at the moment that matters most; an
    // interleaved `.displaced` is [[RFC-0014]] C-ONE-CLIENT's "said, not
    // inferred" turning back into a client that is simply cut off.
    //
    // THE SAME SHAPE ON THE CLIENT SIDE: `attachInputLoop` writes
    // `.input` and `attachSizeLoop` writes `.resize` to one socket, and
    // the `through` path has the pair again. Resize a window during a
    // paste and the holder reads three bytes of a resize header as
    // keystrokes.
    //
    // HAMMERED, NOT STAGED. Driving one real attach does not reproduce
    // it: the window between a header and its payload is a preemption
    // wide, so it has to be offered thousands of times. Measured — a
    // staged attach with the child flooding passed every run, and this
    // fails within twenty frames.
    //
    // IT IS THE INVARIANT THAT IS TESTED, not one of its callers. Point
    // the probes at bare `writeFrame` and this fails; that is the canary,
    // and it is also the contract `writeFrame` now carries.
    var fds: [2]i32 = undefined;
    if (std.c.socketpair(std.c.AF.UNIX, std.c.SOCK.STREAM, 0, &fds) != 0) return error.Unexpected;
    defer sys.close(fds[0]);

    // TWO LENGTHS AS WELL AS TWO KINDS, so a header read out of the wrong
    // place is wrong in a way this test can name.
    const rounds = 4000;
    ...
    @memset(&short, 'S');
    var w = FrameWriter{};
    var a = RaceProbe{ .writer = &w, .fd = fds[1], .kind = .data, .payload = &long, .rounds = rounds };
    var b = RaceProbe{ .writer = &w, .fd = fds[1], .kind = .input, .payload = &short, .rounds = rounds };
    var r = RaceReader{ .fd = fds[0], .long_len = long.len, .short_len = short.len };

    const tr = try std.Thread.spawn(.{}, RaceReader.run, .{&r});
    const ta = try std.Thread.spawn(.{}, RaceProbe.run, .{&a});
    const tb = try std.Thread.spawn(.{}, RaceProbe.run, .{&b});
    ta.join();
    tb.join();
    // End of stream for the reader, so it finishes rather than blocking.
    sys.close(fds[1]);
    tr.join();

    if (r.bad_kind) |kind| {
        std.debug.print(
            "after {d} whole frames: kind byte {d} with length/byte {d} — a write from " ++
                "one thread landed inside a frame from another\n",
            .{ r.frames, kind, r.bad_len },
        );
        return error.InterleavedFrame;
    }
    try testing.expectEqual(rounds * 2, r.frames);
}
