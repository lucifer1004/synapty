...
struct CompiledManifest {
    ...
    init(_ manifest: DetectManifest) {
        ...
        for (index, rule) in manifest.rules.enumerated() {
            // A RULE THIS BUILD CANNOT USE SAYS SO, rather than vanishing.
            //
            // This was one `guard … else { continue }`: an unknown state
            // or region did not error, did not log and did not fail the
            // load, so a manifest compiled with FEWER RULES THAN IT HAS.
            // Rename a `protocol.Status` tag and detection degrades across
            // the whole 19-tool fleet with every test green and every
            // manifest still "loading" — and `waiting` is the state the
            // entire attention UI runs on, so what the human loses is
            // being told their agent is blocked on them
            // ([[RFC-0004]] C-PASSIVE-DETECTION, [[WI-2026-09-11-014]]).
            guard Self.passiveStates.contains(rule.state) || rule.state == "skip" else {
                AppLog.detect.error("""
                    \(manifest.tool, privacy: .public) rule \(index, privacy: .public) names                     state '\(rule.state, privacy: .public)', which this build does not know —                     the rule is not compiled
                    """)
                continue
            }
            guard let region = Region(rule.region) else {
                AppLog.detect.error("""
                    \(manifest.tool, privacy: .public) rule \(index, privacy: .public) names region '\(rule.region ?? "none", privacy: .public)', which this build does not know — the rule is not compiled
                    """)
                continue
            }
...
@MainActor @Observable final class AgentDetector {
    ...
    /// The hub this service talks to, or nil when there is none.
    ///
    /// SET BY THE ONE FOLLOWER, never captured at start. `HubManager.boundPort`
    /// owns this number; a port taken once at wiring time is a port that
    /// stops following it, which is what left four services talking to a
    /// hub that had moved — or, worse, to the literal 9000 that adoption
    /// had just refused ([[WI-2026-09-10-008]]).
    var hubPort: Int?
    ...
    func start(paneManager: WorkspaceManager, agentMonitor: AgentMonitor) {
    ...
    private func sample() {
        ...
        for (leafID, leaf) in paneManager.facts {
            ...
            if ghostty_surface_process_exited(surface) {
                ...
                if !lifecycleSent.contains(key) {
                    ...
                    HubEventClient.sendStatusSignal(
                        port: hubPort, agent: agentID, state: "unknown", signalClass: "lifecycle")
                }
                continue
            }
            ...
            {
                HubEventClient.sendStatusSignal(
                    port: hubPort, agent: agentID, state: edge, signalClass: "passive")
            }
        }
    }
