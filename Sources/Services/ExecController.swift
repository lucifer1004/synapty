...
struct ExecRequest {
    /// What a request that names neither gets. `src/cli/types.zig`'s
    /// `ExecArgs` holds the same two; the CLI always sends both
    /// explicitly, so a drift only reaches an agent on a raw hub
    /// connection — a supported caller ([[WI-2026-09-11-016]]).
    ...
    var cwd: String?
    var pattern: String?
    // THE SAME TWO DEFAULTS `types.ExecArgs` HOLDS. The CLI always sends
    // both explicitly, so a drift only reaches an agent talking to the hub
    // directly — which is a supported caller, and the reason these are
    // named here rather than left as bare numbers ([[WI-2026-09-11-016]]).
    var timeoutSecs: Int = Self.defaultTimeoutSecs
    var rows: Int = Self.defaultRows
...
@MainActor @Observable final class ExecController {
    ...
    func start(paneManager: WorkspaceManager, agentMonitor: AgentMonitor, detector: AgentDetector) {
    ...
    private func receipt(_ req: ExecRequest, kind: String, outcome: ExecOutcome,
        ...
        HubEventClient.sendExecReceipt(
            port: hubPort, kind: kind, owner: req.owner,
    ...
    private func handleRead(_ req: ExecRequest) {
        ...
        case .ok(let record):
            ...
            HubEventClient.sendExecReceipt(
                port: hubPort, kind: "exec_read", owner: req.owner, generation: record.generation,
