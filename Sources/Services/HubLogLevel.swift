...
enum HubLogLevel {
    ...
    static func operatedPorts() -> [Int] {
        ...
        // A peer that has never reported itself has no hub to tell — and
        // neither does a machine whose own hub adoption was refused, which
        // is what the nil is ([[WI-2026-09-10-008]]).
        var ports: [Int] = tm.hubPort.map { [$0] } ?? []
    ...
    /// NIL IS A HUB THAT IS NOT THERE, not a hub at 9000. The caller's
    /// port is `HubManager.boundPort`, which answers nil when adoption was
    /// refused ([[WI-2026-09-10-008]]).
    @MainActor
