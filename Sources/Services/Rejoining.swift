...
enum Rejoining: Equatable {
    ...
    static func settled(registeredAs name: String, recorded: String?) -> Rejoining {
        // COMPARED AS THE HOLDER NAMES THEM. `SessionRecord.live()` maps
        // directory entries to names, so what comes back is the FOLDED
        // name; an id that arrived with an uppercase letter would never
        // match it under `==` and the human would be told their session
        // was gone while it was running ([[WI-2026-09-11-015]]).
        guard let recorded else { return .restarted(.sessionGone) }
        return SessionRecord.canonical(name) == SessionRecord.canonical(recorded)
            ? .rejoined : .restarted(.sessionGone)
    }
