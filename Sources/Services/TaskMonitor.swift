...
@MainActor @Observable final class TaskMonitor {
    ...
    private func runCLI(
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            // full pipe blocks the child forever, and with it every later
            // poll ([[WI-2026-08-08-005]]).
    ...
    /// Extract the flat `{ok, data?, error?}` that `synapty tools exec`
    /// prints. Distinct from parseEnvelopePayload, which unwraps a hub
    /// ENVELOPE — the local executor has no hub in the picture and so no
    /// envelope to unwrap. Pure; unit-tested.
    nonisolated static func parseToolsExecResult(_ output: String) -> [String: Any]? {
    ...
    /// THE PREFIX THE CLI MINTS UNDER ([[protocol.temp_agent_prefix]]),
    /// named once on this side rather than spelled at the call site and
    /// again in a test. If it drifts, the activity view stops filtering
    /// its own polling noise and the human reads their own tools back as
    /// agent activity ([[WI-2026-09-11-016]]).
    static let tempAgentPrefix = "cli-tmp-"

    /// Drop self-generated polling noise (cli-tmp- agents' own
    /// activity.list/task.list calls) from the activity stream.
    ///
    /// The prefix is [[protocol.temp_agent_prefix]], restated because
    /// Swift cannot import it. A change there has to reach here by hand,
    /// and that comment is the only thing that says so.
    /// Pure — unit-tested (WI-2026-08-08-021).
    nonisolated static func filterActivityNoise(_ items: [ActivityItem]) -> [ActivityItem] {
        items.filter { item in
            guard item.agent.hasPrefix(Self.tempAgentPrefix) else { return true }
    ...
    private func fetchTasks() {
        ...
        tasksInFlight = true
        // [[ADR-0008]] decision 6: execute LOCALLY rather than asking the
        // hub. The workbench is the credential holder, so routing its own
        // poll through the hub would only come straight back here — two
        // subprocesses and a round trip to reach the Keychain that is
        // already on this machine. Agents still go through the hub,
        // because they are not on this machine and have no credential.
        runCLI(["tools", "exec", "--tool", "task.list"]) { [weak self] output in
            defer { self?.tasksInFlight = false }
            guard let self, let output else { return }
            guard let payload = Self.parseToolsExecResult(output) else { return }
