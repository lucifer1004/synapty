...
//
// Also owns the ghostty config fragment (~/.config/synapty/machine/ghostty.conf):
...
@MainActor @Observable final class SynaptySettings {
    ...
    /// SYNAPTY'S NUMBER, NOT GHOSTTY'S. The fragment writes this
    /// unconditionally because ghostty's own default line limit is
    /// UNBOUNDED — only byte-capped — which lets the grid grow huge and
    /// drags memory and layout cost with it.
    ///
    /// This doc used to read "nil = ghostty default (10000)", which
    /// attributes our choice to a dependency and is the reading that
    /// would let someone delete the line that makes it true
    /// ([[WI-2026-09-11-012]]).
    static let defaultScrollbackLines = 10_000

    /// Scrollback line limit. nil = `defaultScrollbackLines`.
    ...
    private func buildFragment() -> String {
        ...
        lines.append("scrollback-limit-lines = \(Self.defaultScrollbackLines)")
        ...
        // A SECOND LINE FOR THE SAME KEY, and ghostty takes the last one —
        // so the human's setting wins and there is no bug today. Said
        // plainly because the line above does not mention it, and a reader
        // deleting one of the two has to know which ([[WI-2026-09-11-012]]).
