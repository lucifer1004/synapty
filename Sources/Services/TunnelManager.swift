...
@MainActor @Observable final class TunnelManager {
    ...
    /// Local hub TCP port, or nil when this machine has no hub to reach.
    ///
    /// A HUB REACHED IS A HUB TOLD WHAT THE HUMAN SET
    /// ([[RFC-0012]] C-LEVEL-CONTROL, [[HubLogLevel.applyCurrent]]).
    ///
    /// NO DEFAULT LITERAL. `HubManager.boundPort` owns this number and
    /// answers nil when adoption was refused; 9000 was the one value that
    /// must not stand in for that nil, because it names the hub the
    /// refusal was about ([[WI-2026-09-10-008]]).
    var hubPort: Int? {
    ...
    func nameSession(agentID: String, on host: HostEntry?, name: String) {
        ...
        if let host {
            runOnHost(host, command: "\(HostBinary.remotePath) name --id \(Shell.quote(agentID)) --name \(Shell.quote(trimmed))") { _ in }
            return
        }
        let bin = SynaptyBinary.resolve() ?? "synapty"
    ...
    func runOnHost(
        ...
        // A SUCCESS SENTINEL. The marker only appears if the command
        // actually reached the end, and ssh writes warnings to stderr that
        // say nothing about whether the remote command worked.
        //
        // It is no longer the ONLY way to ask: `SubprocessRunner.Output`
        // carries `exitCode` now, and because the marker is behind `&&`,
        // ssh's status and the marker's presence say the same thing here.
        // Kept because it is what `setupSucceeded` also asks for, not
        // because a status is unavailable — the reason this comment used
        // to give stopped being true when `exitCode` was added
        // ([[WI-2026-09-10-001]]).
    ...
    /// its own pid. `synapty connect` writes the same two files.
    ...
    /// WHAT AN SSH THIS POOL OPENS MUST PRESENT — resolved HERE, on the
    /// actor that owns the records, and handed to the pool alongside the
    /// key.
    ///
    /// THE POOL USED TO REACH BACK FOR THIS. `pool.sshArgs` held a
    /// closure written in this `@MainActor` class and stored as a bare
    /// `(HostKey) -> [String]`, which erases the isolation it was written
    /// in — and `MasterPool` is `@unchecked Sendable`, so nothing in the
    /// type system was left to notice. `grow` called it after dropping
    /// its lock, on one of the background threads `MasterPool`'s own
    /// header REQUIRES, while it walked `trackedHosts` — a dictionary the
    /// heartbeat's main hop mutates in place. That is not a stale read;
    /// it is a walk over a buffer being rehashed
    /// ([[WI-2026-09-12-001]]).
    ///
    /// Every caller already resolves `poolKey` on the main actor before
    /// going off it, which is the discipline `runHeartbeat` states in as
    /// many words. This rides alongside.
    func poolCredentials(for host: HostEntry) -> [String] {
        identityArgs(for: host)
    }
    ...
    /// is up to date. Always runs `host-setup` — it reuses an existing
    ...
    func ensureTunnel(
        ...
        //
        // AND IT ONLY OWNS THE CHANNEL WHEN IT MINTED THE NAME. A dial
        // handed an id is joining a session that already exists, whose
        // client is writing this account right now ([[ConnectProgress]]
        // `beginDial`).
        ConnectProgress.beginDial(for: command.agentID, joining: agentID != nil)

        // FAST PATH (WI-2026-08-08-063): a host whose tunnel is already
        // connected opens a new session immediately — no host-setup run
        // (5-6 SSH round trips + terminfo scp even with a live master),
        // no .connecting status flicker. The ControlMaster is verified
        // off-main; a dead master reconnects in the background. The dial
    ...
    /// the far side attaches when a holder of that name is already
    /// running (WI-2026-08-12-013). Passing the id of a remote agent you
    /// can see in the merged view is what "go to it" means.
    /// `cwd` is where the far side's shell should START — the directory a
    /// duplicated pane is reopened in ([[RFC-0015]] C-LAYOUT). It reaches
    /// a session that is being STARTED; a reattach rejoins the work where
    /// it already is, which is the answer that matters more.
    func connectCommand(for host: HostEntry, agentID overrideID: String? = nil,
        ...
        // An absent one is ABSENT: the empty strings that used to hold
        // their slots open belonged to a positional layout, and there is
        // no layout left to hold open ([[ADR-0020]]).
        ...
        //
        // Two things ride here rather than as options: the human's
        // durability opt-out, and where the local client can find itself
        // — a bundled application cannot rely on a bare name on PATH
        // ([[WI-2026-08-17-009]]). Both are read from the environment by
        // the binary itself (`SYNAPTY_DURABLE`, `SYNAPTY_BIN`), because
        // both must survive into whatever the pane goes on to spawn, and
        // an option is spent on the process it was given to.
        ...
        // THE PANE RUNS THE BINARY, NOT A SCRIPT ([[ADR-0020]]). What was
        // `bash <a launch script> <nine positionals and a variadic run of quads>`
        // is one subcommand with named options; the two positionals it
        // never read (the tunnel and hub ports) are simply gone.
        //
        // A BARE NAME IF THERE IS NO OTHER ANSWER, which is what the
        // script did: a human with the CLI on PATH gets a working pane
        // from a checkout with no bundle.
        parts += [Shell.quote(SynaptyBinary.resolve() ?? "synapty"), "connect",
                  "--id", Shell.quote(agentID),
                  "--fresh-id", Shell.quote(freshID),
                  "--host", Shell.quote(host.address), "--port", "\(port)",
                  "--user", Shell.quote(username)]
        if !key.isEmpty { parts += ["--key", Shell.quote(key)] }
        if !jump.isEmpty { parts += ["--jump", Shell.quote(jump)] }
        if let cwd, !cwd.isEmpty { parts += ["--cwd", Shell.quote(cwd)] }
        // ONE ARGUMENT EACH. As four positionals on the end of a variadic
        // run they left no room after them for anything else, which is why
        // everything else about a connection used to ride in the
        // environment.
        for fwd in effectiveForwardings(for: host) {
            parts += ["--forward", Shell.quote(fwd.wireSpec)]
        }
    ...
    func localCommand(agentID overrideID: String? = nil) -> PaneLaunch {
        ...
        // THE ONE RESOLVER ([[SynaptyBinary]]), which declares itself "the
        // single source of truth for the bundled-vs-dev lookup". This had
        // a second one — copied knowingly, since its doc cross-referenced
        // the first — and the two had already parted: `resolve()` answers
        // nil when neither copy exists so the caller can fall back to a
        // bare name on PATH, exactly as the remote path below does. The
        // copy returned an absolute `cwd/zig-out/bin/synapty` it never
        // checked, so a human with the CLI on PATH, no bundle and a cwd
        // outside the checkout got a local pane whose command named a file
        // that is not there ([[WI-2026-09-11-010]]).
        let synaptyBin = SynaptyBinary.resolve() ?? "synapty"
        // START, THEN ATTACH — the same two requests the far side makes on
        ...
        let shell = DevLaunchArgs.paneCommand.map { "sh -c \(Shell.quote($0))" }
            ?? "${SHELL:-/bin/zsh} -l"
        // `--hub none` WHEN THIS MACHINE HAS NO HUB — a word, not an
        // omission.
        //
        // This used to leave the flag out, on the reasoning that `synapty
        // run --help` documents its own default and so an absent flag says
        // "I do not know" in a vocabulary the far side already has. It
        // does not: an absent `--hub` RESOLVES — SYNAPTY_HUB_PORT, then
        // the discovery file, then 9000 — so omitting it asked for exactly
        // what `127.0.0.1:9000` asks for, and the pane went on dialling
        // the hub an adoption had just refused. The other four consumers
        // [[WI-2026-09-10-008]] corrected were genuinely fixed, because
        // `HubEventClient.loopback` really does return nil; the pane half
        // was a change to the command STRING and not to where the pane
        // lands ([[WI-2026-09-12-002]]).
        let hub = hubPort.map { " --hub 127.0.0.1:\($0)" } ?? " --hub none"
        guard SynaptySettings.shared.localDurableSessions else {
            let cmd = "\(Shell.quote(synaptyBin)) run --id \(Shell.quote(agentID))\(hub)"
        ...
        let inner = "\(bin) run --hold --detach --id \(id)\(hub)"
    ...
    /// `host-setup` prints `PEER_PORT=<n>` when it established a forward
    ...
    /// `host-setup` prints both figures after the hub is ensured. Absent
    ...
    func adoptExistingPeers(_ peers: [(peer: String, port: Int)], capabilities: [String: Set<String>] = [:],
        ...
        if !ours.isEmpty { knownCapabilities = ours }
        for entry in peers where entry.port > 0 {
            ...
            if let host = hostStore?.hosts.first(where: { assignedPeerPorts[$0.id] == entry.port })
                ?? hostStore?.hosts.first(where: {
                    HostStore.inSentence($0).lowercased()
    ...
    /// Capabilities THIS MACHINE'S HUB reports it provides, so a peer's
    /// omissions can be named rather than merely counted.
    ///
    /// ASKED OF THE HUB, NOT KEPT HERE. This was a literal
    /// `["presence_relay"]`, and it went stale the hour `pending-wake` was
    /// added to `federation.local_capabilities`: the hub-status popover
    /// then told the human a peer was missing NOTHING while the tooltip on
    /// that peer's own agents said its build could not report unread mail
    /// — two answers to one question, and the silent one came from the
    /// surface built to satisfy [[RFC-0010]] C-DIAGNOSABILITY
    /// ([[WI-2026-09-10-010]]).
    ///
    /// EMPTY UNTIL THE HUB HAS ANSWERED, and empty means no subtraction is
    /// attempted: a build that expects nothing can never report an
    /// absence, and inventing one from an unanswered hub would be worse
    /// than waiting.
    private(set) var knownCapabilities: [String] = []
    ...
    var peerSummaries: [PeerSummary] {
        assignedPeerPorts.compactMap { hostID, port in
            ...
            return PeerSummary(
                ...
                // AND NIL AGAIN WHILE OUR OWN LIST IS UNANSWERED, for the
                // same reason: subtracting from an empty list answers
                // "missing nothing" about every peer, which is a claim and
                // not an absence of one ([[WI-2026-09-10-010]]).
                missing: (reported == nil || knownCapabilities.isEmpty)
                    ? nil
                    : knownCapabilities.filter { !declared.contains($0) })
    ...
    ///
    /// Synapty has no password path at all: `host-setup` runs in the
    ...
    /// WHETHER A SETUP RUN SUCCEEDED.
    ///
    /// THE STATUS, WHICH WAS NOT BEING READ. This asked
    /// `output.error == nil && !output.timedOut`, and `Output.error` is
    /// documented as a LAUNCH failure — the process could not start.
    /// Neither field is set by a non-zero exit. So a host that refuses key
    /// authentication, runs a platform this project has no binary for, or
    /// has none built exited 1 or 2 and was recorded as CONNECTED:
    /// forwards that were never made recorded, every pending pane callback
    /// fired against a host with nothing deployed on it, and the host
    /// stamped to the top of the recently-used list. The whole failure
    /// branch below — including the sentence that tells a human their host
    /// wants a password — could only be reached by a binary that would not
    /// launch ([[WI-2026-09-10-001]]).
    ///
    /// AND THE MARKER, BECAUSE THE CONTRACT SAYS SO. `synapty host-setup`
    /// documents its stdout as read by the workbench and `SETUP_OK` as the
    /// last line of a run that got that far. Nothing read it. A status
    /// alone would be enough today — every error path exits non-zero
    /// before the marker — and asking for both is what keeps that true:
    /// an exit zero that never reached the end is not a success either.
    ///
    /// `exitCode` is nil when the process never ran, which is a failure
    /// the first two conditions already name — the `?? 1` is for a shape
    /// that cannot reach here rather than a case with meaning.
    static func setupSucceeded(_ output: SubprocessRunner.Output) -> Bool {
        output.error == nil
            && !output.timedOut
            && (output.exitCode ?? 1) == 0
            && output.stdout.contains("SETUP_OK")
    }

    private func runSetup(for host: HostEntry, account: String? = nil) {
        // ASKED BEFORE ANYTHING IS ATTEMPTED, which is what
        // [[HostReadiness]] exists for and what nothing did.
        //
        // Its header argues the two-consumer split at length and only the
        // terminal half had a caller, so this — the machinery that scps a
        // binary, starts a hub and holds a ControlMaster, none of it able
        // to answer a prompt — found out at the far end: sixty seconds of
        // ssh, then a password prompt read back out of its output by
        // `wantsPassword` below. The answer was available here the whole
        // time ([[WI-2026-09-11-007]]).
        //
        // ONLY THE SETTLED GAPS. A host that names no key at all still
        // goes and finds out, because `ssh-agent` and `~/.ssh/config`
        // answer non-interactively and this cannot see either.
        // ASKED OF THE RESOLVER, NOT THE RECORD. `effectiveKeyPath` below
        // substitutes this machine's enrolled key when the named one is
        // not here — "the case [[ADR-0009]] enrolment answers" — so the
        // first version of this guard refused exactly the sync scenario
        // the workbench repairs ([[WI-2026-09-11-019]]).
        if let store = hostStore,
           let why = HostReadiness.evaluate(host: host, store: store)
               .deployRefusal(resolvedKey: effectiveKeyPath(for: host)) {
            failSetup(for: host, reason: why)
            return
        }

        // NAMED OPTIONS TO THE BINARY ([[ADR-0020]]). This was a fixed
        // positional layout ending in a variadic run of forwarding quads,
        // two of whose slots — the hub port among them — the script never
        // read; a fixed layout cannot drop one in the middle.
        var args = ["host-setup",
                    "--host", host.address,
                    "--port", "\(effectivePort(for: host))",
                    "--user", effectiveUsername(for: host),
                    "--tunnel-port", "\(peerPort(for: host))"]
        if let key = effectiveKeyPath(for: host), !key.isEmpty { args += ["--key", key] }
        if let jump = effectiveProxyJump(for: host), !jump.isEmpty { args += ["--jump", jump] }
        for fwd in effectiveForwardings(for: host) {
            args += ["--forward", fwd.wireSpec]
        }
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            // [[RFC-0009]] C-BOUNDARIES: the human's host LABEL is the
            // authoritative peer id, and an option rather than a variable
            // now that there is room for one.
            let output = SubprocessRunner.run(
                executable: SynaptyBinary.resolve() ?? "synapty",
                arguments: args + ["--peer-id", peerID],
                environment: env,
                timeout: 60
            )
            DispatchQueue.main.async {
                guard let self else { return }
                if Self.setupSucceeded(output) {
                    self.tunnelStates[host.id] = .connected
                    self.recordConfiguredForwards(for: host)
                    // [[ADR-0008]] stage 3b: `host-setup` started a hub on
                    ...
                    self.failSetup(for: host, reason: lastLine)
                }
            }
        }
    }

    /// A SETUP THAT WILL NOT HAPPEN, REPORTED THE WAY ONE THAT FAILED IS.
    ///
    /// One place, because a refusal that skipped any of these three would
    /// be a worse bug than the one it prevents: a pane left spinning on a
    /// `pendingCallbacks` entry nothing will ever fire, or a placeholder
    /// with no error in it ([[WI-2026-09-11-007]], WI-2026-03-31-003).
    private func failSetup(for host: HostEntry, reason: String) {
        tunnelStates[host.id] = .failed(reason)
        pendingCallbacks.removeValue(forKey: host.id)
        NotificationCenter.default.post(
            name: .synaptyTunnelFailed,
            object: nil,
            userInfo: ["hostID": host.id, "message": reason]
        )
    }
    ...
    private func runHeartbeat() {
        ...
        for (hostID, host) in hostsToCheck where !checksInFlight.contains(hostID) {
            ...
            let key = poolKey(for: host)
            let credentials = poolCredentials(for: host)
            let pool = self.pool
            ...
            DispatchQueue.global(qos: .utility).async { [weak self] in
                ...
                pool.growIfLoaded(key, credentials: credentials)
                let reachable = alive
                DispatchQueue.main.async {
                    guard let self else { return }
