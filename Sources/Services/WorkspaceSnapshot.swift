...
struct WorkspaceSnapshot: Equatable {
    ...
    struct SessionEntry: Codable, Equatable {
        ...
        /// EVERY FIELD THE DECODER READS, so the two initialisers cannot
        /// hold different sets. `zoomedSlotIndex` was decoded above and
        /// absent here, and the only reason nothing broke is that the one
        /// production builder assigns it on a separate line afterwards —
        /// so a second builder would have silently dropped a human's
        /// zoomed slot ([[WI-2026-09-11-012]]). The doc above says "The
        /// next defaulted field added to this struct needs the same line";
        /// this initialiser is the half that rule did not cover.
        init(id: UUID? = nil, label: String, root: Node? = nil,
             focusedSlotIndex: Int? = nil, zoomedSlotIndex: Int? = nil,
            ...
            self.zoomedSlotIndex = zoomedSlotIndex
