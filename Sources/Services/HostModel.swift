...
struct PortForward: Identifiable, Codable, Equatable {
    ...
    /// The target as everything downstream needs to read it.
    ///
    /// THE DOCUMENTED DEFAULT, MADE EXPLICIT. `targetHost` says "empty =
    /// localhost" and is edited through a bare TextField, so an empty box
    /// is a shape this app produces; it used to travel as an empty field
    /// and be dropped without a word by `synapty connect`, which dialled
    /// the pane anyway ([[WI-2026-09-10-002]]).
    ///
    /// AND BRACKETED WHERE IT HAS TO BE. A colon separates the fields of
    /// the spec and also the groups of an IPv6 literal; ssh spells its way
    /// out of that with brackets, and so does this rather than inventing a
    /// second convention for the same string.
    ...
    /// The `--forward` argument for `synapty connect` and `host-master`.
    var wireSpec: String {
        "\(kind.rawValue):\(listenPort):\(wireTarget):\(targetPort)"
    }
...
@MainActor @Observable final class HostStore {
    ...
    func displayName(of hostID: UUID?) -> String {
        ...
        return Self.named(hostID, in: hosts)
    }

    /// The same machine, named for the MIDDLE of a sentence.
    ///
    /// TWO FORMS, ONE OWNER — and having them in two FUNCTIONS is exactly
    /// how they drifted. `ApprovalSheet` renders both in one flow: its
    /// title said "This Mac → deskmac" from `displayName` while the line
    /// under it said "this Mac already has a file with this name" from a
    /// private re-implementation, lowercase at the start of its own
    /// sentence. The comment above this function was written about that
    /// pair — "A grant a human withdraws must read exactly as it did when
    /// they made it" — and there were four re-implementations of it with
    /// three different local-machine spellings ([[WI-2026-09-11-010]]).
    ///
    /// ONLY THE LOCAL MACHINE HAS TWO FORMS. A host the human named is
    /// written the way they wrote it, wherever it appears: lowercasing
    /// "GPU" mid-sentence would be a different machine.
    func displayNameInSentence(of hostID: UUID?) -> String {
        guard let hostID else { return "this Mac" }
        return Self.named(hostID, in: hosts)
    }

    /// A HOST NAMED AS THE HUMAN NAMED IT, or the local machine
    /// mid-sentence. The one place the label-or-address rule lives.
    ///
    /// Takes the host rather than its id, for the callers that already
    /// hold one and have no store to ask.
    static func inSentence(_ host: HostEntry?) -> String {
        guard let host else { return "this Mac" }
        return host.label.isEmpty ? host.address : host.label
    }

    private static func named(_ hostID: UUID, in hosts: [HostEntry]) -> String {
        guard let host = hosts.first(where: { $0.id == hostID }) else { return "a host" }
        return inSentence(host)
    }
