...
final class MasterPool: @unchecked Sendable {
    ...
    /// CARRIES THE CREDENTIALS TOO, because they were the thing that was
    /// silently wrong. The fake used to take only the key and the socket
    /// path, so no test could see WHAT a master would be opened with —
    /// and what it was opened with came from a closure reaching into
    /// another actor's dictionary ([[WI-2026-09-12-001]]).
    var openMaster: ((HostKey, String, [String]) -> Bool)?
    ...
    @discardableResult
    /// CREDENTIALS ARE PASSED IN, NEVER FETCHED. This used to call a
    /// `sshArgs` closure the pool held, installed by `TunnelManager` and
    /// reading its `@MainActor` `trackedHosts` — from `grow`, after the
    /// lock is dropped, on one of the background threads this type's own
    /// header requires. A background `values.first(where:)` over a
    /// Dictionary the main actor is mutating in place is not a stale
    /// read; it is a walk over a buffer being rehashed.
    ///
    /// The closure's type — a bare `(HostKey) -> [String]` on an
    /// `@unchecked Sendable` class — erased the isolation it was written
    /// in, so nothing in the type system was left to notice
    /// ([[WI-2026-09-12-001]]). Every caller already resolves the key on
    /// the main actor before going off it; the credentials travel the
    /// same way.
    func place(_ key: HostKey, tenant: String, credentials: [String]) -> String? {
        lock.lock(); defer { lock.unlock() }
        ...
        guard let target = candidates.min(by: { count($0) < count($1) }) ?? grow(key, credentials: credentials) else {
            return nil
        }
    ...
    @discardableResult
    func placeExclusive(_ key: HostKey, tenant: String, credentials: [String]) -> String? {
        lock.lock(); defer { lock.unlock() }
        let all = members(for: key)
        let load = occupancy(of: all)
        let idle = all.first {
        ...
        guard let target = idle ?? grow(key, credentials: credentials) else { return nil }
        claim(tenant: tenant, on: target)
        return target
    }
    ...
    /// would not help.
    @discardableResult
    func growIfLoaded(_ key: HostKey, credentials: [String]) -> String? {
        lock.lock(); defer { lock.unlock() }
        let all = members(for: key)
        ...
        // it said yes to.
        if usable.isEmpty { return grow(key, credentials: credentials) }
        guard usable.allSatisfy({ loadedLocked($0) }),
              growingWouldHelpLocked(key)
        else { return nil }
        return grow(key, credentials: credentials)
    }
    ...
    @discardableResult
    func placeAfterRefusal(on socket: String, _ key: HostKey, tenant: String,
                           credentials: [String]) -> String? {
        lock.lock()
        full.insert(socket)
        lock.unlock()
        return place(key, tenant: tenant, credentials: credentials)
    }
    ...
    private func grow(_ key: HostKey, credentials: [String]) -> String? {
        let dir = Self.socketDirectory
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        lock.unlock()
        let opened = openMaster?(key, path, credentials)
            ?? Self.spawnMaster(key: key, socketPath: path, extraArgs: credentials)
        lock.lock()
        growing.remove(path)
