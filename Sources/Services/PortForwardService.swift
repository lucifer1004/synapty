...
@MainActor @Observable final class PortForwardService {
    ...
    func expose(hostID: UUID?, remotePort: Int, agent: String, title: String?,
        ...
        if let host {
            ...
            // RESOLVED ON THE MAIN ACTOR, like the key above it — the pool
            // must not reach back for these from the detached task below
            // ([[WI-2026-09-12-001]]).
            let credentials = tunnelManager.poolCredentials(for: host)
            ...
            let placed: (socket: String?, ok: Bool) = await Task.detached(priority: .userInitiated) {
                guard let socket = pool.place(key, tenant: tenant, credentials: credentials)
            ...
            if let idx = exposures.firstIndex(where: {
                ...
                portOwners.removeValue(forKey: allocated)
                // AND ASKED A THIRD TIME, AFTER THE SECOND AWAIT. The check
                // above was put there because the first round trip let the
                // list change; this cancel is another round trip of up to
                // ten seconds, added later ([[WI-2026-09-02-036]]), and
                // `idx` was still being used across it. A withdrawal in
                // that window moved every later exposure down one, so this
                // set ANOTHER exposure's path and handed the agent back a
                // different service — or indexed past the end
                // ([[WI-2026-09-23-003]]).
                guard let now = exposures.firstIndex(where: {
                    $0.hostID == hostID && $0.remotePort == remotePort
                }) else {
                    return .refused("port \(remotePort) was withdrawn while this request was being settled")
                }
                exposures[now].path = path
                return .ok(exposures[now])
            }
            local = allocated
            carrier = socket
        } else {
