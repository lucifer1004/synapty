...
/// make it so.
///
/// `host-setup` compares md5 and uploads when they differ, and its own
...
enum HostBinary {
    ...
    /// WHERE THE BINARY LIVES ON A HOST THIS WORKBENCH HAS SET UP.
    ///
    /// ONE OWNER, BECAUSE THERE WERE EIGHT SPELLINGS AND NO OWNER. This
    /// file WRITES the path — `mkdir -p`, the scp destination, `chmod +x`
    /// — and four other files READ it to run `version`, `name`, `end` and
    /// `sessions`. Move the install location and those four keep pointing
    /// at the old one, each failing quietly by construction: `nameSession`
    /// discards its completion, and the other three parse stdout, so an
    /// absent binary reads as "no sessions" rather than as an error. A
    /// human's pane rename would simply stop reaching the far side
    /// ([[WI-2026-09-11-010]]).
    ///
    /// RELATIVE TO THE LOGIN DIRECTORY, deliberately: every remote command
    /// this workbench sends runs from there, and an absolute path would
    /// have to know the remote home.
    static let remotePath = ".synapty/bin/synapty"
    /// The directory that holds it, for the one caller that makes it.
    static let remoteDir = ".synapty/bin"

    /// WHICH BUILD A MACHINE TAKES, from what it says it is.
    ///
    /// The same five `uname -sm` answers `host-setup` maps, and named
    ...
    /// refuse the write. `host-setup` does the same thing for the same
    /// reason.
    ///
    ...
    nonisolated static func upload(local: String, connection: RemoteConnection,
        ...
        let removed = SubprocessRunner.runQuiet(
            ...
            arguments: connection.sshOptions + [
                connection.userAtHost,
                "mkdir -p \(remoteDir) && rm -f \(remotePath)",
            ],
            timeout: 20)
        ...
        let sent = SubprocessRunner.runQuiet(
            ...
            arguments: connection.scpOptions
                + [local, "\(connection.userAtHost):\(remotePath)"],
            timeout: timeout)
