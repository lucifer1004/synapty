...
@MainActor @Observable final class TransferService {
    ...
    func machineName(of endpoint: FileEndpoint) -> String {
        // MID-SENTENCE, THROUGH THE OWNER ([[HostStore.displayNameInSentence]]).
        // This was a re-implementation, and the approval sheet rendered it
        // beside `displayName` in one flow ([[WI-2026-09-11-010]]).
        //
        // THE `?` IS ON THE STORE, NOT ON THE ANSWER. `hostStore` is
        // `weak`, and `hostStore?.…  ?? "a host"` short-circuits on a nil
        // STORE — so a record built after the store had gone answered "a
        // host" about THIS MAC, whose name needs no store to know. Low
        // reachability, and the surface is a transfer record answering
        // "where did this file come from" ([[WI-2026-09-12-002]]).
        hostStore.map { $0.displayNameInSentence(of: endpoint.hostID) }
            ?? (endpoint.hostID == nil ? "this Mac" : "a host")
    }
    ...
    private func plan(for transfer: Transfer) -> TransferPlan? {
        ...
        func binding(_ endpoint: FileEndpoint) -> TransferPlan.PoolBinding? {
            ...
            return TransferPlan.PoolBinding(
                pool: tunnelManager.pool,
                key: tunnelManager.poolKey(for: host),
                // CARRIED, NOT FETCHED LATER. This binding is built here
                // on the main actor and used from `takingExclusiveConnections`,
                // which its own doc says belongs off it
                // ([[WI-2026-09-12-001]]).
                credentials: tunnelManager.poolCredentials(for: host))
        }
...
struct TransferPlan: Sendable, Equatable {
    ...
    struct PoolBinding: @unchecked Sendable, Equatable {
        ...
        /// What an ssh this pool opens for this host must present,
        /// resolved on the main actor when the plan was made.
    ...
    func takingExclusiveConnections() -> (TransferPlan, @Sendable () -> Void) {
        ...
        func bind(_ leg: Leg, _ binding: inout PoolBinding?, _ side: String) -> Leg {
            ...
            guard let socket = held_binding.pool.placeExclusive(
                held_binding.key, tenant: tenant, credentials: held_binding.credentials)
            else { return leg }
