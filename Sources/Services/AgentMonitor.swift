...
struct AgentInfo: Identifiable, Equatable {
    ...
    /// ([[RFC-0011]] C-DEFAULT), and `nil` when it CANNOT BE SAID.
    ///
    /// THREE VALUES, NOT TWO. On a hub with no workbench a wake candidate
    /// is never attempted, so no wake event ever fires and this row is the
    /// only carrier the fact has. A peer whose build predates the
    /// `pending-wake` capability cannot report it at all, and rendering
    /// that silence as "nothing waiting" is a MISREAD rather than a
    /// declined capability — so the absence is a value here, with
    /// `pendingWakeUnknownCause` saying which absence it is.
    var pendingWake: Bool? = nil

    /// Why `pendingWake` is nil, in the vocabulary [[RFC-0010]]
    /// C-DIAGNOSABILITY owns. Set by the hub, never derived here: only the
    /// hub that negotiated the link knows what the peer's build provides.
    var pendingWakeUnknownCause: UnknownCause? = nil
    ...
    /// MAIL NOBODY HAS ANNOUNCED, or nil when there is nothing to say.
    ///
    /// [[RFC-0011]] C-DEFAULT: on a hub with no workbench a wake candidate
    /// is never attempted, so no wake event fires and "this agent has mail
    /// it has not been told about" reaches nobody unless the row says so.
    /// That outcome is acceptable and is NOT a defect — an agent reading
    /// its mail when a human next looks is coherent — but it is only
    /// acceptable if the human can see it is happening.
    ///
    /// AND THE ABSENCE IS WORDS TOO, in the one case where it carries
    /// information the row does not otherwise have: a peer that relays
    /// presence perfectly well may still predate this capability, and then
    /// nothing else on the row says why the mail column is blank. Where
    /// the peer is unreachable or contested, `unknownExplanation` has
    /// already said so and repeating it would bury the case that is new.
    var pendingWakePhrase: String? {
        if pendingWake == true { return "has mail nobody has announced" }
        guard pendingWake == nil else { return nil }
        switch pendingWakeUnknownCause {
        case .peerLacksCapability: return "this machine's build does not report unread mail"
        case .peerUnreachable, .contested, .noEvidence, .none: return nil
        }
    }
...
@MainActor @Observable final class AgentMonitor {
    ...
    /// THE LOCAL HUB'S PORT, once `startMonitoring` has been told it. NOT
    /// 9000 BEFORE THAT: 9000 is the one number that must never stand in
    /// for "no hub" ([[WI-2026-09-10-008]]), and a default that nothing is
    /// meant to read is a default something eventually does
    /// ([[WI-2026-09-23-007]]).
    ...
    private func connect() {
        guard monitoring, let port else { return }
    ...
    func applyEvent(_ payload: [String: Any], machine: String = "") {
        ...
        if let type = payload["type"] as? String, type == "tool_request" {
            // THE HUB HOLDING IT, OR NOBODY ([[WI-2026-09-23-007]]). This
            // fell back to the LOCAL hub's port when the sending peer's was
            // not known — the misroute the note above says leaves the agent
            // blocked forever, done silently. It is reachable: an event
            // already queued for the main thread is delivered after
            // `forgetPeer` has dropped that machine's port. The hub that
            // parked the request is then one this side can no longer
            // reach, and the honest act is to not answer it rather than to
            // answer somebody else.
            let replyPort = machine.isEmpty ? port : peerPorts[machine]
            guard let replyPort else {
                AppLog.agentMonitor.error(
                    "tool request from \(machine.isEmpty ? "the local hub" : machine, privacy: .public) dropped: no port for the hub holding it")
                return
            }
    ...
    private func mergeAgents(_ incoming: [AgentInfo]) {
        ...
        // NO ATTENTION TO PRUNE. It lived on the leaf, which goes when
        // the pane does ([[LeafFacts]], [[WI-2026-09-11-008]]).
    }
    ...
    nonisolated static func extractAgents(from dict: [String: Any]) -> [AgentInfo] {
        ...
        return agentArray.compactMap { obj -> AgentInfo? in
            ...
            // AN OPTIONAL, AND THE `??` IS DELIBERATELY ABSENT
            // ([[RFC-0011]] C-DEFAULT). "This peer's build does not report
            // unread mail" and "this agent has none waiting" are different
            // sentences, and defaulting the first to `false` here is the
            // misread the clause names — an agent with mail on an older
            // peer would look identical to one with none. The hub omits
            // the field and says why in the field beside it, and this side
            // keeps both ([[WI-2026-09-10-007]]).
            info.pendingWake = obj["pending_wake"] as? Bool
            info.pendingWakeUnknownCause = (obj["pending_wake_unknown_cause"] as? String)
                .flatMap(AgentInfo.UnknownCause.init(rawValue:))
            return info
        }
    }
}
