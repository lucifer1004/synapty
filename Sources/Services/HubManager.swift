...
@MainActor @Observable final class HubManager {
    ...
    struct HubInfo: Equatable {
        ...
        /// What THIS machine's hub provides — the list a peer's
        /// declaration is subtracted from.
        ///
        /// ASKED, NOT KEPT. The workbench held its own copy of this list,
        /// and the copy went stale the first time a capability was added
        /// on the Zig side: the hub-status popover then reported a peer as
        /// missing NOTHING while the agent tooltip beside it named the
        /// very thing that peer's build could not do. The list belongs to
        /// the build that implements it ([[WI-2026-09-10-010]]).
        ///
        /// EMPTY MEANS THE HUB DID NOT SAY, and a workbench that expects
        /// nothing can never report an absence — so the subtraction is
        /// skipped rather than answered "missing nothing".
