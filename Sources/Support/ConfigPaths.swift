...
enum ConfigPaths {
    ...
    static func resolveRoot(override: URL?, isTestHost: Bool,
        ...
        let expanded = Self.expandingLeadingTilde(raw, home: home)
        guard expanded.hasPrefix("/") else { return real }
        return URL(fileURLWithPath: expanded, isDirectory: true).standardizedFileURL
    }

    /// A LEADING TILDE, AND THE SAME ONE THE CLI EXPANDS.
    ///
    /// This was `expandingTildeInPath`, which also resolves `~someone`
    /// through the password database — and `src/paths.zig` expands neither
    /// form: it tests `raw[0] == '/'` and falls back to the REAL config
    /// root for anything else. So `SYNAPTY_CONFIG_ROOT=~/scratch` sent the
    /// workbench to the scratch root and every `synapty` process it
    /// started to the operator's own config, which is precisely the
    /// accident this variable exists to prevent and the one the comment
    /// above records costing a session file
    /// ([[WI-2026-09-11-001]]).
    ///
    /// `~/` AND BARE `~` ONLY, so the two sides accept the same set. A
    /// `~someone` form would need a password-database lookup on the CLI
    /// side to match, and no launch has ever wanted one.
    static func expandingLeadingTilde(_ raw: String, home: URL) -> String {
        if raw == "~" { return home.path }
        guard raw.hasPrefix("~/") else { return raw }
        return home.appendingPathComponent(String(raw.dropFirst(2))).path
    }
