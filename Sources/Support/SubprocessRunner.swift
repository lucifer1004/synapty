...
enum SubprocessRunner {
    ...
    static func run(
        executable: String,
        arguments: [String],
        /// Extra variables MERGED over the inherited environment. Used
        /// for configuration that has to reach further than the child —
        /// an option is spent on the process it is given to, while a
        /// variable is inherited by everything that process spawns.
        ...
        // LOSSY, NOT STRICT, AND THE DIFFERENCE IS ALL OR NOTHING.
        // `String(data:encoding:)` returns nil for a single malformed byte
        // anywhere in the stream, and the `?? ""` behind it then threw the
        // WHOLE output away — a command that answered in full would be read
        // as one that said nothing. The bytes here come off other people's
        // machines: `PortDiscovery` now carries a host's command lines, and
        // a file name in some other encoding is enough. Substituting U+FFFD
        // for the bad byte keeps every line around it
        // ([[WI-2026-09-07-001]]).
        return Output(
