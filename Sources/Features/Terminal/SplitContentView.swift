...
struct AllPanesSplitView: View {
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(paneManager.allLeaves.filter(\.content.isTerminal), id: \.id) { leaf in
                    if visibleIDs.contains(leaf.id),
                       paneManager.link(ofLeaf: leaf.id) != .live,
                       let frame = paneFrame(leaf.id) {
                        Rectangle()
                ...
                // AND SAYS WHY IT IS DIM. The scrim above is a state
                // without a sentence, and a pane greyed out for no stated
                // reason reads as one that is merely not focused
                // ([[RFC-0015]] C-FAILURE requires the reason;
                // [[WI-2026-09-07-004]]).
                //
                // FIRST OF THE STRIPS, because it is the condition that is
                // true NOW. The rejoin notice below it reports how this
                // pane came back, which is a fact about the past.
                ForEach(paneManager.allLeaves.filter(\.content.isTerminal), id: \.id) { leaf in
                    let state = paneManager.link(ofLeaf: leaf.id)
                    if visibleIDs.contains(leaf.id), state != .live,
                       let frame = paneFrame(leaf.id) {
                        LinkNoticeView(
                            state: state,
                            machine: paneManager.machineLabel(ofLeaf: leaf.id),
                            // WHICH ACT THE PRESS IS, ASKED OF THE STATE
                            // ([[LinkNoticeView.retryMeans]]). Waking a
                            // paused client and dialling a fresh
                            // connection are opposite things, and this
                            // sent both to the first one.
                            onRetry: {
                                switch LinkNoticeView.retryMeans(state) {
                                case .wakeTheClient: paneManager.retryLink(ofLeaf: leaf.id)
                                case .dialAgain, .none: onRetryLeaf?(leaf.id)
                                }
                            },
                            // DELIBERATELY, so the pane's resume plan and
                            // its armed bit go with it — `closeLeaf` alone
                            // takes the pane and leaves both behind.
                            onClose: { paneManager.closeLeafDeliberately(leaf.id) })
                        .frame(width: frame.width, height: PaneStrip.height)
                        .offset(x: frame.minX, y: PaneStrip.offsetY(frame, index: 0))
                    }
                }
                    ...
                    if visibleIDs.contains(leaf.id),
                       ...
                       let frame = paneFrame(leaf.id) {
                        ...
                        .offset(x: frame.minX,
                                y: PaneStrip.offsetY(
                                    frame,
                                    index: PaneStrip.rejoinIndex(
                                        link: paneManager.link(ofLeaf: leaf.id) == .live,
                                        hole: hole(on: leaf.id) != nil)))
                    }
                }

                // WHAT THIS PANE CANNOT SHOW ([[RFC-0015]] C-FAILURE).
                // The pane is working, and the screen on it is missing
                // output the session produced — a fact that reaches the
                // human on no other surface, because the only reader of
                // the dial's account is the placeholder this pane no
                // longer has ([[WI-2026-09-16-002]]).
                ForEach(paneManager.allLeaves.filter(\.content.isTerminal), id: \.id) { leaf in
                    if visibleIDs.contains(leaf.id),
                       let said = hole(on: leaf.id),
                       let frame = paneFrame(leaf.id) {
                        HoleNoticeView(said: said) {
                            paneManager.connectProgress.progress(for: leaf.id)?.dismissHole()
                        }
                        .frame(width: frame.width, height: PaneStrip.height)
                        .offset(x: frame.minX,
                                y: PaneStrip.offsetY(
                                    frame,
                                    index: PaneStrip.holeIndex(
                                        link: paneManager.link(ofLeaf: leaf.id) == .live)))
                    }
                }
    ...
    /// HOW MANY NOTICES SIT ABOVE THIS PANE, so the terminal is inset by
    /// all of them rather than by whichever view remembered to ask
    /// ([[PaneStrip]]).
    private func strips(on leafID: UUID) -> Int {
        PaneStrip.count(link: paneManager.link(ofLeaf: leafID) == .live,
                        hole: hole(on: leafID) != nil,
                        rejoining: paneManager.rejoinNotice(leafID) != nil)
    }

    /// WHAT THIS PANE CANNOT SHOW, or nil. Read from the dial's account
    /// rather than stored beside the pane: the client is what learns of a
    /// gap, and this is the channel it says so on.
    private func hole(on leafID: UUID) -> String? {
        paneManager.connectProgress.progress(for: leafID)?.hole?.text
    }
