...
@MainActor final class GhosttyApp {
    ...
    init() {
        ...
        runtime.action_cb = { app, target, action in
            ...
            // WHOSE SENTENCE THE HUMAN READS WHEN A PANE'S PROCESS GOES.
            //
            // Unhandled, libghostty prints its own into the terminal:
            // "Process exited. Press any key to close the terminal." That
            // is right for a local terminal, where the child IS the work.
            // For a remote pane the child is `synapty attach` — the thing
            // CARRYING the work — and its exit says nothing whatever about
            // the session on the far side. [[RFC-0014]] C-EXIT-SIGNAL
            // forbids exactly that inference, and this hook is what
            // libghostty offers instead of it.
            //
            // RETURNING TRUE MEANS THE WORKBENCH OWNS WHAT HAPPENS NEXT —
            // INCLUDING THE CLOSE. An earlier comment here asserted the
            // opposite, that upstream "falls through to `close()`
            // regardless". It does not, and `Surface.zig` had been read
            // the same day the sentence was written:
            //
            //     if (info.runtime_ms <= abnormal_command_exit_runtime_ms) {
            //         ...
            //         if (performAction(.show_child_exited, info)) return;  // no close
            //         ...
            //     }
            //
            // The threshold defaults to 250ms and Synapty does not set it,
            // and on Darwin the `exit_code == 0` escape above it is
            // compiled out — so EVERY pane whose child goes within a
            // quarter second takes that branch. Answering true there left
            // the surface stranded: `leafDidClose` never fired, so the
            // armed bit stayed set, the resume plan was never dropped
            // ([[RFC-0006]] C-RESUME-PLAN names pane close as its trigger)
            // and the leaf was never forgotten ([[WI-2026-09-07-009]]).
            //
            // SO THE CLOSE IS DECIDED HERE AND NOT WAITED FOR. Whether
            // ghostty calls `close()` afterwards no longer matters: if it
            // does, `leafDidClose` reaches the same answer, and closing a
            // leaf that is already gone is a no-op.
            case GHOSTTY_ACTION_SHOW_CHILD_EXITED:
                let surface = target.target.surface
                let code = action.action.child_exited.exit_code
                DispatchQueue.main.async {
                    guard let leafID = GhosttyApp.shared?.leafID(for: surface) else { return }
                    TerminalCoordinatorRef.instance?.leafChildExited(leafID, code: code)
                }
                return true
        ...
        runtime.close_surface_cb = { userdata, _ in
            ...
            // THE SAME HOP AS THE CHILD-EXIT CALLBACK ABOVE, deliberately.
            // These two arrive one after the other for the same pane and
            // their ORDER decides things; `Task { @MainActor in }` and
            // `DispatchQueue.main.async` both land on the main queue and in
            // practice preserve that order, but only one of them promises
            // FIFO against the other. Nothing downstream depends on the
            // order any more — the close decision reads the account from
            // disk, and an exit for a leaf no longer in the tree is
            // guarded — so this is hygiene: two mechanisms for two
            // callbacks whose sequence matters is a question that need not
            // exist ([[WI-2026-09-09-001]]).
            DispatchQueue.main.async {
