...
struct HostBlockView: View {
    ...
    // Test-connection state (WI-2026-08-08-045): nil = idle, otherwise
    // what the probe came to. It carries the REASON because a failure
    // that does not is one a human cannot act on ([[RFC-0015]]
    // C-FAILURE) — and because the reason was already in hand.
    @State private var testResult: TunnelManager.HostCommandOutcome?
    ...
    /// WHAT THE PROBE CAME TO, AND IT IS THE EXIT STATUS THAT SAYS SO.
    ///
    /// This asked `output.error == nil && !output.timedOut`, which is the
    /// LAUNCH check [[WI-2026-09-10-001]] already corrected in
    /// `TunnelManager.setupSucceeded` — and missed here. `/usr/bin/ssh`
    /// launches perfectly well and exits 255 for `Permission denied
    /// (publickey)`, `Host key verification failed`, an unresolvable
    /// hostname and a refused connection alike, setting neither field. So
    /// the one control a human presses to VALIDATE a host before relying
    /// on it showed a green tick for every failure it exists to detect,
    /// and the sentence they needed sat unread in `stderr`.
    ///
    /// The worst of it is the case this card is otherwise built around: a
    /// host synced from another Mac whose key did not travel
    /// ([[HostReadiness]]). The tick said the key warning beside it was
    /// stale. It was not.
    static func testOutcome(_ output: SubprocessRunner.Output)
        -> TunnelManager.HostCommandOutcome
    {
        if output.timedOut { return .failed("the host did not answer within 8 seconds") }
        if let e = output.error { return .failed(e) }
        // BatchMode is not a caveat here: `SSHPolicy.opening` puts it on
        // every connection this workbench makes, so a host that would
        // prompt for a password fails Connect the same way it fails this.
        guard (output.exitCode ?? 1) == 0 else {
            let said = output.stderr
                .split(separator: "\n")
                .map { $0.trimmingCharacters(in: .whitespaces) }
                .last(where: { !$0.isEmpty })
            return .failed(said ?? "the connection failed, and ssh said nothing about why")
        }
    ...
    private func testConnection() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let outcome = Self.testOutcome(output)
            // ONLY A RUN THAT SUCCEEDED DESCRIBES THE HOST. Parsing the
            // empty stdout of a failed probe and writing whatever it
            // yields into the record was the second half of this defect.
            var hint: String?
            if case .ok(let out) = outcome { hint = OSProbe.parse(out) }
    ...
    private var cardActions: some View {
        ...
        if tunnelStatus == .connected {
            ...
            //
            // `host-setup` compares and uploads, but the fast path does
