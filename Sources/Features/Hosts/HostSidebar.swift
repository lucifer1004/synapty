...
struct HostSidebar: View {
    ...
    private var runningAgents: [AgentInfo] {
        agentMonitor.agents
            ...
            // AND NOT ONE THIS WORKBENCH ENDED. Its session row going is
            // what would otherwise uncover this one, so ending something
            // would change a row's shape rather than remove it, until the
            // hub dropped the directory entry on its own clock
            // ([[WI-2026-09-21-002]]).
            .filter { !paneManager.confirmedEndedHere($0.bareID, onPeer: $0.machine) }
                    ...
                    LazyVStack(spacing: DS.Space.xxs) {
                        ...
                        ForEach(paneManager.archivedPanes) { row in
                            ArchivedPaneRow(
                                ...
                                // AWAITED, like the host row's End beside
                                // it: the row stays if the far side does
                                // not confirm ([[WI-2026-09-22-002]]).
                                onEnd: { Task { await paneManager.endArchivedPane(row.id) } })
                        }
    ...
    private var workspacesSection: some View {
        VStack(spacing: 0) {
                ...
                if paneManager.workspaces.isEmpty {
                    ...
                    LazyVStack(spacing: DS.Space.xxs) {
                        ForEach(Array(paneManager.orderedWorkspaces.enumerated()), id: \.element.id) { index, session in
                            ...
                            // THE LEAF'S RECORD, NOT A SECOND ONE KEYED BY
                            // AGENT. There were two, written in lockstep and
                            // cleared by different gestures: opening a
                            // waiting agent's pane from the go-to-pane
                            // palette cleared the leaf's and left this one
                            // set, so the row kept its mark on a pane the
                            // human was reading ([[WI-2026-09-11-008]]).
                            let attention = agent
                                .flatMap { paneManager.leafShowing(agent: $0.id) }
                                .map { paneManager.isAwaitingAttention($0) } ?? false
                            WorkspaceRow(
                                session: session,
                                ...
                                onTap: {
                                    ...
                                    if let agent,
                                       paneManager.leafShowing(agent: agent.id) != nil {
                                        // `focusAgent` reveals the leaf,
                                        // and revealing clears its mark —
                                        // the second clear was for the
                                        // second record ([[WI-2026-09-11-008]]).
            ...
            .task {
                ...
                // AND THIS MACHINE'S OWN, for the archived rows that name
                // a local session — the only rows in this list made of
                // purely local state, and so the only ones nothing was
                // ever comparing with reality ([[WI-2026-09-22-001]]).
                paneManager.refreshLocalSessions()
...
struct WorkspaceRow: View {
    ...
    private var machineSummary: String {
        let names = paneManager.connections(ofWorkspace: session).map { connection -> String in
            ...
            return HostStore.inSentence(host)
        }
    ...
    ///
    /// WHAT SURVIVES THE MOVE IS WHAT THE WORKSPACE ITSELF KNOWS: that
    /// something in here needs you, or that something in here broke. Which
    /// of the two, and why the order is not the obvious one, is
    /// [[WorkspaceStatus]]'s to say — this row asks and draws.
    ///
    /// IT ALSO USED TO CARRY THE MACHINE'S COLOUR, and that was the
    /// container answering "which machine is this?" one rework after the
    /// address was taken off this row for exactly that reason
    /// ([[WI-2026-09-07-003]]).
    private var status: WorkspaceStatus {
        WorkspaceStatus(archived: session.isArchived,
                        // `workspaceNeedsAttention` already covers every
                        // pane in this workspace, the agent's included, so
                        // the second operand only ever said the same thing
                        // twice ([[WI-2026-09-11-008]]).
                        attention: needsAttention,
                        // A LINK THAT DROPPED MID-SESSION IS A FAILURE THE
                        // SIDEBAR OWES THE HUMAN, and it was invisible
                        // here: `hasFailedConnection` reads the
                        // CONNECTION's state, and a pane that lost its
                        // transport is a fact about the pane's own account
                        // — the host may be perfectly reachable, and every
                        // other pane on it still working. [[RFC-0015]]
                        // C-FAILURE: the workbench MUST NOT require the
                        // human to open a workspace to learn that
                        // something in it failed ([[WI-2026-09-07-004]]).
                        // ONE PREDICATE, AND IT IS THE ONE THAT CAN SAY
                        // WHY. The dot spelled the disjunction out longhand
                        // while the reason text below asked
                        // `failureReason`, which already answers for both
                        // halves — and the two HAVE diverged:
                        // [[WorkspaceStatus]] records that widening the
                        // mark to include a pane's own link without
                        // widening the reason left "a workspace whose only
                        // trouble was a dropped pane link wore a red dot
                        // with no reason anywhere and said nothing at all
                        // to a screen reader" ([[WI-2026-09-07-004]],
                        // [[WI-2026-09-07-007]]). That fix restored the
                        // premise and left the shape that produced it
                        // ([[WI-2026-09-11-008]]).
                        failed: failure != nil)
    }

    /// WHY THIS WORKSPACE IS IN TROUBLE, asked once per body.
    ///
    /// The dot, the sentence and the screen reader all read this, so they
    /// cannot come to disagree — and it is cheaper as well as truer: the
    /// row used to walk the workspace four times, twice for the dot and
    /// once each for the other two.
    private var failure: String? {
        paneManager.failureReason(session, now: now)
    }
        ...
        return HStack(spacing: DS.Space.sm) {
            ...
            Group {
                if status == .archived {
                    ...
                    DSStatusDot(color: status.color, size: DS.scaled(status.dotSize),
                                pulsing: status.pulses)
                }
            }
            // A FIXED SLOT, so a quiet row, an alerting one and an
            // archived one all start their label in the same column. The
            // dot changes size with what it has to say, and the archive
            // glyph is wider than either; without this the labels stepped.
            ...
            // NO SECOND DOT HERE. A bell in this workspace is what the
            // leading dot says, agent or not ([[WI-2026-09-07-003]]).
            // This drew a pulsing amber dot of its own, and a row with a
            // waiting agent carried three unlabelled dots.
            ...
            if let agent {
                Image(systemName: agent.tool.sfSymbol)
                    ...
                    .help([
                        ...
                        agent.pendingWakePhrase,
                    ].compactMap { $0 }.joined(separator: " — "))
                    .accessibilityLabel(agent.tool.displayName)
                // AND NO THIRD ONE. The agent icon still turns amber,
                // because that says WHICH agent wants you — a thing the
                // leading dot cannot say and this glyph already does.
            }
    ...
    private var accessibilityDescription: String {
        ...
        return [session.label, machines, status.spoken,
...
private struct RemoteSessionRow: View {
    ...
    private var title: String {
        // A SESSION THAT DID NOT ANSWER HAS NO DIRECTORY TO NAME, and a
        // row titled with whatever arrived in that column is how a pid
        // came to be shown as the folder the work was in
        // ([[WI-2026-09-07-005]]). The far side now sends `-` there, and
        // this refuses the column outright rather than trusting it to —
        // the machine on the other end is not necessarily this build.
        guard !session.unreachable, let directory = session.directory else {
            return "\(machine) · \(session.name)"
        }
        let leaf = (directory as NSString).lastPathComponent
