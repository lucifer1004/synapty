...
struct ContextStatusBar: View {
    ...
    /// WHAT THE STRIP SAYS ABOUT THE HUB, from the state itself.
    ///
    /// REFUSED IS NOT DOWN ([[WI-2026-09-04-001]]). A hub of a build this
    /// workbench may not adopt IS running, and calling that "down" sent
    /// the human to wait or restart — the two things that cannot resolve
    /// it. This said "Hub: refused" and stopped there, while
    /// `HubStatus.refusalReason` — written to carry the words
    /// `decideAdoption` chose, and read by nothing anywhere — sat unused
    /// ([[WI-2026-09-11-011]]). The reason is what tells them which build
    /// is in the way.
    ///
    /// A SWITCH, NOT A LADDER OF BOOLEANS. Three `if case` wrappers
    /// rebuilt a switch the enum already has, and a seventh state would
    /// have fallen through them into "down".
    static func hubPhrase(_ status: HubManager.HubStatus, agents: Int) -> String {
        switch status {
        case .running:
            return "Hub: \(agents) agent\(agents == 1 ? "" : "s")"
        case .lost:
            return "Hub: restarting…"
        case .conflict(let why):
            return "Hub: refused — \(why)"
        case .starting:
            return "Hub: starting…"
        case .stopped:
            return "Hub: down"
        case .failed(let why):
            // THE REASON HERE TOO, for the same reason `.conflict` carries
            // one: "down" is what a crash looks like, and it sends the
            // human to wait or restart. Three of the four things that
            // reach this state are not fixed by either.
            return "Hub: down — \(why)"
        }
    }

    /// AMBER WHILE IT MAY STILL RESOLVE ITSELF, red once it will not — and
    /// a refused hub is red, because nothing this workbench does resolves
    /// it.
    static func hubTint(_ status: HubManager.HubStatus) -> Color {
        switch status {
        case .running: return DS.success
        case .lost, .starting: return DS.warning
        case .stopped, .failed, .conflict: return DS.danger
        }
    }
                ...
                if let agentID = leafAgent,
                   let agent = agentMonitor.agents.first(where: {
                    ...
                    Image(systemName: agent.tool.sfSymbol)
                        ...
                        // AND WHETHER IT HAS MAIL NOBODY ANNOUNCED
                        // ([[RFC-0011]] C-DEFAULT). On a workbench-less
                        // hub no wake is ever attempted, so this row is
                        // the only carrier that fact has
                        // ([[WI-2026-09-10-007]]).
                        .help([agent.unknownExplanation, agent.pendingWakePhrase]
    ...
    private var hubSummary: some View {
        ...
        Button {
            ...
            HStack(spacing: DS.Space.xs) {
                DSStatusDot(
                    color: Self.hubTint(hubManager.status),
                    size: 6
                )
                ...
                // SWITCHED ON THE STATE, NOT SPELLED OUT IN BOOLEANS.
                // Three `if case` wrappers rebuilt a switch the enum
                // already has, and the reason it mattered is on the next
                // line: this printed the bare word "refused" while
                // `refusalReason` — written to carry exactly that, and
                // read by nothing — sat beside it ([[WI-2026-09-11-011]]).
                Text(Self.hubPhrase(hubManager.status, agents: count))
                    .font(DS.Typography.caption)
                    .foregroundStyle(DS.textSecondary)
            }
            .padding(.horizontal, DS.Space.sm)
        ...
        // NOT "logs". The popover deliberately has none — it is a look
        // and not a control surface, and it says so: logs are Console and
        // `just hub-log` ([[HubStatusPopover]]). A tooltip promising a
        // third thing the popover does not contain, and not saying where
        // it is instead, is a dead end with extra steps.
        .help("Hub service — status and the agents registered here")
