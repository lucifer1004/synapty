...
@MainActor @Observable final class ConnectionRegistry {
    ...
    ///
    /// STATED WHERE AN OPERATOR CAN READ IT, which C-RELEASE requires and
    /// a source comment does not satisfy. `Settings → Network` prints this
    /// value, and prints THIS value rather than a second copy of the
    /// number ([[WI-2026-09-10-005]]).
    static let defaultGrace: TimeInterval = 30

    var grace: TimeInterval = ConnectionRegistry.defaultGrace
