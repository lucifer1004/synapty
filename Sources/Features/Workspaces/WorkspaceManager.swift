...
@MainActor @Observable final class WorkspaceManager: TerminalCoordinator {
    ...
    func leafDidClose(_ leafID: UUID) {
        ...
        //
        // READ FROM THE FILE, NOT FROM THE ACCOUNT OBJECT: the poll that
        // feeds the object cannot be relied on to have run before this
        // call, which IS the client's exit arriving. That is
        // `keepsPaneNow`'s job now, for every ending rather than this one
        // ([[WI-2026-09-07-011]]) — it recognises the displacement, sets
        // the flag, and answers that the pane stays.
        // AND NEITHER DOES ANY OTHER PANE WHOSE WORK OUTLIVED ITS
        // CARRIER. [[RFC-0014]] C-EXIT-SIGNAL: a client "MUST NOT end,
        // close, or discard the session's local state" on the strength of
        // a lost connection, and its rationale names the failure exactly —
        // "the safe guess — treat every drop as a death — is the behaviour
        // that makes a network blip destroy a pane".
        //
        // THE SCREEN IS WHY IT MATTERS. It is the last true thing the
        // session said, usually the output the human was looking for, and
        // it lives exactly as long as this surface does. Closing to save a
        // row costs them the reason they were looking
        // ([[WI-2026-09-07-004]]).
        if keepsPaneNow(leafID) { return }
        closeLeafDeliberately(leafID)
    }

    /// A CLOSE SOMEBODY DECIDED ON, and so one with no guards.
    ///
    /// THE TWO ARE NOT THE SAME ACT. `leafDidClose` above is the SURFACE's
    /// process going, which is a fact about a process and only sometimes a
    /// reason to take the pane away. This is the human pressing a close,
    /// or the workbench closing a pane it opened for its own purposes —
    /// the decision is already made and asking again would refuse it.
    ///
    /// It was one method, and `closeExecPane` reached it: an exec pane
    /// whose command failed would have been kept open by a guard meant for
    /// a remote session, and the workbench would have leaked one per
    /// failure ([[WI-2026-09-07-004]]).
    func closeLeafDeliberately(_ leafID: UUID) {
    ...
    /// WHY THIS WORKSPACE IS MARKED, in words — a CONNECTION that failed
    /// to dial, or a PANE whose link or session did.
    ///
    /// [[RFC-0015]] C-FAILURE: "A failure MUST carry the reason it
    /// failed." The sidebar had one source for that reason,
    /// `firstFailure`, and it walks CONNECTIONS. That was the whole of
    /// what could mark a row until [[WI-2026-09-07-004]] widened the mark
    /// to include a pane's own `LinkState` — and widened it WITHOUT
    /// widening this, so a workspace whose only trouble was a dropped
    /// pane link wore a red dot with no reason anywhere and said nothing
    /// at all to a screen reader.
    ///
    /// WORSE, IT FALSIFIED THE ARGUMENT THAT LET ATTENTION TAKE THE DOT.
    /// [[WorkspaceStatus]] puts attention ahead of failure on the grounds
    /// that "a failure ALSO puts its reason on the right of the row in
    /// red" — true while `failed` meant a failed connection, and false the
    /// moment it meant a pane link as well. The premise is restored here
    /// rather than the precedence being changed, because the premise is
    /// the thing that was broken ([[WI-2026-09-07-007]]).
    ///
    /// THE CONNECTION'S REASON LEADS. A host that could not be dialled at
    /// all explains every pane on it, and reporting one of those panes
    /// instead would name a symptom over its cause.
    func failureReason(_ session: Workspace, now: Date = Date()) -> String? {
        if let why = firstFailure(session) { return why }
        for pane in session.panes {
            let state = link(ofLeaf: pane.id)
            guard state.needsSidebarMark(now: now),
                  let said = state.sentence(now: now, on: machineLabel(ofLeaf: pane.id))
            else { continue }
            return said
        }
        return nil
    ...
    func paneDidConnect(_ leafID: UUID, command: String, agentID: String?) {
        ...
        // A NEW CHILD IS NOT THE OLD ONE'S EXIT. The code was written once
        // and cleared nowhere, and `LinkState.from` consults it ahead of
        // everything the account says — so a pane that lost its link once
        // and was then re-dialled read as `dropped` for the rest of its
        // life, whatever it went on to do ([[WI-2026-09-07-009]]).
        facts[leafID, default: .init()].childDied = false
    ...
    /// Whether this leaf's session outlives its client, read from the
    /// same two settings the spawn path reads and without asking the
    /// surface — so a leaf whose pane is not on screen answers too.
    ...
    private func leafIsOn(machineIn qualified: String, leaf: UUID) -> Bool {
        ...
        return HostStore.inSentence(host).lowercased()
            == machine.lowercased()
    }
    ...
    /// WHAT THIS LEAF'S LINK AND SESSION ARE DOING ([[LinkState]]).
    ///
    /// A LEAF'S, NOT A CONNECTION'S. The host's link is shared by every
    /// pane on it and lives in `connections`; this is the pane's own
    /// transport and the session behind it, and only the pane's account
    /// knows about either ([[RFC-0015]] C-FAILURE puts a connection's state
    /// on the leaves that reference it, and this is the half that was
    /// never there to put).
    func link(ofLeaf leafID: UUID) -> LinkState {
        link(ofLeaf: leafID, fromDisk: false)
    }

    /// `fromDisk` reads the account's ending from the FILE instead of from
    /// the polled snapshot, for the callers that are deciding rather than
    /// drawing.
    ///
    /// DRAWING MUST NOT READ THE DISK: this is reached from view bodies,
    /// several times a frame, for every pane. DECIDING MUST NOT READ THE
    /// POLL: the decisions below are taken at the instant the client
    /// exits, which is before the quarter-second poll can be relied on to
    /// have run — so a session that had just written `end no_session`
    /// looked like an ordinary exit and its pane was closed, which is the
    /// exact outcome the whole of [[WI-2026-09-07-004]] exists to prevent
    /// ([[WI-2026-09-07-011]]).
    ///
    /// ONLY `ended` COMES FROM DISK, AND WHY THAT IS SAFE RATHER THAN
    /// LUCKY. `lostSince` and `pausedSince` still come from the poll, and
    /// the one caller that asks `fromDisk` is deciding `keepsPane` — which
    /// is true only for `gone`, `mismatch`, `taken` and `dropped`, and
    /// every one of those derives from `ended` or from the exit code set
    /// microseconds earlier. The two states the poll feeds, `lost` and
    /// `paused`, both answer FALSE, which is the same answer as `live`. So
    /// a stale poll can only move the result between two values that are
    /// already identical here.
    ///
    /// IT WOULD STOP BEING SAFE THE MOMENT A SECOND CALLER ASKED. Said
    /// here rather than left as "harmless today", because "harmless today"
    /// is an invitation to reuse ([[WI-2026-09-08-011]]).
    private func link(ofLeaf leafID: UUID, fromDisk: Bool) -> LinkState {
        // A DEAD CHILD MEANS SOMETHING ONLY WHERE THERE IS A LINK TO LOSE.
        //
        // `dropped` exists for a host that keeps no session between
        // connections, where ssh IS the pane's child and a dead link kills
        // it with no account of itself. On THIS Mac the pane's child is
        // the human's shell, and a shell exiting is the ordinary end of a
        // terminal. Fed to the same rule it read as `dropped`, which keeps
        // the pane: so typing `exit` stopped closing a terminal, and the
        // workspace wore a red mark about a connection to this Mac
        // ([[WI-2026-09-08-016]]).
        // AND ONLY WHERE A DEATH IS EVIDENCE OF ANYTHING. On a durable
        // host the client writes an `end` before it goes, so a death with
        // no account is the client itself dying. On a host that keeps no
        // session ([[RFC-0014]] C-OPT-OUT) the transport IS the child, and
        // a dropped link and a human typing `exit` arrive identically —
        // the exit code that was supposed to separate them is always zero
        // ([[LinkState]] `dropped`). Guessing there would leave a dead
        // pane open every time somebody left a shell.
        let died = facts[leafID]?.childDied == true
            && host(ofLeaf: leafID)?.durableSessions == true
        guard let account = connectProgress.progress(for: leafID) else {
            return LinkState.from(ended: nil, lostSince: nil, pausedSince: nil, childDied: died)
        }
        return LinkState.from(ended: fromDisk ? account.accountEnd() : account.ended,
                              lostSince: account.lostSince,
                              pausedSince: account.pausedSince,
                              childDied: died)
    }

    /// Whether this pane must survive its process going, asked the way a
    /// decision has to ask it — AND THE RECORD THAT GOES WITH THE ANSWER.
    ///
    /// `surface(of:)` reads `takenByAnother` to draw the surface that says
    /// somebody took this session, and that flag used to be set on ONE of
    /// the two paths into this question. So a pane kept by
    /// `leafChildExited` survived without the one thing that explains why
    /// it is still there, and the promise made to the OTHER human in the
    /// displacement dialog — "they will be told who took it" — was kept
    /// only when libghostty happened to call close as well
    /// ([[WI-2026-09-08-002]]).
    ///
    /// RECORDING IT HERE RATHER THAN AT EACH CALLER is the point: two
    /// callers each remembering to write it is how one of them forgets.
    private func keepsPaneNow(_ leafID: UUID) -> Bool {
        let state = link(ofLeaf: leafID, fromDisk: true)
        if state == .taken { facts[leafID, default: .init()].takenByAnother = true }
        return state.keepsPane
    }

    /// THE PANE'S OWN PROCESS EXITED, and the workbench heard it directly
    /// rather than letting libghostty write a sentence about it.
    ///
    /// libghostty's fallback — "Process exited. Press any key to close the
    /// terminal." — is written for a local terminal where the child IS the
    /// work. For a remote pane the child is the thing CARRYING the work,
    /// and saying its exit is the work's end is precisely what
    /// [[RFC-0014]] C-EXIT-SIGNAL forbids ([[WI-2026-09-07-004]]).
    /// THE CODE IS TAKEN AND DISCARDED, DELIBERATELY. libghostty hands one
    /// over and on macOS it is always 0 — every pane is spawned through
    /// `/usr/bin/login`, which reports 0 for a child that exits 42, is
    /// killed, or does not exist ([[LinkState]] `dropped` carries the
    /// measurements). The parameter stays because it is the callback's
    /// shape; what stops is believing it ([[WI-2026-09-09-001]]).
    func leafChildExited(_ leafID: UUID, code _: UInt32) {
        // A LEAF THAT IS GONE GETS NO NEW FACTS. `facts[…, default: .init()]`
        // CREATES an entry, so arriving after the pane has closed would
        // resurrect one keyed to a leaf nothing holds — permanently, since
        // `forget` has already run and will not run again. That is the
        // failure [[LeafFacts]]'s header opens with, and this handler and
        // `leafDidClose` are two callbacks whose order is not ours to fix
        // ([[WI-2026-09-08-011]]).
        guard workspaceIndex(containing: leafID) != nil else { return }
        facts[leafID, default: .init()].childDied = true
        // AND THE CLOSE IS DECIDED HERE, not left to libghostty. Taking
        // the action means it may never call `close()` at all — the
        // branch it takes for a child that went within 250ms returns
        // instead — so a pane that should go would have been stranded
        // with its armed bit set and its resume plan never dropped
        // ([[GhosttyApp]], [[WI-2026-09-07-009]]).
        //
        // THE SAME ANSWER EITHER WAY. `leafDidClose` asks this same
        // question when the surface does call close, and closing a leaf
        // that has already gone is a no-op — so the two paths cannot
        // disagree and cannot both do damage.
        if !keepsPaneNow(leafID) { closeLeafDeliberately(leafID) }
    }

    /// THE MACHINE THIS LEAF IS ON, NAMED. Asked of the leaf and never of
    /// its container, which is [[RFC-0015]] C-LEAF-BINDING's whole point: a
    /// workspace may hold panes on three machines and has no single one to
    /// answer with.
    func machineLabel(ofLeaf leafID: UUID) -> String {
        // MID-SENTENCE: this feeds [[LinkState]]'s sentences — "The link to
        // X dropped" — so it takes the owner's in-sentence form rather
        // than a fourth copy of the ternary ([[WI-2026-09-11-010]]).
        HostStore.inSentence(host(ofLeaf: leafID))
    }

    /// TELL A PAUSED CLIENT TO DIAL AGAIN.
    ///
    /// A MARKER BESIDE THE ACCOUNT, not a keystroke into the pane. The
    /// workbench CAN type into a pane, and that was the obvious way to
    /// wake a paused client — but what a keystroke means depends on what
    /// receives it ([[RFC-0006]] C-RESUME-RESTORE makes exactly that
    /// argument), and a button pressed at the instant the link came back
    /// on its own would put a stray byte into the human's shell. The
    /// paused client polls for this file and takes it
    /// ([[progress.RetrySignal]], [[WI-2026-09-07-004]]).
    /// ADDRESSED BY THE NAME THE CHANNEL WAS OPENED UNDER, which the
    /// account itself knows. `facts[leaf].agent` answers a different
    /// question and drifts from this one — the hub renames it to the
    /// durable id ([[RFC-0008]], `remapLeafAgent`) — so keying the marker
    /// off it left permission in a path nothing polls and the button did
    /// nothing ([[WI-2026-09-08-016]]).
    func retryLink(ofLeaf leafID: UUID) {
        guard let agentID = connectProgress.progress(for: leafID)?.agentID
                ?? facts[leafID]?.agent ?? facts[leafID]?.candidateAgent
        else { return }
        ConnectProgress.requestRetry(for: agentID)
    }
    ...
    func tabTooltip(for pane: SplitNode.Pane) -> String {
        ...
        // AND WHICH MACHINE. The tab says that in a colour along its edge
        // ([[HostSpine]]), which is a hue nobody can look up — so the one
        // place it can be READ is the hover this tab already has. It used
        // to hang off a 6pt dot, which is a target and not an answer.
        if let host = host(ofLeaf: pane.id)?.label { name += " · on \(host)" }
    ...
    private func endAgentAwaiting(_ agentID: String, host: HostEntry?) async -> Bool {
        guard let started = startEnding(agentID, host: host) else {
            return confirmedEnded(agentID, onHost: host?.id)
        }
    ...
    private func startEnding(_ agentID: String, host: HostEntry?) -> Task<Bool, Never>? {
        let key = AgentOnMachine(name: agentID, host: host?.id)
        if endedAgents.contains(key) { return nil }
        guard endingAgents.insert(key).inserted else { return nil }
    ...
    private func settleEnding(_ agentID: String, host: HostEntry?,
        ...
        endingAgents.remove(AgentOnMachine(name: agentID, host: host?.id))
        guard ended else { return false }
        // THE ROW GOES WHEN THE SESSION DOES, and not a moment before.
        // Recording it on the way out made a failed end look like a
        // successful one until the next poll put the row back.
        //
        // RECORDED, NOT SUBTRACTED HERE. This used to reach into
        // `reportedSessions` and remove the row, which a poll already in
        // flight then put straight back — the store is a host's last
        // answer and nothing this side edits into it survives the next
        // one. `remoteSessions` subtracts on READ, which is where the
        // same question about open and archived panes is already asked
        // and for the same reason ([[WI-2026-09-21-002]]).
        noteAgentEnded(agentID, onHost: host?.id)
        return true
    }
    ...
    /// A SESSION NAME IS ONLY A NAME ON A MACHINE ([[RFC-0008]]
    /// C-IDENTITY): they come from a four-hex namespace, so two hosts can
    /// hold namesakes. `shown` has been scoped this way from the start and
    /// the two sets below were not — so ending `local-1a2b` on one host
    /// marked the namesake on another as ended too, and pressing End on it
    /// returned `true` having asked nobody anything
    /// ([[WI-2026-09-21-002]]).
    private struct AgentOnMachine: Hashable {
        let name: String
        /// nil is this Mac.
        let host: UUID?
    }
    ...
    private var endedAgents: Set<AgentOnMachine> = []
    ...
    private var endingAgents: Set<AgentOnMachine> = []

    /// THE FAR SIDE CONFIRMED THIS NAME IS GONE, and this workbench may
    /// stop offering it whatever a poll says next.
    ///
    /// A POLL THAT WAS ALREADY IN FLIGHT CARRIES A LISTING FROM BEFORE THE
    /// END. `noteRemoteSessions` assigns a host's rows WHOLESALE, so that
    /// answer put the row back after the end had taken it away — both are
    /// ssh round trips and the poll's is bounded at ten seconds, so the
    /// overlap is ordinary rather than rare. It came back until a later
    /// poll disagreed, and if the host's connection was released in the
    /// meantime there is no later poll: `refreshRemoteSessions` iterates
    /// the CONNECTED hosts, and nothing else ever writes that key again
    /// ([[WI-2026-09-21-002]]).
    ///
    /// INTERNAL SO A TEST CAN SAY IT, because the thing that says it in
    /// production is the tail of two ssh calls.
    func noteAgentEnded(_ name: String, onHost hostID: UUID?) {
        endedAgents.insert(AgentOnMachine(name: name, host: hostID))
    }

    /// Whether this workbench has been told the far side ended this name.
    private func confirmedEnded(_ name: String, onHost hostID: UUID?) -> Bool {
        endedAgents.contains(AgentOnMachine(name: name, host: hostID))
    }
    ...
    private func stepThroughStack(by delta: Int) {
        guard let sIdx = activeWorkspaceIndex,
              ...
              // THE NAME, NOT THE COUNT. `isStacked` carries a warning
              // this expression cannot — it is not "the tab bar is drawn",
              // and the bar IS drawn for a single pane because the tab is
              // what the human grabs ([[WI-2026-09-11-011]]).
              slot.isStacked,
    ...
    /// WHETHER THIS SESSION IS STILL ON THAT MACHINE, answered from what
    /// is already here rather than with another round trip
    /// ([[WI-2026-09-22-002]]).
    ///
    /// IGNORANCE KEEPS THE ROW, in both directions. A host nothing has
    /// ever listed, and a local claim this process could not read, are
    /// both "I cannot tell" — and the cost of being wrong that way is
    /// litter, where the other way is telling a human their work is gone.
    private func sessionStillThere(_ agent: String, on host: HostEntry?) -> Bool {
        guard let host else {
            // `free` is a holder that has gone and `absent` is a name no
            // holder reached; only those two are tombstones
            // ([[SessionRecord.Claim]]).
            switch SessionRecord.claim(agent) {
            case .free, .absent: return false
            case .held, .unknown: return true
            }
        }
        guard let listed = reportedSessions[host.id] else { return true }
        return listed.contains { $0.name == agent }
    }
    ...
    private(set) var reportedSessions: [UUID: [RemoteSessions.Session]] = [:]

    /// … MINUS WHAT THIS WORKBENCH ALREADY SHOWS, ASKED RATHER THAN
    /// RECORDED.
    ///
    /// The subtraction used to happen once, in `noteRemoteSessions`, and
    /// the result was what everything read. So it was only ever as fresh
    /// as the last poll — sixty seconds, on the sidebar's ticker — and a
    /// session the human PICKED UP went on being offered as somewhere to
    /// return to for the rest of that minute, with its pane open in front
    /// of them ([[WI-2026-09-07-002]]).
    ///
    /// Nothing was missing at pick-up in particular; every transition that
    /// moves a name across this line had the same gap, and `settleEnding`
    /// had already been patched by hand for one of them. Computing it here
    /// costs a filter over two small lists and needs no trigger, no extra
    /// state and no round trip to a host to learn something this side
    /// already knows.
    ///
    /// A READ, AND ONLY A READ. `shown` asks; it must never record. A view
    /// body that mutates `facts` invalidates every view that read it,
    /// including itself ([[WI-2026-08-29-001]]).
    ///
    /// EVERY KEY SURVIVES, including one whose rows all fell away: a host
    /// that answered "nothing" is a different fact from a host that never
    /// answered, and the sidebar shows the step between them.
    /// AND WHAT THIS SIDE HAS ALREADY ENDED, which the store cannot be
    /// trusted to have dropped: a poll in flight when the end confirmed
    /// carries a listing from before it, and `noteRemoteSessions` assigns
    /// a host's rows wholesale ([[noteAgentEnded]], [[WI-2026-09-21-002]]).
    var remoteSessions: [UUID: [RemoteSessions.Session]] {
        reportedSessions.reduce(into: [:]) { out, entry in
            out[entry.key] = entry.value.filter {
                !shown($0.name, onHost: entry.key)
                    && !confirmedEnded($0.name, onHost: entry.key)
            }
        }
    }
    ...
    func refreshRemoteSessions() {
        for connection in connections.connections {
            ...
            // WHEN THE QUESTION WAS ASKED, carried with it. What comes
            // back describes the host at THIS instant, not at the one it
            // arrives in ([[noteRemoteSessions]]).
            let asOf = Date()
            Task.detached(priority: .utility) {
                ...
                await MainActor.run {
                    ...
                    self.noteRemoteSessions(rows, for: host, asOf: asOf)
                }
            }
        }
    }
    ...
    /// WHEN THE QUESTION BEHIND EACH VERDICT WAS ASKED ([[noteHostBinary]]).
    private var hostBinaryAsOf: [UUID: Date] = [:]

    /// RECORD WHAT A HOST SAID IT RUNS, unless something asked it later
    /// has already answered ([[WI-2026-09-23-006]]).
    ///
    /// TWO THINGS ASK, AND NEITHER COULD SEE THE OTHER. The minute's poll
    /// and the human's Update each run an ssh round trip and each wrote the
    /// verdict when it returned. A poll that left before an update landed
    /// after it and put `stale` back over `current`: the host said it was
    /// behind right after being brought up to date, and offered the Update
    /// button — another scp — for the minute until the next poll. The same
    /// shape as a stale session listing ([[WI-2026-09-21-002]]), answered
    /// the same way: by when the question was asked, not when the answer
    /// arrived.
    func noteHostBinary(_ verdict: HostBinary.Verdict, for hostID: UUID, asOf: Date) {
        if let newer = hostBinaryAsOf[hostID], newer > asOf { return }
        hostBinary[hostID] = verdict
        hostBinaryAsOf[hostID] = asOf
    }
        ...
        for connection in connections.connections {
            ...
            Task.detached(priority: .utility) {
                ...
                await MainActor.run {
                    ...
                    self.noteHostBinary(verdict, for: host.id, asOf: asOf)
                }
            }
        }
    }
    ...
    ///
    /// EXPLICIT, BECAUSE NOTHING ELSE WILL. `host-setup` compares and
    ...
    func updateHostBinary(_ host: HostEntry) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            // WHEN IT WAS ASKED, which is after the upload — so a poll that
            // left before the upload cannot overwrite what this finds.
            let askedAt = Date()
            ...
            await MainActor.run {
                ...
                if ok {
                    self.noteHostBinary(HostBinary.verdict(remote: theirs, local: expected),
                                        for: host.id, asOf: askedAt)
                }
            }
        }
    }

    /// RECORD WHAT A HOST ANSWERED, WHOLE.
    ///
    /// WHAT THIS WORKBENCH SHOWS IS SUBTRACTED WHEN THE LIST IS READ, not
    /// here. This list is for holders no pane here names — a crashed run,
    /// another Mac's work, a session from before a reinstall. Everything a
    /// workspace is holding is reported by its host as well, so it arrives
    /// from both sides, and the row here is the one that stays: an open
    /// pane is its own tab and an archived pane is a row that knows the
    /// title, the workspace it came from and that returning puts it back,
    /// where the host's row knows only a name.
    ///
    /// Subtracting here instead froze the answer until the next poll —
    /// see `remoteSessions` ([[WI-2026-09-07-002]]).
    /// `asOf` IS WHEN THE LISTING WAS TAKEN, not when it arrived. A
    /// listing can only speak about what existed when it was asked for,
    /// and both things below act on what it does NOT name
    /// ([[WI-2026-09-22-001]]).
    func noteRemoteSessions(_ rows: [RemoteSessions.Session], for host: HostEntry,
                            asOf: Date = Date()) {
        reportedSessions[host.id] = rows
        // THE MARK IS A BRIDGE, NOT A GRAVESTONE. `noteAgentEnded` exists
        // to outlast a poll that was in flight when the end confirmed;
        // once a listing arrives that no longer names it, the host and
        // this side agree and the mark has done its work. Kept forever it
        // would hide a NEW session that happened to mint the same name on
        // the same host — four hex digits is not many.
        let listed = Set(rows.map(\.name))
        endedAgents = endedAgents.filter { $0.host != host.id || listed.contains($0.name) }
        reconcileArchivedPanes(against: listed, onHost: host.id, asOf: asOf)
    }

    /// WHICH OF THIS MACHINE'S ARCHIVED ROWS ARE STILL HELD. The local
    /// answer to the same question the hosts are polled for.
    ///
    /// ASKED OF THE ROWS, NOT OF THE DIRECTORY. `SessionRecord.live()`
    /// would answer this in one read and SWEEPS what it finds dead on the
    /// way — that is the CLI's job and not a thing a sidebar tick should
    /// do behind the human. Asking the claim of each row this list
    /// actually holds is a few syscalls a minute and removes nothing from
    /// disk.
    func refreshLocalSessions() {
        let asOf = Date()
        let names = workspaces.flatMap(\.archivedPanes)
            .filter { $0.host == nil }
            .compactMap(\.agent)
        guard !names.isEmpty else { return }
        Task.detached(priority: .utility) {
            // A QUESTION THAT COULD NOT BE ASKED LEAVES THE ROW
            // ([[SessionRecord.Claim]]): only `free` and `absent` are
            // tombstones, and `unknown` is this process being unable to
            // look. A stale row is litter; a row removed on a failed read
            // is work the human is told they no longer have.
            let held = names.filter { SessionRecord.claim($0) != .free
                                        && SessionRecord.claim($0) != .absent }
            await MainActor.run { self.noteLocalSessions(held, asOf: asOf) }
        }
    }

    func noteLocalSessions(_ names: [String], asOf: Date = Date()) {
        reconcileArchivedPanes(against: Set(names), onHost: nil, asOf: asOf)
    }

    /// A ROW FOR WORK THAT IS NO LONGER RUNNING IS THE ONE THING THIS LIST
    /// MUST NOT SHOW ([[RFC-0014]] C-END, [[WI-2026-09-22-001]]).
    ///
    /// An archived pane is the only row in Still Running made of purely
    /// local state: it is written when the human puts a pane away and read
    /// back from the workspace file, and nothing ever compared it with
    /// what the machine it names actually holds. So a session that ended
    /// on the far side — the holder reaped, the box rebooted, somebody
    /// ended it from another Mac — left a row offering to return to it,
    /// for the life of the app and across restarts, because the row is
    /// persisted.
    ///
    /// THE EVIDENCE WAS ALREADY ARRIVING AND BEING THROWN AWAY. The host's
    /// listing is polled for the section's other rows; this asks it the
    /// one further question it can already answer.
    ///
    /// THREE THINGS IT WILL NOT DO. It does not touch a row that names no
    /// agent, because there is nothing to ask about one. It does not touch
    /// a row on another machine, because this host answered only for
    /// itself. And it does not touch a row ARCHIVED AFTER the listing was
    /// taken — the same rule that keeps a poll in flight from resurrecting
    /// an ended session ([[WI-2026-09-21-002]]), pointing the other way: a
    /// pane put away while the poll was out is not absent from it because
    /// it is gone.
    private func reconcileArchivedPanes(against listed: Set<String>,
                                        onHost hostID: UUID?, asOf: Date) {
        for i in workspaces.indices {
            workspaces[i].archivedPanes.removeAll { row in
                guard let agent = row.agent, row.host?.id == hostID, row.at < asOf
                else { return false }
                return !listed.contains(agent)
            }
        }
    }
    ...
    private func shown(_ agent: String, onHost hostID: UUID) -> Bool {
        openHere(agent, onHost: hostID) || workspaces.flatMap(\.archivedPanes).contains { row in
            row.agent == agent && row.host?.id == hostID
        }
    }

    /// … AND THE HALF OF IT THAT IS A CLIENT.
    ///
    /// AN ARCHIVED PANE IS A ROW, NOT A SEAT. Archiving ends the pane's
    /// client — that is precisely what leaves the session running with
    /// nobody attached — so the two halves of `shown` answer different
    /// questions and only this one answers "are we the ones holding it".
    /// Reading the whole of it for that is how returning to an archived
    /// pane displaced another Mac without asking ([[WI-2026-09-08-016]]).
    private func openHere(_ agent: String, onHost hostID: UUID) -> Bool {
        workspaces.flatMap(\.panes).contains { pane in
            guard self.host(ofLeaf: pane.id)?.id == hostID else { return false }
            let f = facts[pane.id]
    ...
    /// WHETHER THIS WORKBENCH ENDED THIS NAME ON THAT MACHINE, asked the
    /// way the hub names things — by peer rather than by host.
    ///
    /// BECAUSE TAKING THE SESSION ROW AWAY CAN UNCOVER AN AGENT ROW. The
    /// agent list subtracts what a session row already names, so the
    /// moment the ended session stops being listed, a directory entry the
    /// hub has not yet dropped becomes a row of its own — and the human
    /// sees a row for the thing they just ended change shape rather than
    /// go. The hub's entry expires on the hub's own clock; this is the
    /// fact this side already has ([[WI-2026-09-21-002]]).
    func confirmedEndedHere(_ agentID: String, onPeer machine: String) -> Bool {
        guard let host = TunnelManager.shared?.host(forPeer: machine) else { return false }
        return confirmedEnded(agentID, onHost: host.id)
    }
    ...
    ///
    /// READ OFF WHAT THE HOST LAST SAID, minus the seats that are OURS:
    /// an entry that still says `attached` is attached to a client that is
    /// not us — another Mac, or a `synapty attach` somebody ran on the
    /// host.
    ///
    /// NOT `remoteSessions`, WHICH SUBTRACTS TOO MUCH FOR THIS QUESTION.
    /// That list also drops what an ARCHIVED pane names, because it is
    /// about what to offer as somewhere to return to. An archived pane
    /// holds no seat, so subtracting it here hid the very row that says
    /// somebody else has taken one ([[WI-2026-09-08-016]]).
    ///
    ...
    func attachWouldDisplace(_ agentID: String, on host: HostEntry) -> Bool {
        seatTaken(agentID, on: host) != nil
    }

    /// WHO would be displaced, as their client said ([[RFC-0014]]
    /// C-CLIENT-LABEL); nil when the far side did not say — which is not
    /// the same as nobody being there, so this is never what decides
    /// whether to ask.
    func attachedClient(_ agentID: String, on host: HostEntry) -> String? {
        seatTaken(agentID, on: host)?.attachedBy
    }

    private func seatTaken(_ agentID: String, on host: HostEntry) -> RemoteSessions.Session? {
        guard !openHere(agentID, onHost: host.id) else { return nil }
        // NOR ONE THIS SIDE ENDED. Erring towards asking is right for a
        // listing that is merely old; it is not right for a session this
        // workbench was told is gone, where the dialog would name a seat
        // nobody is in ([[WI-2026-09-21-002]]).
        guard !confirmedEnded(agentID, onHost: host.id) else { return nil }
        return reportedSessions[host.id]?.first { $0.name == agentID && $0.attached }
    }
    ...
    ///
    /// AWAITED, AND THE ROW GOES ONLY ON A YES. Two End buttons sit in
    /// this one list and they kept different discipline: the host row's
    /// waits for the far side and keeps its row when the answer is no,
    /// while this one fired and forgot and removed the row either way. So
    /// a failed end read as a successful one — the session was still
    /// running, and the row that knew its title and which workspace it
    /// came from was gone, leaving the human the host's barer listing of
    /// the same name ([[WI-2026-09-22-002]]).
    ///
    /// A ROW THAT NAMES NO AGENT IS THE EXCEPTION, because there is
    /// nothing to ask: a pane archived before its session registered has
    /// no session to end, and the row IS the whole of what End can act on.
    /// Requiring a confirmation nobody can give would make it the one row
    /// the human cannot get rid of.
    @discardableResult
    func endArchivedPane(_ paneID: UUID) async -> Bool {
        guard let sIdx = workspaces.firstIndex(where: {
            $0.archivedPanes.contains { $0.id == paneID }
        }) else { return false }
        let row = workspaces[sIdx].archivedPanes.first { $0.id == paneID }!
        if let agent = row.agent, await !endAgentAwaiting(agent, host: row.host) {
            // THE END DID NOT CONFIRM, AND THAT IS TWO DIFFERENT FACTS.
            // "Still running, could not end it" keeps the row; "it was
            // already gone" must not, or End would be the one press that
            // does nothing on the one row a human most wants rid of. The
            // exit code cannot tell them apart — `synapty end` answers 2
            // for a name it does not hold, and 2 is also this CLI's
            // universal usage error — so the machine is asked instead.
            guard !sessionStillThere(agent, on: row.host) else { return false }
        }
        // THE INDEX IS RE-FOUND, NOT REUSED. An await sits above it, and
        // workspaces can be added, archived or removed while the far side
        // is being asked — `sIdx` describes a list that may no longer have
        // that shape.
        guard let now = workspaces.firstIndex(where: {
            $0.archivedPanes.contains { $0.id == paneID }
        }) else { return true }
        workspaces[now].archivedPanes.removeAll { $0.id == paneID }
        syncConnectionReferences()
        return true
    }
