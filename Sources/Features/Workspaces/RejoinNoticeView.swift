...
struct RejoinNoticeView: View {
    /// ONE OWNER FOR THE HEIGHT AND FOR WHERE EACH STRIP SITS
    /// ([[PaneStrip]]). This used to own both, and owning them here worked
    /// exactly as long as there was one kind of strip: a second notice
    /// insetting the pane by its own height would have drawn on top of
    /// this one, with the terminal under whichever won.
    static var height: CGFloat { PaneStrip.height }
