...
struct LeafFacts: Equatable {
    ...
    /// THE PANE'S OWN PROCESS IS GONE.
    ///
    /// Reached because the workbench takes
    /// `GHOSTTY_ACTION_SHOW_CHILD_EXITED` rather than letting libghostty
    /// answer for it. It is the only evidence there is in the one case
    /// nothing else speaks for — a host that keeps no session between
    /// connections, where the transport IS this pane's child and a dropped
    /// link kills it ([[LinkState]] `dropped`, [[WI-2026-09-07-004]]).
    ///
    /// A FACT, NOT A CODE. This held the exit code until the code was
    /// measured: libghostty spawns every pane through `/usr/bin/login`,
    /// which reports 0 for a child that exits 42, is killed, or does not
    /// exist. Storing an always-zero number invited the classification
    /// that read it ([[LinkState]] `dropped`, [[WI-2026-09-09-001]]).
    var childDied = false
