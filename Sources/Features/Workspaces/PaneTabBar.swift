...
struct PaneTab: View {
    ...
    /// The machine this pane is on, for its spine ([[HostSpine]]); nil
    /// for a local pane, which wears none.
    ...
    var body: some View {
        HStack(spacing: DS.Space.xs) {
            ...
            // NO IDENTITY DOT HERE ANY MORE ([[HostSpine]]). Which
            // machine is said along the tab's leading edge, because a
            // second 6pt dot beside the attention pulse taught the eye to
            // discount both ([[WI-2026-09-17-001]]).
        ...
        // WHICH MACHINE, ALONG THE EDGE ([[HostSpine]]). Over the
        // background so it reads as part of the tab's chrome, and under
        // the hit shape below so it takes no clicks of its own.
        .overlay(alignment: .leading) {
            if let hostLabel { HostSpineView(hostLabel: hostLabel) }
        }
        .contentShape(Rectangle())
        .onTapGesture {
            let now = Date()
        ...
        // THE MACHINE IS PART OF WHAT A PANE IS. The identity dot carried
        // an `accessibilityLabel` that nothing ever spoke: `children:
        // .ignore` above drops it, so a remote pane and a local one read
        // identically to VoiceOver. Moving the mark to a decoration is
        // what forces this to be said properly.
        .accessibilityLabel(hostLabel.map { "Pane: \(displayLabel), on \($0)" }
                            ?? "Pane: \(displayLabel)")
