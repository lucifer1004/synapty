...
struct HubStatusPopover: View {
    ...
    /// WHAT IS WRONG WITH THE HUB, as one place rather than an arm of a
    /// view's body — so it can be asked without a window, the way
    /// [[ContextStatusBar]]`.hubPhrase` already is.
    ///
    /// `canSpawn` and `restart` are passed IN rather than reached for,
    /// because the only thing this decides is what a human is told and
    /// which button they get for it.
    static func hubProblem(_ status: HubManager.HubStatus,
                           canSpawn: Bool,
                           restart: @escaping () -> Void) -> Problem? {
        switch status {
        case .lost(let why, let attempt):
            return Problem(
                message: "The hub went away: \(why). Restarting (attempt \(attempt + 1) of \(HubManager.recoveryAttempts))…",
                isFailure: false,
                action: ("Restart now", restart))
        case .running:
            return nil
        case .conflict(let why):
            ...
            return Problem(
                ...
                isFailure: true)
        case .starting:
            // NOT A FAILURE, AND NOT A BUTTON. This arm used to fall into
            // a `default` that answered the click with a red dot, "The hub
            // is not running", and a Start button for something already
            // starting — while the chip the human clicked to get here said
            // "Hub: starting…" in amber. Two surfaces describing one
            // condition in opposite colours, one click apart.
            return Problem(
                message: "The hub is starting.",
                isFailure: false)
        case .stopped:
            return Problem(
        ...
        case .failed(let why):
            // THE REASON, WHICH WAS ALREADY IN HAND. `.failed` carries one
            // and is reachable with four distinct ones — the recovery gave
            // up, the binary is not in the bundle, the spawn itself
            // failed, the hub never answered after starting. All four
            // arrived here as the single sentence "The hub is not
            // running", which is [[RFC-0015]] C-FAILURE on a state whose
            // reason the enum was built to carry. The file next door had
            // already learned this about `.conflict`
            // ([[WI-2026-09-11-011]]); a `default` is what hid it here.
            //
            // AND THE BUTTON ONLY WHERE IT CAN WORK. `restartNow()` is the
            // right answer to three of the four — the giving-up path even
            // says so in its own notification. It cannot be the answer to
            // a bundle with no hub binary in it: pressing Start re-runs
            // the same lookup, fails identically, and leaves the human
            // with no path to the one act that fixes it. "A control that
            // must fail is worse than none" is this file's own rule, three
            // arms up.
            return Problem(
                message: canSpawn
                    ? "The hub could not be started: \(why). Agents on this Mac cannot exchange messages."
                    : "The hub could not be started: \(why). This copy of the app has no hub binary "
                        + "in it, so starting cannot succeed — reinstalling the app is what fixes it.",
                isFailure: true,
                action: canSpawn ? ("Start", restart) : nil)
        }
    }
    ...
    private var problems: [Problem] {
        ...
        if let hub = Self.hubProblem(hubManager.status,
                                     canSpawn: SynaptyBinary.resolve() != nil,
                                     restart: { hubManager.restartNow() }) {
            out.append(hub)
        }
