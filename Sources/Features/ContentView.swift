...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: agentMonitor.agents) { _, agents in
            for agent in agents {
                ...
                switch agent.status {
                // ONE RECORD. This wrote two in lockstep and the
                // gestures that clear them had already diverged
                // ([[WI-2026-09-11-008]]).
                case "waiting", "done":
        ...
        .onChange(of: hubManager.boundPort) { (_: Int?, port: Int?) in
            pointEverythingAtTheHub(port)
        }
    ...
    /// POINT EVERY CONSUMER AT THE PORT THE HUB ANSWERED ON, or at
    /// nothing when it answered nothing.
    ///
    /// ONE OWNER, ONE FOLLOWER. `HubManager.boundPort` is the only thing
    /// that knows this number — it honours `SYNAPTY_HUB_PORT`, it climbs
    /// the ladder when 9000 is taken, and it is nil when adoption was
    /// REFUSED. Seven places held their own copy and two of them followed
    /// it; the other five were set once at launch, four of those with no
    /// path back at all. So a hub that moved, or a `conflict` the human
    /// was shown on screen, left wake reports, exec receipts, tool
    /// receipts and passive detection addressed to a literal 9000 — the
    /// very hub the refusal was about. The symptom is silence, which is
    /// why it survived ([[WI-2026-09-10-008]]).
    ///
    /// The agent list already followed this channel
    /// ([[WI-2026-09-02-029]]); this is the rest of the list joining it.
    @MainActor
    private func pointEverythingAtTheHub(_ port: Int?) {
        tunnelManager.hubPort = port
        agentDetector.hubPort = port
        wakeCoordinator.hubPort = port
        execController.hubPort = port
        toolBridge.hubPort = port
        if let port {
    ...
    /// EVERYTHING THIS WINDOW DOES ONCE, AT LAUNCH.
    ///
    /// A METHOD RATHER THAN AN INLINE CLOSURE, for the reason `statusBar`
    /// below gives: this closure lives inside `body`, so its hundred lines
    /// are part of one expression the type checker solves as a whole, and
    /// it was already at the budget. Adding one call to it tipped the file
    /// over ([[WI-2026-09-10-008]]). A named method gives the checker a
    /// boundary to stop at.
    @MainActor
    private func startWindow() {
        ...
        Task { @MainActor in
        // NO `?? 9000`. `start()` answers nil in exactly the cases
        // where there is no hub — a refused adoption, a failed
        // spawn — and 9000 is the one number that must not stand
        // in for that, because it names the hub the refusal was
        // about ([[WI-2026-09-10-008]]).
        let hubPort: Int? = await hubManager.start()
        ...
        // EVERY CONSUMER FOLLOWS THE ONE OWNER, through the one
        // function `onChange` also calls. Six of them were pointed
        // once here and never again; four had no path back at all,
        // so a hub that moved took wake reports, exec receipts,
        // tool receipts and passive detection with it and said
        // nothing ([[WI-2026-09-10-008]]).
        pointEverythingAtTheHub(hubPort)
        // Passive detection (RFC-0004 C-PASSIVE-DETECTION, ADR-0005).
        agentDetector.start(paneManager: paneManager, agentMonitor: agentMonitor)
        ...
        // with, instead of assuming a fresh launch starts alone. With no
        // hub there is nothing to adopt FROM — asking 9000 anyway would
        // adopt the peers of the hub adoption had just refused
        // ([[WI-2026-09-10-008]]).
        if let hubPort, let info = HubManager.queryHubInfo(port: hubPort) {
            tunnelManager.adoptExistingPeers(info.peers, capabilities: info.peerCapabilities,
                                             ours: info.capabilities)
        }
        taskMonitor.start()
    ...
    private func emitGazeIfDone(_ agentID: String) {
        ...
        HubEventClient.sendStatusSignal(port: hubManager.boundPort, agent: agentID, state: "idle")
    }
    ...
    /// rather than starting a fresh one: the far side runs `synapty
