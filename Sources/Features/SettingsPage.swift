...
struct SettingsPage: View {
            ...
            DSSectionBlock(
                ...
                DSFormField("Limit", density: .page) {
                    DSDropdown(
                        ...
                        options: [
                            // THE NUMBER THE FRAGMENT WRITES, not a third
                            // spelling of it. This label and the doc on
                            // `scrollbackLimit` each named 10,000
                            // independently ([[WI-2026-09-11-012]]).
                            (Int?.none,
                             "Default (\(SynaptySettings.defaultScrollbackLines.formatted()))"),
                            (Int?.some(1000), "1,000"),
    ...
    private var networkSection: some View {
            ...
            // A BOX THAT DID NOTHING, LABELLED AS THOUGH IT DID. "Tunnel
            // port" wrote a settings field that no tunnel ever read: the
            // peer forward is bound per host from `peerPort(for:)`, and
            // has been since the reverse tunnel was retired. The help text
            // beside it said "Applied on the next tunnel connection"
            // ([[WI-2026-09-10-003]]).
            DSSectionBlock(
                title: "Ports",
                help: "The hub port is automatic (9000 with fallback; SYNAPTY_HUB_PORT overrides), and each host's peer link is bound to its own free port. SSH connection robustness is automatic — fail-fast timeouts, keepalive probes and auto-reconnect."
            ) {
                Text("Ports are chosen automatically — one per host, so two connections cannot contend for the same number.")
                    .font(DS.Typography.detail)
                    .foregroundStyle(DS.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
                // [[RFC-0015]] C-RELEASE requires this number to be stated
                // where an operator can read it; it was stated in a source
                // comment ([[WI-2026-09-10-005]]).
                Text("A host's connection is kept for \(Int(ConnectionRegistry.defaultGrace)) seconds after its last pane closes, so reopening one costs nothing. Archiving a workspace or quitting releases it at once.")
                    .font(DS.Typography.detail)
                    .foregroundStyle(DS.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
