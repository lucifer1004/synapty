...
struct RemoteFilesView: View {
    ...
    private func load() {
        ...
        // WHICH REQUEST THIS IS, not only where it is for: two listings of
        // one directory are otherwise the same answer ([[FileBrowsing.request]]).
        ...
        guard let host else {
            ...
            if let why = listed.1 {
                ...
                browsing.arrive(at: expanded, files: listed.0, answering: ticket)
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                // AN ANSWER TO AN EARLIER QUESTION IS NOT AN ANSWER. A newer
                // navigation or a refresh started while this was in flight,
                // so this reply describes either a directory the pane is not
                // going to, or this directory as it was before — a deleted
                // file still in it. Checked by the ticket inside `arrive` and
                // `fail`, which is the only way to settle the pane.
                switch result {
                case .success(let listing):
                    ...
                    guard browsing.arrive(at: listing.canonicalPath, files: entries,
                                          answering: ticket) else { return }
                ...
                case .failure(let error):
                    ...
                    guard browsing.fail(answering: ticket) else { return }
