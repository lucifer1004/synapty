...
struct ServicesView: View {
    ...
    @State private var discovered: [PortDiscovery.Listener] = []
    ...
    /// a title somebody chose, and the most this can do is report what the
    /// machine says is holding the socket. So it sits under its own
    /// heading, below, behind a button.
    ///
    /// WHAT THE MACHINE SAYS IS STILL WORTH SAYING. These rows were the
    /// port number and nothing else, which is not something a human
    /// recognises among the dozens a development machine answers on
    /// ([[WI-2026-09-07-001]]).
    ///
    ...
    private var unclaimed: some View {
        VStack(alignment: .leading, spacing: DS.Space.xs) {
            ...
            ForEach(offerable) { listener in
                Button {
                    ...
                    Task { @MainActor in
                        if case .ok(let exposure) = await forwards.expose(
                            hostID: host?.id, remotePort: listener.port,
                            agent: "you", title: listener.title) {
                            show(exposure)
                        }
                    }
                } label: {
                ...
                .buttonStyle(.plain)
                .help(listener.command ?? listener.title)
            }
        }
        .padding(.horizontal, DS.Space.lg)
    }

    /// ONE ROW, THREE LINES AT MOST, AND THE PROGRAM ON TOP.
    ///
    /// The number was the whole row until [[WI-2026-09-07-001]], and a
    /// number identifies nothing: a development machine answers on dozens
    /// of ports and the human has no way to pick theirs out of them. The
    /// shape here is deliberately the one the agent-exposed rows above use
    /// — what it is, then what it is on — so the two lists read as one
    /// list with two provenances.
    ///
    /// WHAT EACH LINE SAYS IS THE LISTENER'S TO DECIDE, not this view's.
    /// The first attempt built the strings here, and printed `port 3000`
    /// twice on a listener the host would not name — a defect no test could
    /// reach, because it lived in a private method of a view that needs a
    /// forwarding service and a tunnel manager to exist at all.
    private func row(_ listener: PortDiscovery.Listener) -> some View {
        HStack(spacing: DS.Space.xs) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(listener.name)
                    .font(DS.Typography.body)
                    .lineLimit(1)
                if let detail = listener.detail {
                    Text(detail)
                        .font(DS.Typography.caption)
                        .foregroundStyle(DS.textTertiary)
                        .lineLimit(1)
                }
                if let arguments = listener.arguments {
                    Text(arguments)
    ...
    private var offerable: [PortDiscovery.Listener] {
    ...
    private func scan() {
        ...
        Task.detached(priority: .userInitiated) {
            let out = SubprocessRunner.run(
                ...
                timeout: 15)
            let listeners = PortDiscovery.parse(out.stdout)
            await MainActor.run {
    ...
    private var hostLabel: String { HostStore.inSentence(host) }
                        ...
                        HStack(spacing: DS.Space.sm) {
                            ...
                            VStack(alignment: .leading, spacing: 1) {
                                Text(exposure.title ?? "port \(exposure.remotePort)")
                                    .font(DS.Typography.body)
                                    .lineLimit(1)
                                // String(_:), not the Int — a port is an
                                // identifier and must not be formatted for
                                // the locale (`Listener.portText`).
