...
enum Whereabouts: Equatable {
    ...
    /// FILE-PRIVATE, SO NOTHING OUTSIDE CAN SETTLE THE PANE WITHOUT THE
    /// TICKET ([[FileBrowsing.arrive]], [[WI-2026-09-23-005]]).
    fileprivate mutating func arrive(at canonical: String, files: [BrowsedFile]) {
...
struct FileBrowsing: Equatable {
    ...
    mutating func navigate(to target: String, cached: [BrowsedFile]? = nil) {
        ...
        // AND WHATEVER WAS ASKED BEFORE IS NO LONGER THE QUESTION. A reply
        // for the directory being left, landing after this, would move the
        // human back to it ([[WI-2026-09-23-005]]).
        outstanding = nil
    }

    // MARK: - Which answer is the answer ([[WI-2026-09-23-005]])

    /// THE ONE LISTING REQUEST WHOSE REPLY MAY SETTLE THE PANE.
    ///
    /// A PATH CANNOT SAY WHICH REQUEST A REPLY ANSWERS. The view compared
    /// the path it asked for with the path it was now after, which tells a
    /// reply for somewhere else from one for here — and cannot tell two
    /// replies for HERE apart. A refresh after a delete is exactly that:
    /// the listing from before the delete and the one after it both name
    /// this directory, and if the older landed last, the deleted file came
    /// back.
    private(set) var outstanding: UUID?

    /// Ask for a listing. Every request supersedes every earlier one.
    mutating func request() -> UUID {
        let ticket = UUID()
        outstanding = ticket
        return ticket
    }

    /// Settle the pane with this reply, IF it answers the request still
    /// outstanding. False means it was superseded and nothing changed.
    @discardableResult
    mutating func arrive(at canonical: String, files: [BrowsedFile],
                         answering ticket: UUID) -> Bool {
        guard ticket == outstanding else { return false }
        outstanding = nil
        here.arrive(at: canonical, files: files)
        return true
    }

    /// The same rule for a request that failed.
    @discardableResult
    mutating func fail(answering ticket: UUID) -> Bool {
        guard ticket == outstanding else { return false }
        outstanding = nil
        here.fail()
        return true
