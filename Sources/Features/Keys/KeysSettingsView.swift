...
struct KeysSettingsView: View {
    ...
    /// WHAT A KEY THAT IS NOT A SHORTCUT IS TOLD, once.
    ///
    /// Two recorders answer `.notAChord` and both held this sentence.
    /// Their `.refused` siblings had ALREADY drifted — one names the chord
    /// and one does not — which is defensible, since a whole family has no
    /// single chord to name, but nothing recorded that and nothing
    /// distinguished it from the pair that simply had not drifted yet
    /// ([[WI-2026-09-11-012]]).
    static let notAChordMessage = "A shortcut needs ⌘, ⌃ or ⌥. Try again."
    ...
    private func startRecordingFamily(_ family: KeyCommandTable.Family) {
        ...
        dispatcher.beginRecording { chord in
            ...
            case .notAChord:
                account = .init(text: Self.notAChordMessage, tone: .warning)
            // NO CHORD TO NAME HERE, and that is why this sentence differs
            // from its sibling rather than having drifted from it: this
            // recorder answers for a whole FAMILY ([[WI-2026-09-11-012]]).
            case .refused:
