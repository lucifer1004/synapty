...
const deploy_targets = @import("src/deploy_targets.zig");
...
const ModuleSet = struct {
    ...
    /// The version in `build.zig.zon`, so no source file carries a second
    /// copy of it ([[WI-2026-09-11-004]]).
...
fn createModuleSet(
    ...
    // THE VERSION COMES FROM THE MANIFEST, which the justfile already
    // calls the one place it lives ("ONE PLACE: build.zig.zon. It was
    // hand-synced across three files and the three could disagree without
    // anything noticing"). That was true of the app, the DMG and the cask
    // and false of the binary: `mcp.zig` announced `"version":"0.1.0"` to
    // every MCP client from a literal, and the test beside it asserted the
    // same literal — so a bump left the server telling clients the old
    // number and the suite green about it ([[WI-2026-09-11-004]]).
    // THE LADDER, WRITTEN DOWN FOR THE SIDE THAT CANNOT IMPORT IT.
    //
    // `protocol.zig`'s header says "SWIFT HOLDS THE FIRST RUNG AND CANNOT
    // IMPORT THIS", and what kept the two equal was a Swift test asserting
    // the literal 60. That pins Swift to itself: drop `tool_exec_budget_ms`
    // and `tool_park_slack_ms` together and the Zig test still passes (its
    // three rungs are still ordered) and the Swift test still passes (60 is
    // still 60) while the hub un-parks the requester and reports expiry
    // with the workbench's subprocess still running — the exact failure
    // the ladder exists to prevent, one rung out from where it is checked
    // ([[WI-2026-09-11-015]]).
    //
    // Same shape as `deploy-targets.txt`: the build states the fact, and
    // the side that cannot import it reads what was built.
    const ladder_wf = b.addWriteFiles();
    const ladder_blob = comptime blk: {
        const p = @import("src/protocol.zig");
        break :blk std.fmt.comptimePrint(
            "tool_exec_budget_ms={d}\ntool_park_slack_ms={d}\ntool_park_ms={d}\ntool_wait_ms={d}\n",
            .{ p.tool_exec_budget_ms, p.tool_park_slack_ms, p.tool_park_ms, p.tool_wait_ms },
        );
    };
    const ladder_file = ladder_wf.add("tool-ladder.txt", ladder_blob);
    b.getInstallStep().dependOn(&b.addInstallFile(ladder_file, "tool-ladder.txt").step);

    // AND THE GEOMETRY A CLIENT ANNOUNCES WITH NO TERMINAL, for the same
    // reason and by the same route. A shell test asserting that a
    // restoration reaches the LAST row of the screen has to know which row
    // that is, and the copy it carried was unlinked from the one the
    // client sends ([[protocol.hello_fallback_rows]],
    // [[WI-2026-09-12-002]]).
    const geom_wf = b.addWriteFiles();
    const geom_blob = comptime blk: {
        const p = @import("src/protocol.zig");
        break :blk std.fmt.comptimePrint(
            "rows={d}\ncols={d}\n",
            .{ p.assumed_rows, p.assumed_cols },
        );
    };
    const geom_file = geom_wf.add("client-geometry.txt", geom_blob);
    b.getInstallStep().dependOn(&b.addInstallFile(geom_file, "client-geometry.txt").step);

    const version_opts = b.addOptions();
    version_opts.addOption([]const u8, "string", @import("build.zig.zon").version);
    const version = version_opts.createModule();
    ...
    return .{ .diag = diag, .protocol = protocol, .ipc = ipc, .run = run, .holder = holder, .mcp = mcp, .version = version, .io = io, .sys = sys, .github = github, .tools = tools, .framing = framing, .paths = paths };
}
...
pub fn build(b: *std.Build) void {
    ...
    // ONE STEP THAT CANNOT MISS A TARGET, so no caller has to name them.
    // `just deploy-all` spelled all five out and `project.yml` looped over
    // them again; either could go one short without anything noticing
    // ([[WI-2026-09-11-003]]).
    const deploy_all = b.step("deploy", "Cross-compile every deploy target");

    // AND THE LIST, WRITTEN DOWN FOR EVERYONE WHO IS NOT ZIG.
    //
    // `<directory>\t<uname -sm>` per line: the packaging shells take the
    // first field, and the Swift test takes both, so neither has to keep
    // a copy of its own. That test held a sixth one until this carried
    // the unames too ([[WI-2026-09-11-003]]).
    const names_blob = comptime blk: {
        var out: []const u8 = "";
        for (deploy_targets.all) |t| out = out ++ t.name ++ "\t" ++ t.uname ++ "\n";
        break :blk out;
    };
    const names_wf = b.addWriteFiles();
    const names_file = names_wf.add("deploy-targets.txt", names_blob);
    deploy_all.dependOn(&b.addInstallFile(names_file, "deploy-targets.txt").step);

    // THE LIST LIVES IN ONE PLACE ([[src/deploy_targets.zig]]). It was
    // written out here and in eight other spots, and the two tests that
    // guarded it each asserted a fixed five — so neither could notice a
    // sixth ([[WI-2026-09-11-003]]).
    inline for (deploy_targets.all) |deploy| {
        ...
        deploy_all.dependOn(&install_deploy.step);
    }
    ...
    // diag: no imports of its own either. It was created as a MODULE for
    // everything else to import and never added here, so its three tests
    // had never run — the same gap `refAllDecls` closes for a file reached
    // only through another's import graph, in the one shape that guard
    // cannot see ([[WI-2026-09-09-012]]).
    addTestModule(b, test_step, "src/diag.zig", &.{}, target, optimize);
    {
    ...
    inline for (.{ "run", "mcp" }) |name| {
        addTestModule(b, test_step, "src/" ++ name ++ ".zig", &.{
            ...
            .{ .name = "paths", .module = mods.paths },
            .{ .name = "framing", .module = mods.framing },
            // THE SAME MODULE THE BINARY GETS, so the test reads the
            // version off the manifest exactly as the server does.
            .{ .name = "version", .module = mods.version },
        }, target, optimize);
    }
